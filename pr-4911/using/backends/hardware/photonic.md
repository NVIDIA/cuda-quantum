...
Previous](neutralatom.html "Neutral Atom"){.btn .btn-neutral .float-left
...
::: {#photonic .section}
# Photonic[¶](#photonic "Permalink to this heading"){.headerlink}
...
computation, in which multiple single photons are interfered with each
other within a network of beam splitters, and photon detectors measure
where the photons leave this network. This process is implemented within
a time-bin interferometer (TBI) architecture where photons are created
in different time-bins and interfered within a network of delay lines.
This can be represented by a circuit diagram, like the one below, where
...
style="width: 400px;"}](../../../_images/orca_tbi.png){.reference
...
Programmers of CUDA-Q may access the ORCA API from either C++ or Python.
There is an environment variable [`ORCA_ACCESS_URL`{.docutils .literal
.notranslate}]{.pre} that can be set so that the ORCA target can look
for it during configuration.
...
Sometimes the requests to the PT-1 require an authentication token. This
token can be set as an environment variable named
[`ORCA_AUTH_TOKEN`{.docutils .literal .notranslate}]{.pre}. For example,
if the token is [`token123`{.code .docutils .literal
.notranslate}]{.pre}, you can set the environment variable as follows:
...
To set which ORCA URL to be used, set the [`url`{.code .docutils
...
You can then execute a time-bin boson sampling experiment against the
platform using an ORCA device.
...
::: {.tab-content .docutils}
To execute a boson sampling experiment on the ORCA platform, provide the
flag [`--target`{.docutils .literal .notranslate}]{.pre}` `{.docutils
.literal .notranslate}[`orca`{.docutils .literal .notranslate}]{.pre} to
the [`nvq++`{.docutils .literal .notranslate}]{.pre} compiler. You
should then pass the [`--orca-url`{.docutils .literal
.notranslate}]{.pre} flag set with the previously set environment
variable [`$ORCA_ACCESS_URL`{.docutils .literal .notranslate}]{.pre} or
an [`url`{.code .docutils .literal .notranslate}]{.pre}.
