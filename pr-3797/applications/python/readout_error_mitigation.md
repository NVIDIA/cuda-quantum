...
bitflip [`probs`{.docutils .literal .notranslate}]{.pre} can be the same
or different for all the qubits.
...
present in the add_channel method, so we model the Identity gate using a
rotation gate with an angle of 0.0.
:::
...
Let's run the [`ghz_kernel`{.docutils .literal .notranslate}]{.pre} with
the noise model. We will perform two experiments. First, we keep the
bitflip probability the same for all the qubits. In the second
experiment, we use different bitflip probabilities for qubits.
...
[\\(A_1\\)]{.math .notranslate .nohighlight} for a single qubit can be
defined as

::: {.math .notranslate .nohighlight}
\\\[\\begin{split}A_1 = \\begin{bmatrix} P(0\|0) & P(0\|1)\\\\ P(1\|0) &
...
.notranslate .nohighlight}. Using [\\(A_1\\)]{.math .notranslate
