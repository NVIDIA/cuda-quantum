...
def test_shift_phase_absorbed():
    ...
    assert abs(drive_ops[0].attrs["frame_phase_offset"] - math.pi / 2) < 1e-10
...
def test_consecutive_shifts_merge():
    ...
    assert len(drive_ops) == 1
    assert abs(drive_ops[0].attrs["frame_phase_offset"] - 0.3) < 1e-10
...
def test_virtual_z_persists_across_drives():
    ir = _vz_persists(pulse.qudit_ref())
    prog = to_program(ir, clock_ghz=2.0, qubit_freq_hz={0: 5.0e9})
    result = run_virtual_z(prog)
    drive_ops = [op for op in result.ops if op.kind == OpKind.DRIVE]
    assert len(drive_ops) == 2
    assert all(
        abs(op.attrs["frame_phase_offset"] - 0.4) < 1.0e-10 for op in drive_ops)
    # The first drive consumes the allocation tone; the second consumes the
    # first drive's updated tone, preserving linear SSA.
    assert drive_ops[1].operands[2].vid == drive_ops[0].results[1].vid
