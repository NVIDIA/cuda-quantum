...
def _helper_waveform():
    return gaussian(32, 0.2, 8.0)


def _helper_drive(line, tone, waveform):
...
class TestBasicCompilation:
    ...
    def test_pulse_helpers_are_inlined(self):

        def helper_kernel(q0):
            d0, t0 = get_drive_line(q0)
            wf = _helper_waveform()
            _helper_drive(d0, t0, wf)

        ir = compile_kernel_bytecode(helper_kernel)(qudit_ref())
        kinds = [op.kind for op in ir.ops]
        assert kinds == [
            "pulse.qudit_arg",
            "pulse.get_drive_line",
...
class TestForLoop:
    ...
    def test_for_range_is_fully_unrolled(self):
        ...
        kinds = [op.kind for op in ir.ops]
        assert kinds.count("pulse.drive") == 5
        assert "scf.for" not in kinds

    def test_for_loop_does_not_leave_region_markers(self):
        ...
        kinds = [op.kind for op in ir.ops]
        assert kinds.count("pulse.drive") == 3
        assert "scf.yield" not in kinds
