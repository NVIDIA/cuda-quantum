...
def qudit_ref(index: int | None = None) -> QuditRef:
    """Create a qudit reference, optionally bound to a physical index."""
    ...
...
class CompiledKernel:
    ...
    def __call__(self, *args: float | int,
                 **kwargs: float | int) -> "CompiledKernel":
    ...
    def lower_to_llvm(self) -> str:
        """Lower the scheduled pulse module to LLVM IR."""
        ...

    def run(self,
            *,
            entry: str = "main",
            n_qubits: int | None = None) -> list[Any]:
        """Execute the compiled pulse module with cuDensityMat."""
        ...


class EvolveResult:
    """Final state and integration-time metadata from ``evolve``."""

    final_state: Any
    times: Any
    expectation_values: dict[str, Any] | None
...
def evolve(
    ...
    *,
    ...
    """Compile and evolve a pulse program on the cuDensityMat GPU path."""
    ...
...
def shift_phase(tone: Tone, phase: Numeric) -> Tone:
    """Add a relative phase offset to a tone's rotating frame."""
    ...


def set_phase(tone: Tone, phase: Numeric) -> Tone:
    """Set the absolute phase of a tone's rotating frame."""
    ...


def shift_frequency(tone: Tone, frequency: Numeric) -> Tone:
    """Add a relative frequency offset to a tone."""
    ...


def set_frequency(tone: Tone, frequency: Numeric) -> Tone:
...
def tanh_ramp(duration: Numeric, amplitude: Numeric,
              sigma: Numeric) -> Waveform:
...
def custom_samples(samples: Sequence[float]) -> Waveform:
