...
Default C6 coefficient corresponds to Rb-87 |70S_{1/2}> Rydberg state. The
stored value is 862690 MHz um^6; Hamiltonian construction converts it to the
angular coefficient 862690 * 2pi rad MHz um^6.
...
_DEFAULT_C6_MHZ_UM6 = 862690.0  # Cyclic-frequency coefficient, MHz * um^6.
...
class RydbergTarget:
    ...
    c6 : float
        Van der Waals coefficient in MHz * um^6 (before conversion to angular
        frequency).
    ...
    def hamiltonian_terms(self) -> List[Dict[str, Any]]:
        ...
        for atom in self.atoms:
            rabi_rad = self.global_rabi_mhz * 1e-3 * 2.0 * math.pi
            terms.append({
                "kind": "rabi_drive",
            ...
            if self.global_detuning_mhz != 0:
                delta_rad = self.global_detuning_mhz * 1e-3 * 2.0 * math.pi
                terms.append({
                    "kind": "detuning",
                    "qubit_indices": (atom.index,),
    ...
    def dissipator_terms(self) -> List[Dict[str, Any]]:
        ...
        terms: List[Dict[str, Any]] = []
        for atom in self.atoms:
            gamma = 1.0 / (1000.0 * 1e3)  # ~1/ms, expressed in 1/ns
            terms.append({
