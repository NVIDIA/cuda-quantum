...
from ._bytecode_normalize import CanonicalInstr, normalize
from ._structure import Block, ForLoop, IfElse, Region, recover_structure
...
def _extract_closures(fn: types.FunctionType) -> dict[str, Any]:
    """Return the values captured by a Python helper function."""
...
def _uses_pulse_ops(fn: types.FunctionType,
                    seen: set[int] | None = None) -> bool:
    """Whether *fn* directly or transitively calls the pulse DSL."""
    seen = seen or set()
    if id(fn) in seen:
        return False
    seen.add(id(fn))
    ...
    for name in fn.__code__.co_names:
        if name in _PULSE_OP_NAMES:
            return True
        value = fn.__globals__.get(name)
        if isinstance(value, types.FunctionType) and _uses_pulse_ops(
                value, seen):
            return True
    return False
...
class PulseIREmitter:
    ...
    def _emit_for_loop(self, loop: ForLoop) -> None:
        values = self._extract_range_values(loop)
        if values is None:
            raise CompilationError(
                "range() bounds must be compile-time integers in pulse kernels")

        # The packed compiler has no region encoding. Always unroll here so
        # loop bodies cannot be silently emitted only once.
        for i in values:
    ...
    def _emit_if_else(self, ifelse: IfElse) -> None:
        ...
        if isinstance(cond, IRValue):
            raise CompilationError(
                "runtime-dependent branches are not supported in the pulse "
                "research preview; measurement results cannot control Python "
                "if statements")
        elif cond:
    ...
    def _dispatch_call(self, callable_val: Any,
        ...
        # Inline user helpers which consume IR values or call the pulse DSL.
        # Executing these as ordinary Python would invoke pulse operations
        # outside the compiler context and lose their IR effects.
        if isinstance(callable_val, types.FunctionType) and (
                any(isinstance(arg, IRValue) for arg in call_args) or
                _uses_pulse_ops(callable_val)):
            self._inline_helper(callable_val, call_args)
            return

        # Regular Python callable (int, float, len, abs, math.sin, etc.)
        # int()/float() on a symbolic value create an explicit IR cast.
        if callable(callable_val):
            if len(call_args) == 1 and is_symbolic(call_args[0]):
                if callable_val is int:
                    self.stack.append(call_args[0].cast("i64"))
                    return
                if callable_val is float:
                    self.stack.append(call_args[0].cast("f64"))
                    return
            if any(is_symbolic(a) for a in call_args):
                raise CompilationError(
                    f"Cannot call {fname}() with symbolic parameter "
                    "expressions. Only int(), float(), arithmetic, and pulse "
    ...
    def _inline_helper(self, fn: types.FunctionType,
                       call_args: list[StackVal]) -> None:
        """Emit a pulse-aware Python helper into the current IR builder."""
        code = fn.__code__
        params = list(code.co_varnames[:code.co_argcount])
        if len(call_args) != len(params):
            raise CompilationError(
                f"{fn.__name__}: expected {len(params)} args, got {len(call_args)}"
            )

        helper = PulseIREmitter(
            self.b,
            fn_globals=fn.__globals__,
            fn_closures=_extract_closures(fn),
        )
        helper.locals.update(zip(params, call_args))
        helper.emit_regions(recover_structure(normalize(code)))

        # Propagate rebindings of directly passed linear values back to the
        # caller, then leave the helper's return value on the caller stack.
        for name, original in zip(params, call_args):
            updated = helper.locals.get(name)
            if not isinstance(original, IRValue) or updated is original:
                continue
            for caller_name, caller_value in self.locals.items():
                if caller_value is original:
                    self.locals[caller_name] = updated

        self.stack.append(helper.stack[-1] if helper.stack else None)
    ...
    def _push_pulse_results(
        ...
        if fname == "readout":
            self.stack.append(results[-1])
        elif fname in {"drive", "wait", "sync"}:
            return
        elif len(results) == 1:
    ...
    def _emit_pulse_call(
        ...
        if n_val == -1:
            ...
            if len(attr_vals) != len(attr_names):
                raise CompilationError(
                    f"{fname}: expected {n_val + len(attr_names)} args, got {len(call_args)}"
                )
            attrs = dict(zip(attr_names, attr_vals))
            out_types = (operands[0].vtype,) if (rtypes is None and
                                                 operands) else (rtypes or ())
    ...
    def _extract_range_values(self, loop: ForLoop) -> range | None:
        """Extract exact compile-time ``range`` values for a loop."""
        if self.stack:
            top = self.stack[-1]
            if isinstance(top,
        ...
        for ci in loop.range_setup:
            if ci.op == "LOAD_CONST":
                if isinstance(ci.arg, int):
                    return range(ci.arg)
                return None
        for ci in loop.range_setup:
            if ci.op == "LOAD_FAST" and ci.arg in self.locals:
                value = self.locals[ci.arg]
                return range(value) if isinstance(value, int) else None
        return None
