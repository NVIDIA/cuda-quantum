...
- **Linear-resource validity**: Ensures drive/readout lines have exactly one
  continuation at each step and detects multiple uses of a consumed line.
- **Monotone time ordering**: After scheduling, traces each physical line and
  verifies that drive/readout intervals do not overlap or move backwards.
