...
Once the `nvcc`{.code .docutils .literal .notranslate} compiler is
installed, it is possible to write CUDA kernels and have them execute on
the system GPU. See NVIDIA's [An Easy Introduction to CUDA C and
...
the extension `.cu`{.code .docutils .literal .notranslate}. For our
example, assume we have written our CUDA code in the file
`my_proj.cu`{.code .docutils .literal .notranslate}.
...
file with the `.cpp`{.code .docutils .literal .notranslate} extension.
Again for our example, let's assume that we've written quantum kernels
and saved them in the file `my_proj_quantum.cpp`{.code .docutils
.literal .notranslate}.

By default, CUDA-Q uses C++ 20 and builds source code against the LLVM
C++ standard library (`libc++`{.code .docutils .literal .notranslate}).
To create a CUDA library that can link against CUDA-Q code, make sure to
define an API that does not rely on C++ data structures that rely on a
specific C++ toolchain for all functions intended to be called from
CUDA-Q (see also [[Interfacing between binaries compiled with a
different toolchains]{.std
...
.literal .notranslate} declaring the `setFirstNElements`{.code .docutils
.literal .notranslate} template. You can then call
`setFirstNElements`{.code .docutils .literal .notranslate} from within
`my_proj_quantum.cpp`{.code .docutils .literal .notranslate} by
including `my_proj.h`{.code .docutils .literal .notranslate}, for
example
...
Above, `nvq++`{.code .docutils .literal .notranslate} is used for the
link step and will make sure the CUDA-Q runtime libraries are linked
correctly to the executable program. The CUDA runtime is explicitly
