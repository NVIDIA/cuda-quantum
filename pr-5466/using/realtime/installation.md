...
Previous](../realtime.html "CUDA-Q Realtime"){.btn .btn-neutral
.float-left accesskey="p"} [Next []{.fa .fa-arrow-circle-right
aria-hidden="true"}](host.html "CUDA-Q Realtime Host API"){.btn
...
We provide pre-built installers for CUDA-Q Realtime that can be
downloaded from our [GitHub
Releases](https://github.com/NVIDIA/cuda-quantum/releases){.reference
.external}, starting with the CUDA-Q release tag 0.14.0. The binaries
are available for [`x86_64`{.code .docutils .literal
.notranslate}]{.pre}, and [`ARM64`{.code .docutils .literal
.notranslate}]{.pre} Linux system with CUDA Versions 12.6+ (Driver
560.35.05+) and 13.x (Driver 580.65.06+). Instructions for building
CUDA-Q Realtime from source can be found on our [GitHub
repository](https://github.com/NVIDIA/cuda-quantum/tree/main/realtime/docs/){.reference
.external}.
...
-   [DOCA 3.3.0
    installation](https://developer.nvidia.com/doca-downloads){.reference
    .external} with [`gpunetio`{.code .docutils .literal
    .notranslate}]{.pre} support.

::: {.admonition .note}
Note

Please make sure [`doca-sdk-gpunetio`{.code .docutils .literal
.notranslate}]{.pre} is installed along with [`doca-all`{.code .docutils
...
::: {.tab-content .docutils}
CUDA-Q Realtime defines a Network Provider Interface to build a
networking-agnostic application. This interface consists of a set of
APIs to construct a real-time RPC dispatch solution in a
networking-agnostic manner. These APIs are backed by a provider plugin
(a shared library) implementing the specific transport protocol.

A guide that explains how to integrate a new networking provider with
CUDA-Q Realtime via the Network Provider Interface can be found
[here](https://github.com/NVIDIA/cuda-quantum/tree/main/realtime/docs/cudaq_realtime_network_interface.md){.reference
.external}.
:::
:::
:::
...
## Latency Measurement[¶](#latency-measurement "Permalink to this heading"){.headerlink}

The CUDA-Q Realtime installer contains a validation script that includes
a latency measurement. After completing the installation and setup
steps, you should find the [`validate.sh`{.code .docutils .literal
.notranslate}]{.pre} script in the CUDA-Q Realtime installation location
(usually in [`/opt/nvidia/cudaq/realtime`{.code .docutils .literal
.notranslate}]{.pre}). The script executes a simple RPC dispatch tests,
whereby the FPGA sends data (array of bytes) to the GPU, the GPU
performs a simple increment by one calculation on each of the byte in
the incoming array (unless in the [`--forward`{.code .docutils .literal
.notranslate}]{.pre} mode) and returns the array to the FPGA. The
validation includes checking the data correctness and measuring the
round-trip latency.
...
The command line arguments need to be adjusted based on the system
setup: - [`--device`{.code .docutils .literal .notranslate}]{.pre} is
the [`IB`{.code .docutils .literal .notranslate}]{.pre} device name that
is connected to the FPGA. - [`--gpu`{.code .docutils .literal
.notranslate}]{.pre} is the GPU device Id that we want to run the RPC
callback on. - [`--fpga-ip`{.code .docutils .literal
.notranslate}]{.pre} is the IP address of the [`HSB`{.code .docutils
.literal .notranslate}]{.pre} FPGA. - [`--bridge-ip`{.code .docutils
...
the [`RPCHeader`{.code .docutils .literal .notranslate}]{.pre} /
[`RPCResponse`{.code .docutils .literal .notranslate}]{.pre} (see the
message protocol documentation) to capture the timestamp for latency
analysis. Specifically, the stimulus tool (FPGA) stores the 'send'
timestamp in the [`RPCHeader`{.code .docutils .literal
.notranslate}]{.pre} (incoming message), which will be echoed by the GPU
in the outgoing [`RPCResponse`{.code .docutils .literal
.notranslate}]{.pre} after processing it (e.g., with the RPC handler).
Using the Integrated Logic Analyzer timestamp when the FPGA receives the
response from the GPU, we can compute the round-trip latency. [This
file](https://github.com/NVIDIA/cuda-quantum/tree/main/realtime/unittests/utils/hsb_fpga_playback.cpp){.reference
.external} contains an example of such a data generation tool.
:::
:::
:::
:::
