...
| Metal resident gate paths | `test_mklq_metal_correctness_fixtures.py`, `MKLQMetalTester.cpp` | Resident single-target, controlled single-target, parameterized phase, two-target, and control gate kernels | Does not prove full GPU residency for every operation |
