...
| Metal runtime counter probe | `python3 benchmarks/mklq/run_metal_runtime_counter_probe.py --build-dir build-python --output benchmarks/mklq/reports/local-metal-runtime-counter-probe-YYYY-MM-DD.counter.json` | The complete expected build-tree Metal counter-test set is present and each exact `mklq_metal_MKLQMetalTester.*` ctest asserts real `MetalStateVectorExecutor` runtime counters for resident gate, probability, sampling, measurement, collapse, reset, and unsupported-gate fallback/reupload paths | Timing performance, release sign-off, or proof that every `mklq-metal` operation stays on Metal |
...
| `python/tests/backends/test_mklq_metal_correctness_fixtures.py` | Experimental Metal mixed-path fixtures | Resident single-target, controlled single-target, two-target, Python custom three-target fallback followed by supported gates, QFT-like, seeded Clifford, deterministic sampling, resident measurement feedback, resident reset after measurement, qpp-cpu tolerance comparisons |
...
| `unittests/nvqpp/backends/MKLQMetalTester.cpp` | Metal runtime and mixed-path simulator tests | Metal device detection, resident gate kernels, probability fills, marginal paths, measurement/collapse/reset paths, unsupported-gate fallback/reupload behavior, and fallback/error behavior |
...
| Metal resident gate paths | `test_mklq_metal_correctness_fixtures.py`, `MKLQMetalTester.cpp` | Resident single-target, controlled and multi-control single-target, parameterized Rx/Ry/Rz rotations, phase-family S/T/Sdg/Tdg gates, two-target, control gate kernels, Python custom three-target fallback correctness, and reupload after unsupported gate fallback | Does not prove full GPU residency for every operation |
...
| Benchmark tooling | `test_mklq_benchmark_harness.py`, benchmark helper `py_compile`, summary JSON parse, `check_performance_evidence.py`, `check_metal_evidence.py`, `run_metal_runtime_counter_probe.py`, `summarize_metal_runtime_counters.py` | Benchmark case/schema/report changes, tracked clean CPU evidence quality, tracked experimental Metal evidence boundary quality, bounded runtime counter evidence parsing, and public counter-summary rendering | Does not prove current runtime speed unless real benchmark rows run |
