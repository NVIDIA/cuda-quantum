...
| Metal resident gate paths | `test_mklq_metal_correctness_fixtures.py`, `MKLQMetalTester.cpp` | Resident single-target, controlled single-target, parameterized Rx/Ry/Rz rotations, phase-family S/T/Sdg/Tdg gates, two-target, and control gate kernels | Does not prove full GPU residency for every operation |
