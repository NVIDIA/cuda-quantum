...
int main() {
  ...
  // However, the example implies direct invocation logic for pass-by-value illustration.
  // To test this, we'd ideally check host vars after a `cudaq::sample` or similar.
  // Let's use cudaq::sample to ensure the kernel is processed.
