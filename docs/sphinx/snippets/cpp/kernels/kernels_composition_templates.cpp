...
void callKernels_cpp() {
  ...
  // MyStatePrep_cpp is __qpu__ only, cannot be passed directly as callable from host to __qpu__ operator()
  // The generic algorithm itself must be an entry point, or called by one.
  // Let's make an entry point that uses MyGenericAlgorithm_cpp
  auto entry_point_algo1 = [&]() __qpu__ {
      algorithm(MyStatePrep_cpp);
  };
  cudaq::sample(entry_point_algo1);


  MyGenericAlgorithm2_cpp anotherVersion;
  auto entry_point_algo2 = [&]() __qpu__ {
      anotherVersion(MyStatePrep_cpp);
  };
