...
// Compile and run with: `nvq++ synthesis_examples.cpp && ./a.out`
...
__qpu__ void adjoint_example_kernel_cpp(int N_qubits, double angle_param) {
  ...
  cudaq::qubit &target_qubit =
      q_vec[0]; // Use first qubit as target for simplicity
...
int main() {
  ...
  // For adjoint, if we apply Op then Op_adj, we should return to initial state
  // (often |0...0>) Let's run a sequence: H, RX(pi/2), then adjoint(RX_H_gate,
  // pi/2) RX_H_gate(pi/2) = RX(pi/2) H Adjoint(RX_H_gate, pi/2) = H RX(-pi/2)
  ...
  auto counts_adjoint_rst =
      cudaq::sample(adjoint_example_kernel_cpp, 3, M_PI_4);
  ...
  // If qubit0=|0> (true), qubit1=|0> (!qubit1 is true), then X applied to
  // target. Default state |000> -> control active -> |001>
