...
// Compile and run with: `nvq++ optimizer_lbfgs_gradient_example.cpp && ./a.out`
...
cudaq::spin_op h_deut_lbfgs =
    5.907 - 2.1433 * cudaq::spin_op::x(0) * cudaq::spin_op::x(1) -
    2.1433 * cudaq::spin_op::y(0) * cudaq::spin_op::y(1) +
    .21829 * cudaq::spin_op::z(0) - 6.125 * cudaq::spin_op::z(1);
cudaq::spin_op h3_deut_lbfgs =
    h_deut_lbfgs + 9.625 - 9.625 * cudaq::spin_op::z(2) -
...
int main() {
  cudaq::gradients::central_difference gradient(deuteron_n3_ansatz,
                                                argsMapper_lbfgs);
  cudaq::optimizers::lbfgs optimizer;
  optimizer.initial_parameters = std::vector<double>{0.5, 0.5};

  auto [min_val, opt_params] =
      optimizer.optimize(2, [&](const std::vector<double> &x_lambda,
                                std::vector<double> &grad_vec) {
        auto cost = cudaq::observe(deuteron_n3_ansatz, h3_deut_lbfgs,
                                   x_lambda[0], x_lambda[1]);
        grad_vec = gradient.compute(
            x_lambda,
            [&](const std::vector<double> &p_grad) {
              return cudaq::observe(deuteron_n3_ansatz, h3_deut_lbfgs,
                                    p_grad[0], p_grad[1]);
            },
            cost);
        return cost;
