...
def generate_tokens(results_fpath_str: str,
                    save_path_str: str,
                    config_path_temp: str,
                    n_nodes: int,
                    rounding_digits=2,
                    min_block_size=128,
                    max_block_size=256,
                    ...
                    perform_coef_mod_range=True,
                    apply_sliding_window=True,
                    n_workers=1,
                    skip_only_qaoa_circ=False,
                    allowed_graph_generators_list=['all'],
                    approx_ratio_thr=0.97,
                    debug_limit=0,
                    target_val_size=10,
                    verbose=True):
    """
    ...
    for results_fpath in results_fpath_list:
        assert results_fpath.exists() and results_fpath.is_dir(
        ), "Results path is invalid."

    if verbose:
        print(f"Results paths: {results_fpath_list}")

    ##########################################
    ## ADAPT-QAOA results
    ###########################################
    if verbose:
        print("Opening ADAPT-QAOA results...")
    df_list = []
    df_list_all = []

    for cur_dataset_res_path in results_fpath_list:
        cur_dataset_res_flist = sorted(
            cur_dataset_res_path.joinpath('res').glob('*.csv'))
        if debug_limit:
        ...
        df_list = Parallel(n_jobs=n_workers)(
            delayed(open_df_from_res_csv)(fname) for fname in tqdm(
                cur_dataset_res_flist,
                desc=f'Opening ADAPT results ({cur_dataset_res_path.stem})'))
        df_list_all += df_list
    ...
    full_run_df['prefix'] = full_run_df['worker_id'].apply(lambda x: x[:-15])

    ####################################
    ## Graphs
    ####################################
    if verbose:
        print("Opening graphs results...")
    df_list = []
    df_list_all = []
    for cur_dataset_res_path in results_fpath_list:
        cur_dataset_res_flist = sorted(
            cur_dataset_res_path.joinpath('graphs').glob('*.csv'))
        if debug_limit:
            cur_dataset_res_flist = cur_dataset_res_flist[:debug_limit]
        ...
        df_list = Parallel(n_jobs=n_workers)(
            delayed(open_df_from_res_csv)(fname) for fname in tqdm(
                cur_dataset_res_flist,
                desc=f'Opening graphs ({cur_dataset_res_path.stem})'))
        df_list_all += df_list
    ...
    full_run_graphs_df['edgelist_list_len'] = (
        full_run_graphs_df['edgelist_list'].progress_apply(lambda x: len(x)))
    full_run_graphs_df['num_connected_comp'] = full_run_graphs_df[
        'edgelist_list'].progress_apply(lambda x: len(
            list(nx.connected_components(nx.Graph([edge[:2] for edge in x])))))
    ...
    ###################################################
    if verbose:
        print("Merging ADAPT-QAOA results and graphs results...")
    ...
    combined_res_df = pd.merge(left=full_run_df,
    ...
    if verbose:
        ...
        print("Unique prefixes in full_run_graphs_df:",
              full_run_graphs_df['prefix'].unique())

        print("Unique graph_num in full_run_df:",
              full_run_df['graph_num'].unique())
        print("Unique graph_num in full_run_graphs_df:",
              full_run_graphs_df['graph_num'].unique())
    ...
    combined_res_df['graph_id'] = (combined_res_df['prefix'] + '_^_' +
                                   combined_res_df['graph_num'].astype(str))

    # convert mixer_pool_index from string to list of ints
    combined_res_df['mixer_pool_index'] = combined_res_df[
        'mixer_pool_index'].apply(
            lambda x: [int(i) for i in ast.literal_eval(x)]
            if isinstance(x, str) else x)

    combined_res_df['only_qaoa_circ'] = combined_res_df[
        'mixer_pool_index'].progress_apply(
    ...
    if allowed_graph_generators_list != ['all']:
        if verbose:
            print(
                f"Filtering graphs based on allowed generators: {allowed_graph_generators_list}"
            )
        if verbose:
            print(f"N circuits before: {len(combined_res_df)}")
        combined_res_df = combined_res_df[combined_res_df['g_method'].isin(
            allowed_graph_generators_list)]
        if verbose:
    ...
    combined_unique_graphs_df = (combined_res_df[['graph_id', 'edgelist_json'
                                                 ]].drop_duplicates())
    ...
    combined_unique_graphs_df['edgelist_py_list'] = combined_unique_graphs_df[
        'edgelist_json'].progress_apply(lambda x: [(e[0] - 1, e[1] - 1, e[2])
    ...
    combined_unique_graphs_df['graph_nx'] = (
        combined_unique_graphs_df['edgelist_py_list'].progress_apply(
            lambda x: convert_node_labels_to_integers(
                create_weighted_graph_nx(x))))

    combined_unique_graphs_w_idx_df = combined_unique_graphs_df.set_index(
        'graph_id')
    graphs_nx_dict = combined_unique_graphs_w_idx_df['graph_nx'].to_dict()
    graphs_nx_filt_dict = dict([(name, g)
                                for name, g in tqdm(graphs_nx_dict.items())
    ...
    emb_graph_idx_to_id_dict = {
        k: v for k, v in enumerate(graphs_nx_filt_names)
    }
    emb_graph_id_to_idx_dict = {
        v: k for k, v in enumerate(graphs_nx_filt_names)
    }
    ...
    def get_feather_emb(g_list):
        ...
        feather_model = FeatherGraph(order=5,
                                     eval_points=25,
                                     theta_max=2.5,
                                     seed=42,
                                     pooling="mean")
    ...
    def embed_nx_w_feather_parallel(graphs_list, n_workers=2):
        ...
        emb_np_list = Parallel(n_jobs=n_workers)(
            delayed(get_feather_emb)(g_chunk)
            for g_chunk in graphs_chunked_list)

        return np.vstack(emb_np_list)

    feather_par_emb = embed_nx_w_feather_parallel(graphs_nx_filt_list[:],
                                                  n_workers=n_workers)
    ...
    if verbose:
        print(
            f"Number of rows in combined_res_df before filtering: {len(combined_res_df)}"
        )

    # Convert gamma_coef from string to list of floats
    combined_res_df['gamma_coef'] = combined_res_df['gamma_coef'].apply(
        lambda x: [float(coef) for coef in ast.literal_eval(x)]
        if isinstance(x, str) else x)

    # convert approx_ratio from string to float
    combined_res_df['approx_ratio'] = pd.to_numeric(
        combined_res_df['approx_ratio'], errors='coerce')
    ...
    combined_res_filt_df = combined_res_df[
        ...
        (combined_res_df['gamma_coef'].apply(
            lambda x: all([abs(coef) < max_abs_param_val for coef in x]))) &
        (combined_res_df['approx_ratio'] > approx_ratio_thr)].copy()
    ...
    if skip_only_qaoa_circ:
        ...
        if verbose:
            print(
                f"Removing {n_only_qaoa_circ} out of total {len(combined_res_filt_df)}"
            )
    ...
    combined_res_filt_df['gamma_coef'] = combined_res_filt_df[
        'gamma_coef'].apply(
            lambda x: [float(coef) for coef in ast.literal_eval(x)]
            if isinstance(x, str) else x)

    combined_res_filt_df['mixer_pool_index'] = combined_res_filt_df[
        'mixer_pool_index'].apply(
            lambda x: [int(idx) for idx in ast.literal_eval(x)]
            if isinstance(x, str) else x)

    if verbose:
        print(
            f"Number of rows in combined_res_filt_df after filtering: {len(combined_res_filt_df)}"
        )
    ...
    ## Special symbols
    special_symbols_list = ['pad', 'bos', 'eos', 'new_layer_p', 'end_of_graph']
    ...
    all_coefs_round_set = set([
        ...
        for coef in np.linspace(start=-max_abs_param_val,
                                stop=max_abs_param_val,
                                num=n_steps).tolist()
    ])
    ...
    combined_res_filt_df[
        f'token_seq_round_d{rounding_digits}'] = combined_res_filt_df.progress_apply(
    ...
    combined_res_tok_df[f'token_int_seq_round_d{rounding_digits}'] = (
        combined_res_tok_df[f'token_seq_round_d{rounding_digits}'].
        progress_apply(lambda x: [token_to_int_idx_dict[token] for token in x]))

    ########################################################
    # Generating training split for nanoGPT
    ########################################################
    if verbose:
        print("Generating training, validation and test splits...")

    n = len(combined_res_tok_df)

    combined_res_tok_shf_df = (combined_res_tok_df.sample(frac=1).reset_index(
        drop=True))
    ...
    n_test = int(n_total *
                 test_frac)  # Define test_frac for the size of the test set
    ...
    combined_res_tok_shf_df.loc[
        combined_res_tok_shf_df['graph_id'].isin(val_graph_ids_set),
        'label'] = 'val'
    combined_res_tok_shf_df.loc[
        combined_res_tok_shf_df['graph_id'].isin(test_graph_ids_set),
        'label'] = 'test'

    if apply_sliding_window:
        print('Applying sliding window...')
        combined_res_tok_shf_df[
            f'token_int_seq_round_d{rounding_digits}_sw'] = combined_res_tok_shf_df[
                f'token_int_seq_round_d{rounding_digits}'].progress_apply(
                    lambda x: sliding_window(x,
                                             min_block_size=min_block_size,
                                             max_block_size=max_block_size))

    train_data = combined_res_tok_shf_df[combined_res_tok_shf_df['label'] ==
                                         'train']
    val_data = combined_res_tok_shf_df[combined_res_tok_shf_df['label'] ==
                                       'val']
    test_data = combined_res_tok_shf_df[combined_res_tok_shf_df['label'] ==
                                        'test']

    print(
        f"\tNumber of training samples: {len(train_data)}, val samples: {len(val_data)}, test samples: {len(test_data)}"
    )
    ...
    if apply_sliding_window:
        ...
        for cur_graph_id, l in zip(
            ...
            if cur_graph_id in emb_graph_id_to_idx_dict:
                train_data_conc += l
                train_data_graph_idx_list += [
                    emb_graph_id_to_idx_dict[cur_graph_id]
                ] * len(l)
    ...
    if apply_sliding_window:
        ...
        np.save(save_path.joinpath('train.npy'), train_data_conc_np)
        np.save(save_path.joinpath('val.npy'), val_data_conc_np)
        np.save(save_path.joinpath('test.npy'), test_data_conc_np)

    combined_res_df.to_pickle(save_path.joinpath('combined_res_df.pkl'))
    ...
    val_data_sampled = (val_data[(
        ~val_data[f'token_seq_round_d{rounding_digits}'].isna())].groupby(
            'edgelist_list_len', group_keys=False).filter(lambda group: len(
                group) > sample_size_per_w_bucket).reset_index(drop=True))
    ...
    np.save(save_path.joinpath(f'feather_emb_d{emb_size}.npy'), feather_par_emb)
    ...
    pd.to_pickle(meta, save_path.joinpath('meta.pkl'))
