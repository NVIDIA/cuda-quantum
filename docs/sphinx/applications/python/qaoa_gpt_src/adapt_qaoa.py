...
def adapt_qaoa_run(hamiltonian, qubits_num, pool = 'all_pool', gamma_0 = 0.01, norm_threshold = 1e-3, 
                   energy_threshold = 1e-5, approx_ratio = 1.0, true_energy = 0.0, optimizer = 'BFGS', 
                   parameter_shift = False, max_iter = 10, verbose = False):
    ...
    @cudaq.kernel
    def grad(state:cudaq.State, ham_words:list[cudaq.pauli_word], ham_coeffs: list[complex], gamma_0:float):
    ...
    for iter in range(max_iter):
        ...
        for op in com_op:
            op = op * -1j
            gradient_vec.append(cudaq.observe(grad, op , state, ham_words, ham_coeffs, gamma_0).expectation())

        # Compute the norm of the gradient vector
        norm = np.linalg.norm(np.array(gradient_vec))
        if verbose: print('Norm of the gradient: ', norm)
        
        if norm <= norm_threshold:
            if verbose: print('\n', 'Final Result: ', '\n')
            if verbose: print('Norm of the gradient is below the threshold', norm)
            if verbose: print('Final mixer_pool: ', mixer_pool_str)
            if verbose: print('Number of layers: ', len(layer))
            if verbose: print('Number of mixer pool in each layer: ', layer)
            if verbose: print('Final Energy: ', E_current)
            if verbose: print('Ratio of the energy: ', ratio)
            ...
            if verbose: print('Total number of pool with max gradient: ', tot_pool)
            ...
            if verbose: print('Mixer pool at step', istep)
            if verbose: print(mixer_pool_str)
            
            num_layer = len(layer)
            if verbose: print('Number of layers: ', num_layer)
            ...
            def cost(theta):
                ...
                energy = cudaq.observe(kernel_qaoa, hamiltonian, qubits_num, ham_words, ham_coeffs,
            ...
            if optimizer == 'COBYLA':
                result_vqe = minimize(cost, theta, method='COBYLA', options={'rhobeg': 1.0, 'maxiter'
                        : 10000, 'disp': False, 'tol': 1e-6})    
                E_current = result_vqe.fun
                theta = result_vqe.x.tolist()
                if verbose: print('Optmized Energy: ', result_vqe.fun, flush=True)
                if verbose: print('Optimizer exited successfully: ',result_vqe.success, flush=True)
                
            elif optimizer == 'BFGS':
                if parameter_shift:
                    result_vqe = minimize(cost, theta, method='BFGS', jac=parameter_shift, tol= 1e-5)
                    E_current = result_vqe.fun
                    theta = result_vqe.x.tolist()
                    if verbose: print('Optmized Energy: ', result_vqe.fun, flush=True)
                    if verbose: print('Optimizer exited successfully: ',result_vqe.success, flush=True)
                else:
                    result_vqe = minimize(cost, theta, method='BFGS', jac='2-point', options={'gtol': 1e-4})
                    E_current = result_vqe.fun
                    theta = result_vqe.x.tolist()
                    if verbose: print('Optmized Energy: ', result_vqe.fun, flush=True)
                    if verbose: print('Optimizer exited successfully: ',result_vqe.success, flush=True)
                ...
                if parameter_shift:
                    result_vqe = minimize(cost, theta, method='L-BFGS-B', jac=parameter_shift, tol=1e-5)
                    E_current = result_vqe.fun
                    theta = result_vqe.x.tolist()
                    if verbose: print('Optmized Energy: ', result_vqe.fun, flush=True)
                    if verbose: print('Optimizer exited successfully: ',result_vqe.success, flush=True)
                else:
                    result_vqe = minimize(cost, theta, method='L-BFGS-B', jac='2-point', tol=1e-5)
                    E_current = result_vqe.fun
                    theta = result_vqe.x.tolist()
                    if verbose: print('Optmized Energy: ', result_vqe.fun, flush=True)
                    if verbose: print('Optimizer exited successfully: ',result_vqe.success, flush=True)
            ...
            if verbose: print('Result from the step ', istep)
            if verbose: print('Optmized Energy: ', result_vqe.fun)
            
            dE = np.abs(E_current - E_prev)
            E_prev = E_current
            
            if verbose: print('dE= :', dE)
            ...
            if dE <= energy_threshold:
                if verbose: print('\n', 'Final Result: ', '\n')
                if verbose: print('dE below the threshold is satisfied: ', dE)
                if verbose: print('Final mixer_pool: ', mixer_pool_str)
                if verbose: print('Number of layers: ', len(layer))
                if verbose: print('Number of mixer pool in each layer: ', layer)
                if verbose: print('Final Energy= ', E_current)
                ...
                if verbose: print('\n', 'Final Result: ', '\n')
                if verbose: print('Approximation ratio is satisfied', ratio)
                if verbose: print('Final mixer_pool: ', mixer_pool_str)
                if verbose: print('Number of layers: ', len(layer))
                if verbose: print('Number of mixer pool in each layer: ', layer)
                if verbose: print('Final Energy= ', E_current)
                ...
                state = cudaq.get_state(kernel_qaoa, qubits_num, ham_words, ham_coeffs, mixer_pool, gamma, beta, num_layer)
                if verbose: print('State at step ', istep)
                #print(state)
                istep += 1
                if verbose: print('\n')
    ...
    if iter == max_iter - 1:
        if verbose: print('\n', 'Final Result: ', '\n')
        if verbose: print('Maximum number of iterations reached without satisfying the convergence criteria.')
        if verbose: print('Final mixer_pool: ', mixer_pool_str)
        if verbose: print('Number of layers: ', len(layer))
        if verbose: print('Number of mixer pool in each layer: ', layer)
        if verbose: print('Final Energy= ', E_current)
    ...
    # Sample the circuit
    count=cudaq.sample(kernel_qaoa, qubits_num, ham_words, ham_coeffs, mixer_pool, gamma, beta, num_layer, shots_count=5000)
    if verbose: print('The most probable max cut: ', count.most_probable())
    if verbose: print('All bitstring from circuit sampling: ', count)
    
    return (energy_list, mixer_pool_str, pool_list_index, gamma, beta, ratio, str(count.most_probable()), len(layer), result_vqe.success)