...
def uccsd_circuit_vqe(spin_mult, only_singles, only_doubles,
                      qubits_num, electron_count, optimize, 
                      theta, hamiltonian, method = 'BFGS', verbose = False):
    """
    Generate the UCCSD circuit for VQE.
    
    Parameters:
    - spin_mult: Spin multiplicity of the system.
    ...
    if optimize:
        if method == 'L-BFGS-B':
            ...
            result_vqe = minimize(cost, theta, method='COBYLA', options={'rhobeg': 1.0, 'maxiter'
                        : 20000, 'disp': False, 'tol': 1e-7})
        else:
            raise ValueError("Invalid optimization method. Use 'L-BFGS-B', 'BFGS', or 'COBYLA'.")
    
        total_energy = result_vqe.fun 
        if verbose: print(f"Total energy: {total_energy:.10f} Hartree")
        # Print the optimized parameters: first n are singles, then doubles.
        if verbose: print(f"optimized parameters: {result_vqe.x}")
