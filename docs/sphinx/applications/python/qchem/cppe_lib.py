...
try:
    ...
    print('cppe library should be installed to use the polaizable embedding model. Use pip install cppe')
...
class PolEmbed:
    ...
    def __init__(self,mol,peoptions):
        ...
        self.polarizable_coords = np.array([site.position for site in self.cppe_state.potentials if site.is_polarizable])
    ...
    def _compute_multipole_potential_integrals(self, site, order, moments):
        ...
        if order>2:
            raise NotImplementedError("""Multipole potential integrals not implemented for order > 2.""")
        ...
        if order>0:
            ...
            es_operator += np.einsum('aij,a->ij', integral1, moments[1] * cppe.prefactors(1))
        ...
        if order>1:
            ...
            es_operator += np.einsum('aij,a->ij', integral2[[0, 1, 2, 4, 5, 8], :, :], moments[2] * cppe.prefactors(2))
    ...
    def get_pe_contribution(self, dm, elec_only=False):
        ...
        if self.V_es is None:
            ...
            for p in self.potentials:
                ...
                self.V_es += self._compute_multipole_potential_integrals(p.position, m.k, moments)
        ...
        # Step IV: build induction operator
            ...
            for p in self.potentials:
                if not p.is_polarizable:
                    continue
                site = p.position
                V_ind += self._compute_field_integrals(site=site, moment=induced_moments[3*current_polsite:3*current_polsite + 3])
                current_polsite += 1
