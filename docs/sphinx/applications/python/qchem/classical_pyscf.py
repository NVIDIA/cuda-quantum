        ...
        for q in range(nqubits//2):
            ...
            for r in range(nqubits//2):
                for s in range(nqubits//2):
                    ...
                    temp=str(0.5 * h2e[p, q, r, s]) +' a_' + str(p) + '^dagger ' + 'a_' + str(q) + '^dagger ' + 'a_' + str(r) + ' a_' + str(s)
                    ferm_ham.append(temp)
                    two_body_coeff[2 * p + 1, 2 * q + 1, 2 * r + 1, 2 * s + 1] = 0.5 * h2e[p, q, r, s]
                    temp=str(0.5 * h2e[p, q, r, s]) +' b_' + str(p) + '^dagger ' + 'b_' + str(q) + '^dagger ' + 'b_' + str(r) + ' b_' + str(s)
                    ferm_ham.append(temp)

                    # Mixed spin(abab, baba) <a|a><b|b>, <b|b><a|a>
                    #<a|b>= 0 (orthogoanl)
                    two_body_coeff[2 * p, 2 * q + 1, 2 * r + 1, 2 * s] = 0.5 * h2e[p, q, r, s]
                    temp=str(0.5 * h2e[p, q, r, s]) +' a_' + str(p) + '^dagger ' + 'a_' + str(q) + '^dagger ' + 'b_' + str(r) + ' b_' + str(s)
                    ferm_ham.append(temp)
                    two_body_coeff[2 * p + 1, 2 * q, 2 * r, 2 * s + 1] = 0.5 * h2e[p, q, r, s]
                    temp=str(0.5 * h2e[p, q, r, s]) +' b_' + str(p) + '^dagger ' + 'b_' + str(q) + '^dagger ' + 'a_' + str(r) + ' a_' + str(s)
                    ferm_ham.append(temp)
...
def get_mol_hamiltonian(xyz:str, spin:int, charge: int, basis:str, symmetry:bool = False, memory:float = 4000, cycles:int = 100, \
    ...
    if viz_orb: molden.from_mo(mol, filename+'_HF_molorb.molden', myhf.mo_coeff)
    ...
    if MP2:
        ...
        if spin != 0:
            ...
            if integrals_natorb or natorb:
                ...
                if viz_orb: molden.from_mo(mol, filename+'_MP2_natorb.molden', natorbs)
    ...
    if casscf:
        ...
        if natorb and (spin == 0):
            ...
            mycas = mcscf.CASSCF(myhf, norb_cas, nele_cas)
            mycas.max_cycle_macro = cycles  # Fixed the typo from "ycles" to "cycles"
            mycas.kernel()
    ...
    if casci and casscf:
        ...
        if natorb and (spin != 0):
            ...
            e_fci, fcivec = fci.direct_spin1.kernel(h1e_cas, h2e_cas, norb_cas, nele_cas, ecore=ecore)
