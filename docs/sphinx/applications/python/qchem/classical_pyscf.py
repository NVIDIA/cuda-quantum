...
def get_mol_pe_hamiltonian(xyz:str, potfile:str, spin:int, charge: int, basis:str, symmetry:bool=False, memory:float=4000, cycles:int=100,\
    ...
    if spin != 0:
        print('WARN: UHF is not implemented yet for PE model. RHF & ROHF are only supported.')
    ...
    if casci:
        ...
        if nele_cas is None:
            ...
            if natorb and (spin == 0):
                ...
                mycasci.run(natorbs)
                if verbose: print('[pyscf] CASCI energy (using natural orbitals) with solvent= ', mycasci.e_tot)

            else:
                mycasci = mcscf.CASCI(mf_pe, norb_cas, nele_cas)
                mycasci = solvent.PE(mycasci,potfile)
                mycasci.run()
                if verbose: print('[pyscf] CASCI energy (using molecular orbitals) with solvent= ', mycasci.e_tot)
                if verbose: print('[Pyscf] Polarizable embedding energy from CASCI: ', mycasci.with_solvent.e)
    ...
    if casscf:
        if natorb and (spin == 0):
            ...
            mycas.max_cycle_macro = cycles
            mycas.kernel(natorbs)
            if verbose: print('[pyscf] CASSCF energy (using natural orbitals) with solvent= ', mycas.e_tot)
            ...
            mycas.max_cycle_macro = cycles
            mycas.kernel()
            if verbose: print('[pyscf] CASSCF energy (using molecular orbitals) with solvent= ', mycas.e_tot)
    ...
    if nele_cas is None:
        ...
        # Compute the molecular spin electronic Hamiltonian from the 
        # molecular electron integrals 
        obi, tbi, e_nn, ferm_ham = generate_molecular_spin_ham_restricted(h1e,h2e,nuclear_repulsion)
        ...
        if integrals_natorb:        
            ...
            obi, tbi, core_energy, ferm_ham = generate_molecular_spin_ham_restricted(h1e_cas, h2e_cas, ecore)
            ...
            if casci:
                ...
                V_pe_cas = V_pe_mo[mycasci.ncore:mycasci.ncore+mycasci.ncas,mycasci.ncore:mycasci.ncore+mycasci.ncas]
            ...
            if casscf:
                ...
                raise ValueError("WARN: You need to run casscf. Use casscf=True.")
            obi, tbi, core_energy, ferm_ham = generate_molecular_spin_ham_restricted(h1e_cas, h2e_cas, ecore)
            ...
            V_pe_cas = V_pe_mo[mycas.ncore:mycas.ncore + mycas.ncas, mycas.ncore:mycas.ncore + mycas.ncas]
            obi_pe = generate_pe_spin_ham_restricted(V_pe_cas)
            #obi_pe.astype(complex).tofile(f'{filename}_pe_one_body.dat')
