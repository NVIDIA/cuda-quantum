...
def get_parameters(xyz:str, spin:int, charge: int, basis:str, symmetry:bool=False, memory:float=4000,cycles:int=100, \
    ...
    if verbose: print('[pyscf] Computing initial guess parameters using the classical CCSD')
        ...
        if ccsd:
            ...
            if UR:
                ...
                if nele_cas is None:
                    ...
                    qubits_num = 2 * norb
                    init_params = uccsd_get_amplitude(mycc.t1, mycc.t2, nelec, qubits_num, UR)
                    ...
                    frozen += [y for y in range(mc.ncore+norb_cas, len(myhf.mo_coeff))]
                    ...
                    init_params = uccsd_get_amplitude(mycc.t1, mycc.t2, nele_cas, qubits_num, UR)
                    #np.array(init_params).tofile(f'{filename}_params.dat')
                    return init_params
    ...
    # With solvent
        ...
        if verbose: print('[Pyscf] Total number of orbitals = ', norb)
        if verbose: print('[Pyscf] Total HF energy with solvent:', mf_pe.e_tot)
        if verbose: print('[Pyscf] Polarizable embedding energy from HF: ', mf_pe.with_solvent.e)
        ...
        if MP2:
            if spin != 0:
                ...
                mymp.run()
                if verbose: print('[pyscf] R-MP2 energy with solvent= ', mymp.e_tot)
                if verbose: print('[Pyscf] Polarizable embedding energy from MP: ', mymp.with_solvent.e)

                if natorb:
                    # Compute natural orbitals
                    noons, natorbs = mcscf.addons.make_natural_orbitals(mymp)
                    if verbose: print('[Pyscf] Natural orbital occupation number from R-MP2: ')
                    if verbose: print(noons)
        ...
        if ccsd:
            if nele_cas is None:
                ...
                mycc.run()
                if verbose: print('[Pyscf] Total CCSD energy with solvent: ', mycc.e_tot)
                if verbose: print('[Pyscf] Polarizable embedding energy from CCSD: ', mycc.with_solvent.e)
                qubits_num = 2 * norb
                init_params = uccsd_get_amplitude(mycc.t1, mycc.t2, nelec, qubits_num, UR)
                #np.array(init_params).tofile(f'{filename}_params.dat')
                return init_params
                ...
                frozen = []
                frozen += [y for y in range(0,mc.ncore)]
                frozen += [y for y in range(mc.ncore+norb_cas, len(mf_pe.mo_coeff))]

                if natorb and (spin == 0):
                    mycc = cc.CCSD(mf_pe,frozen=frozen, mo_coeff=natorbs)
                    mycc = solvent.PE(mycc,potfile)
                    mycc.run()
                    if verbose: print('[pyscf] R-CCSD energy of the active space (using natural orbitals) with solvent= ', mycc.e_tot)
                else:
                    mycc = cc.CCSD(mf_pe, frozen = frozen)
                    mycc = solvent.PE(mycc, potfile)
                    mycc.run()
                    if verbose: print('[pyscf] CCSD energy of the active space (using molecular orbitals) with solvent= ', mycc.e_tot)
                    if verbose: print('[Pyscf] Polarizable embedding energy from CCSD: ', mycc.with_solvent.e)
