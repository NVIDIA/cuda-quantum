...
def uccsd_get_excitation_list(n_electrons:int, n_qubits:int):
    ...
    if n_qubits%2!=0:
        raise ValueError("Total number of spin molecular orbitals (number of qubits) should be even.")
    else:
...
def add_single_excitation(op, p_occ, q_virt):
    ...
    op.append(spin.y(p_occ) * parity * spin.x(q_virt) - spin.x(p_occ) * parity * spin.y(q_virt))
...
def add_double_excitation(op, p_occ, q_occ, r_virt, s_virt):
    ...
    temp_op=spin.x(i_occ) * parity_a * spin.x(j_occ) * spin.x(a_virt) * parity_b * spin.y(b_virt)
    temp_op+=spin.x(i_occ) * parity_a * spin.x(j_occ) * spin.y(a_virt) * parity_b * spin.x(b_virt)
    temp_op+=spin.x(i_occ) * parity_a * spin.y(j_occ) * spin.y(a_virt) * parity_b * spin.y(b_virt)
    temp_op+=spin.y(i_occ) * parity_a * spin.x(j_occ) * spin.y(a_virt) * parity_b * spin.y(b_virt)
    temp_op-=spin.x(i_occ) * parity_a * spin.y(j_occ) * spin.x(a_virt) * parity_b * spin.x(b_virt)
    temp_op-=spin.y(i_occ) * parity_a * spin.x(j_occ) * spin.x(a_virt) * parity_b * spin.x(b_virt)
    temp_op-=spin.y(i_occ) * parity_a * spin.y(j_occ) * spin.x(a_virt) * parity_b * spin.y(b_virt)
    temp_op-=spin.y(i_occ) * parity_a * spin.y(j_occ) * spin.y(a_virt) * parity_b * spin.x(b_virt)
...
def get_uccsd_pool(nelectrons, n_qubits):
    ...
    for i in range(n_mixed_doubles):
        ...
        add_double_excitation(pool_op, p_alpha_occ,q_beta_occ, r_beta_virt,s_alpha_virt)

    for i in range(n_alpha_doubles):
        p_alpha_occ,q_alpha_occ,r_alpha_virt,s_alpha_virt=doubles_alpha[i]
        add_double_excitation(pool_op,p_alpha_occ,q_alpha_occ, r_alpha_virt,s_alpha_virt)

    for i in range(n_beta_doubles):
        p_beta_occ,q_beta_occ,r_beta_virt,s_beta_virt=doubles_beta[i]

        add_double_excitation(pool_op,p_beta_occ,q_beta_occ,r_beta_virt,s_beta_virt)
