...
def transform_state_with_pauli_operator(computational_state, pauli_operator_string):
    """
    ...
    for qubit_position, pauli_op in enumerate(pauli_operator_string):
        if pauli_op == 'I':
            ...
            transformed_state[qubit_position] = 1 - transformed_state[qubit_position]
        elif pauli_op == 'Y':
...
def construct_hamiltonian_in_subspace(pauli_operator_list, hamiltonian_coefficients, subspace_basis_states):
    """
    ...
    for initial_state_idx, initial_state in enumerate(subspace_basis_states):
        ...
        for pauli_operator, coefficient in zip(pauli_operator_list, hamiltonian_coefficients):
            # Compute P_k|i⟩ = phase × |j⟩
            final_state, phase_factor = transform_state_with_pauli_operator(initial_state, pauli_operator)
    ...
    return csr_matrix((matrix_elements, (matrix_rows, matrix_cols)), shape=(subspace_dimension, subspace_dimension))

def diagonalize_subspace_hamiltonian(subspace_basis_states, pauli_operator_list, hamiltonian_coefficients, verbose=False, **solver_options):
    """
    ...
    projected_hamiltonian = construct_hamiltonian_in_subspace(pauli_operator_list, hamiltonian_coefficients, subspace_basis_states)
    
    if verbose:
        print(f"Subspace dimension: {projected_hamiltonian.shape[0]}")
        print(f"Hamiltonian sparsity: {projected_hamiltonian.nnz / (projected_hamiltonian.shape[0]**2):.4f}")
    ...
    try:
        ...
        eigenvalues = eigsh(projected_hamiltonian, return_eigenvectors=False, **solver_options)
        return eigenvalues
...
def accumulate_krylov_measurements(measurement_results_sequence, krylov_dimension):
    """
    ...
    for evolution_step in range(krylov_dimension):
        ...
        for measurement_data in measurement_results_sequence[: evolution_step + 1]:
...
def construct_xyz_spin_hamiltonian(system_size: int, interaction_strengths: tuple[float, float, float] = (1.0, 1.0, 1.0), 
    ...
    for site_i in range(system_size):
        site_j = (site_i + 1) % system_size  # Nearest neighbor with periodic wrapping
...
def create_heisenberg_hamiltonian(n_spins: int, Jx: float, Jy: float, Jz: float, h_x: list[float], h_y: list[float], h_z: list[float]):
...
def extract_hamiltonian_data(spin_operator: cudaq.SpinOperator):
    ...
    for pauli_term in spin_operator:
        ...
        assert abs(term_coefficient.imag) < 1e-10, f"Non-real coefficient encountered: {term_coefficient}"
...
def extract_basis_states_from_measurements(measurement_counts):
    ...
    basis_state_matrix = np.array([[int(bit) for bit in bitstring] 
                                   for bitstring in observed_bitstrings], dtype=np.int8)
