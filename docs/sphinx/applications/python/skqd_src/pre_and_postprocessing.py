...
def vectorized_projected_hamiltonian(basis_states, hamiltonian_pauli_words, hamiltonian_coefficients_numpy, use_gpu):
    """
    ...
    coefficients_xp = xp.asarray(hamiltonian_coefficients_numpy, dtype=xp.complex128)
    
    # Convert Pauli strings to numeric array: I=0, X=1, Y=2, Z=3
    pauli_to_int = {'I': 0, 'X': 1, 'Y': 2, 'Z': 3}
    pauli_ops_np = np.array(
        [[pauli_to_int[p] for p in pauli_word] for pauli_word in hamiltonian_pauli_words],
        dtype=np.int8
    )
    ...
    transformed_states = xp.where(flip_mask, 1 - states_expanded, states_expanded)
    ...
    n_y0 = xp.sum(y_mask & (states_expanded == 0), axis=2, dtype=xp.int32)  # (n_basis, n_terms)
    ...
    phase_lookup = xp.array([1.0+0j, 0.0+1j, -1.0+0j, 0.0-1j], dtype=xp.complex128)
    ...
    powers_of_2 = xp.asarray(1 << np.arange(n_qubits - 1, -1, -1, dtype=np.int64))
    
    basis_ints = xp.sum(basis_states_xp.astype(xp.int64) * powers_of_2, axis=1)  # (n_basis,)
    transformed_ints = xp.sum(transformed_states.astype(xp.int64) * powers_of_2, axis=2)  # (n_basis, n_terms)
    ...
    actually_found = in_bounds & (sorted_basis_ints[search_positions_clipped] == transformed_flat)
...
def projected_hamiltonian(basis_states, hamiltonian_pauli_words, hamiltonian_coefficients_numpy, verbose):
    
    matrix_rows, matrix_cols, matrix_elements = [], [], []


    for i in range(basis_states.shape[0]):  #loop over all basis states in subspace 
        
        for pauli_operator, coefficient in zip(hamiltonian_pauli_words, hamiltonian_coefficients_numpy): #loop over each term in the Hamiltonian
            ...
            if verbose: print('initial_state            ', initial_state)
            if verbose: print('pauli_operator, coeff     ', pauli_operator, coefficient)
            
            for qubit_position, pauli_op in enumerate(pauli_operator): #loop over each qubit in each term of the Hamiltonian
            ...
            if verbose: print('final_state, phase_factor', transformed_state, phase_factor)
            if verbose: print(' ')
