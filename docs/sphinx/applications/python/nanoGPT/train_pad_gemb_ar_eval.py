...
train_data = np.load(os.path.join(data_dir, 'train.npy'), mmap_mode=mmap)
val_data = np.load(os.path.join(data_dir, 'val.npy'), mmap_mode=mmap)
...
def get_batch(split):
    ...
    if device_type == 'cuda':
        ...
        graph_emb_data = graph_emb_data.pin_memory().to(device,
...
def eval_model_ar():
    ...
    test_energies_expl_df = test_energies_df[[
        'adapt_gpt_energies', 'true_energy'
    ]].explode('adapt_gpt_energies')

    test_energies_expl_corr_df = test_energies_expl_df[
        test_energies_expl_df['adapt_gpt_energies'] != 999]

    test_energies_expl_corr_df['ar'] = test_energies_expl_corr_df[
        'adapt_gpt_energies'] / test_energies_expl_corr_df['true_energy']
    ...
    wrong_circ_rate = round(
        len(test_energies_expl_inc_df) / len(test_energies_expl_df), 5)
