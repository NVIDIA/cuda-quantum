...
involving multiple architectures working in tandem. This page will walk through the simple steps to accelerate any quantum circuit simulation with a GPU and how to scale large simulations uing multi-GPU multi-node capabilities.
...
As the number of qubits increaces to even modest size, the CPU simulation will become impractically slow.  By swiiching to the :code:`nvidia` backend, you can accelerate the same code on a single GPU and achieve a speedup of up to **2500x**.  If you have a GPU availible, this the default backend to ensure maximum productivity.
...
As :code:`N` gets larger, the size of the state vector that needs to be stored in memory increaces exponentially. The state vector has :math:`2^N` elements, each a complex number requiring 8 bytes. This means a 30 qubit simulation requires roughly 8 GB. Adding a few more qubits will quickly exceed the memory of as single GPU.  The MGPU backend solved this problem by pooling the memeory of multiple GPUs across multiple nodes to perform a single state vector simulation. 
...
This code will execute in an MPI context and provide additional memeory to simulation much larger state vectors.
...
As the results are run asynchronously, they need to be stored in a list (:code:`asyncresults`) and retrieved later with the :code:`get` command. The following loops over the parameter batches, and the sets of parameters in each batch. Theparameter sets are provided as inputs to :code:`observe_async` along with specification of a :code:`qpu_id` which designates the GPU (of the four availible) which will run computation. A speedup of up to 4x can be expected with results varying by problem size.
