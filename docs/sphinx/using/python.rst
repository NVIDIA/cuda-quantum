...
To enable running code interactively, 
launch the CUDA Quantum container with

.. code-block:: console

   docker run -it --gpus all -p 8888:8888 --name cudaq-jupyter ghcr.io/nvidia/cuda-quantum:preview
   jupyter-lab --no-browser --ip=* --port=8888 \
      --ServerApp.allow_origin='*' --ServerApp.tornado_settings='{"headers":{"Content-Security-Policy":"frame-ancestors self *"},"cookie_options":{"SameSite":"None","Secure":True}}' \
      --IdentityProvider.cookie_options='{"SameSite":"None","Secure":True}' --IdentityProvider.token="my-custom-token"
