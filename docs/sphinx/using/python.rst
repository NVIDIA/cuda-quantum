...
---------------------------------------------------
This example showcases the prototype of the quantum-enhanced Markov chain Monte Carlo (QMCMC) algorithm. The process involves generating a random bitstring, loading it as a quantum state, and evolving the circuit by applying the first-order Trotter decomposition of both the mixer and the problem Hamiltonians. Subsequently, an acceptance probability is generated based on the energy of the initial state as well as the energy of the most probable bitstring obtained from executing this circuit. This acceptance probability determines whether the observed bitstring is accepted as the new state for the next iteration or not. The algorithm continues to run until the specified number of iterations is achieved.
