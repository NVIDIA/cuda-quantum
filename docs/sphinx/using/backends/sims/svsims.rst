...
.. list-table:: **Additional environment variable options for multi-node multi-GPU mode**
  ...
  * - ``CUDAQ_GLOBAL_INDEX_BITS``
    ...
    - Specify the inter-node network structure (faster to slower). For example, assuming a 32-node (MPI processes) simulation, whereby the network topology is divided into 4 groups of 8 nodes, which have faster communication network between them. In this case, the `CUDAQ_GLOBAL_INDEX_BITS` environment variable can be set to `3,2`. The first `3` (`log2(8)`) represents **8** nodes with fast communication within the group and the second `2` represents the **4** groups (8 nodes each) in those total 32 nodes. The sum of all elements in this list is `5`, corresponding to the total number of nodes/MPI processes (`2^5 = 32`). Default is an empty list (no customization based on network structure of the cluster).
