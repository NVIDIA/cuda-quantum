...
To execute your kernels, pass the ``--anyon-machine`` flag to the ``nvq++`` compiler
to specify which machine to submit quantum kernels to:
...
.. code:: bash
    ...
    nvq++ --emulate --target anyon src.cpp
...
To execute your kernels, specify which machine to submit quantum kernels to
by setting the :code:`machine` parameter of the target. 
If :code:`machine` is not specified, the default machine type will be ``telegraph-8q``.
