...
The tokens file cannot be generated by the ``iqmclient`` tool anymore but can be created manually using the token obtained from the Resonance profile page.
...
.. tab:: Python
        ...
        This mode will still contact the configured IQM Server to retrieve
        the dynamic quantum architecture resulting from the active calibration
        unless a QPU architecture file is specified.
...
.. tab:: C++
        ...
        you can pass the ``--emulate`` flag to ``nvq++``. This will emit any
        target specific compiler diagnostics, before running a noise free emulation.

        .. code:: bash

            nvq++ --target --emulate iqm src.cpp -o program
            IQM_SERVER_URL="https://demo.qc.iqm.fi/" ./program

        Emulation mode will still contact the configured IQM Server to retrieve
        the dynamic quantum architecture resulting from the active calibration
        unless an architecture is specified. This can be done by specifying a file
        with the architecture either at compile time or in an variable in the
        environment executing the binary. If an architecture is specified no
        ...
        .. code:: bash
            ...
            nvq++ --target --emulate iqm src.cpp -o program
            IQM_QPU_QA="<path+filename of architecture file>" ./program

        .. code:: bash

            nvq++ --target --emulate --mapping-file <path+filename of architecture file> iqm src.cpp -o program
            ./program
