...
-   [Applications](../../using/applications.html){.reference .internal}
    ...
    -   [Readout Error Mitigation](#){.current .reference .internal}
        -   [Inverse confusion matrix from single-qubit noise
            model](#Inverse-confusion-matrix-from-single-qubit-noise-model){.reference
            .internal}
        -   [Inverse confusion matrix from k local confusion
            matrices](#Inverse-confusion-matrix-from-k-local-confusion-matrices){.reference
            .internal}
        -   [Inverse of full confusion
            matrix](#Inverse-of-full-confusion-matrix){.reference
            .internal}
...
Previous](quantum_volume.html "Quantum Volume"){.btn .btn-neutral
.float-left} [Next []{.fa .fa-arrow-circle-right
aria-hidden="true"}](unitary_compilation_diffusion_models.html "Compiling Unitaries Using Diffusion Models"){.btn
...
::: {#Readout-Error-Mitigation .section}
Readout Error Mitigation[¶](#Readout-Error-Mitigation "Permalink to this heading"){.headerlink}
===============================================================================================

Readout errors are caused by imperfect qubit measurement and are a
common source of error in quantum computing. Properly modelling these
errors in simulation can give the user tools to understand better what
these errors are and how to mitigate them when running on actual quantum
devices.

Readout errors can be mitigated with a confusion matrix [\\(A\\)]{.math
.notranslate .nohighlight}. It is a square matrix of size [\\(2\^n
\\times 2\^n\\)]{.math .notranslate .nohighlight} and tells about the
probability of observing the state [\\(\|y\\rangle\\)]{.math
.notranslate .nohighlight} given the true state
[\\(\|x\\rangle\\)]{.math .notranslate .nohighlight}. The confusion
matrix characterizes the readout error of the device and is
circuit-independent. Once [\\(A\\)]{.math .notranslate .nohighlight} is
estimated, we can compute its pseudoinverse [\\(A\^+\\)]{.math
.notranslate .nohighlight} which can be applied to the noisy probability
distribution [\\(p\_{\\text{noisy}}\\)]{.math .notranslate .nohighlight}
to obtain an adjusted quasi-probability distribution

::: {.math .notranslate .nohighlight}
\\\[p\_{\\text{mitigated}} = A\^+ p\_{\\text{noisy}}\\\]
:::

In this tutorial, we show how to build a confusion matrix with the
following approaches.

-   Using a single qubit model

-   Using [\\(k\\)]{.math .notranslate .nohighlight} local confusion
    matrices

-   A full confusion matrix for each [\\(2\^n\\)]{.math .notranslate
    .nohighlight} combination

The last method works well for correcting correlated errors (which
affect multiple qubits together). However, it becomes impractical for
...
To model the readout error, we apply a bitflip channel on every qubit at
the end of the circuit using an Identity gate. The probability of
bitflip `probs`{.docutils .literal .notranslate} can be the same or
different for all the qubits.

::: {.alert .alert-block .alert-success}
Note: In principle, readout error is applied to the measurement gate but
we use the Identity gate because adding a quantum error on the
measurement gate is not yet supported. Also, the Identity gate is not
present in the add\_channel method, so we model the Identity gate using
a rotation gate with an angle of 0.0.
:::
...
We define a cudaq kernel which will help create a set of quantum
circuits to take measurements of the basis states for [\\(n\\)]{.math
.notranslate .nohighlight} qubits. The kernel takes the number of qubits
and the basis state as arguments.
...
For the tutorial, we will use the GHZ state on three qubits to test
readout error mitigation. The GHZ state on 3 qubits is described as:
...
It is possible that the adjusted quasi-probability distribution
[\\(p\_{\\text{mitigated}}\\)]{.math .notranslate .nohighlight} obtained
by application of [\\(A\^+\\)]{.math .notranslate .nohighlight} to
[\\(p\_{\\text{noisy}}\\)]{.math .notranslate .nohighlight} may be
non-positive. We need to find the closest positive probability
distribution in such a case.

::: {.math .notranslate .nohighlight}
\\\[p\'\' = \\min\_{p\_{\\rm positive}} \\\|p\_{\\rm noisy} - p\_{\\rm
positive}\\\|\_1\\\]
:::

Then, we can draw samples from [\\(p\'\'\\)]{.math .notranslate
.nohighlight}.
...
Let's run the `ghz_kernel`{.docutils .literal .notranslate} with the
noise model. We will perform two experiments. First, we keep the bitflip
probability the same for all the qubits. In the second experiment, we
use different bitflip probabilities for qubits.
...
::: {#Inverse-confusion-matrix-from-single-qubit-noise-model .section}
Inverse confusion matrix from single-qubit noise model[¶](#Inverse-confusion-matrix-from-single-qubit-noise-model "Permalink to this heading"){.headerlink}
-----------------------------------------------------------------------------------------------------------------------------------------------------------

Here we assume that the readout error of each qubit is independent with
the same confusion probabilities. Then, the confusion matrix
[\\(A\_1\\)]{.math .notranslate .nohighlight} for a single qubit can be
defined as

::: {.math .notranslate .nohighlight}
\\\[\\begin{split}A\_1 = \\begin{bmatrix} P(0\|0) & P(0\|1)\\\\ P(1\|0)
& P(1\|1) \\end{bmatrix}\\end{split}\\\]
:::

where [\\(P(y\|x)\\)]{.math .notranslate .nohighlight} is the
probability of observing state [\\(\|y\\rangle\\)]{.math .notranslate
.nohighlight} when measuring the true state [\\(\|x\\rangle\\)]{.math
.notranslate .nohighlight}. Using [\\(A\_1\\)]{.math .notranslate
.nohighlight}, the full [\\(2\^n \\times 2\^n\\)]{.math .notranslate
.nohighlight} confusion matrix [\\(A\\)]{.math .notranslate
.nohighlight} can be written as

::: {.math .notranslate .nohighlight}
\\\[A = A\_1 \\otimes \\dots \\otimes A\_1\\\]
:::
...
::: {.output_area .docutils .container}
![](../../_images/applications_python_readout_error_mitigation_21_0.png)
:::
:::
:::

::: {#Inverse-confusion-matrix-from-k-local-confusion-matrices .section}
Inverse confusion matrix from k local confusion matrices[¶](#Inverse-confusion-matrix-from-k-local-confusion-matrices "Permalink to this heading"){.headerlink}
---------------------------------------------------------------------------------------------------------------------------------------------------------------

This method works under the assumption that the readout errors for
different qubits are independent of each other and the confusion matrix
[\\(A\\)]{.math .notranslate .nohighlight} can be factorized into the
product of [\\(k\\)]{.math .notranslate .nohighlight} small local
confusion matrices. We consider the special case when [\\(k =
n\\)]{.math .notranslate .nohighlight} for [\\(n\\)]{.math .notranslate
.nohighlight} qubits and different [\\(2 \\times 2\\)]{.math
.notranslate .nohighlight} confusion matrices for each qubit

::: {.math .notranslate .nohighlight}
\\\[A = A\_1 \\otimes \\dots \\otimes A\_n\\\]
:::
...
The local confusion matrices are generated using the marginal
...
::: {.output_area .docutils .container}
![](../../_images/applications_python_readout_error_mitigation_27_1.png)
:::
:::
...
::: {.output_area .docutils .container}
![](../../_images/applications_python_readout_error_mitigation_28_0.png)
:::
:::
:::

::: {#Inverse-of-full-confusion-matrix .section}
Inverse of full confusion matrix[¶](#Inverse-of-full-confusion-matrix "Permalink to this heading"){.headerlink}
---------------------------------------------------------------------------------------------------------------

Here we generate a quantum circuit for each of the basis states of
[\\(n\\)]{.math .notranslate .nohighlight} qubits (i.e.,
[\\(2\^n\\)]{.math .notranslate .nohighlight} combinations). This gives
more accurate readout error mitigation. Since it scales exponentially
with the number of qubits, it is only practical for small devices.
...
::: {.output_area .docutils .container}
![](../../_images/applications_python_readout_error_mitigation_32_0.png)
:::
:::
...
::: {.output_area .docutils .container}
![](../../_images/applications_python_readout_error_mitigation_35_0.png)
:::
:::
:::
:::
