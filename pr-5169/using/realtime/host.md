...
-   [Realtime](../realtime.html){.reference .internal}
    ...
    -   [Host API](#){.current .reference .internal}
        -   [What is the
            GpuRoceTransceiver?](#what-is-the-gpurocetransceiver){.reference
            .internal}
        ...
        -   [The 3-Kernel Architecture (GpuRoceTransceiver Example)
            {#three-kernel-architecture}](#the-3-kernel-architecture-gpurocetransceiver-example-three-kernel-architecture){.reference
            .internal}
        ...
        -   [Unified Dispatch Mode](#unified-dispatch-mode){.reference
            ...
            -   [Wiring Example (Unified Mode with
                GpuRoceTransceiver)](#wiring-example-unified-mode-with-gpurocetransceiver){.reference
                .internal}
        ...
        -   [GpuRoceTransceiver 3-Kernel Workflow
            (Primary)](#gpurocetransceiver-3-kernel-workflow-primary){.reference
            .internal}
        -   [NIC-Free Testing (No GpuRoceTransceiver / No
            ConnectX-7)](#nic-free-testing-no-gpurocetransceiver-no-connectx-7){.reference
            .internal}
...
multiple data transport options, including the NVIDIA GpuRoceTransceiver
(RDMA via ConnectX NIC's, from the Holoscan Sensor Bridge project),
[`libibverbs`{.docutils .literal .notranslate}]{.pre}, and proprietary
transport layers. Handlers can execute on GPU (via CUDA kernels) or CPU
(via host threads). Examples in this document use the
GpuRoceTransceiver's 3-kernel workflow (RX kernel/dispatch/TX kernel)
for illustration, but the same principles apply to other transport
mechanisms.

::: {#what-is-the-gpurocetransceiver .section}
## What is the GpuRoceTransceiver?[¶](#what-is-the-gpurocetransceiver "Permalink to this heading"){.headerlink}

**GpuRoceTransceiver** is the GPU RoCE transceiver from NVIDIA's
Holoscan Sensor Bridge (HSB) framework. It enables direct GPU memory
access from external devices (FPGAs, sensors) over Ethernet using RDMA
(Remote Direct Memory Access) via ConnectX NIC's. In the context of
quantum error correction, the GpuRoceTransceiver is one example of a
...
5.  **Transport independence**: The protocol works with the
    GpuRoceTransceiver, [`libibverbs`{.docutils .literal
    .notranslate}]{.pre}, or proprietary transports
...
For example, the [`GpuRoceTransceiver`{.docutils .literal
...
.notranslate}]{.pre}, no [`GpuRoceTransceiver`{.docutils .literal
...
launch function implementation. For example, the
[`GpuRoceTransceiver`{.docutils .literal
.notranslate}]{.pre}/[`DOCA`{.docutils .literal .notranslate}]{.pre}
transport packs [`DOCA`{.docutils .literal .notranslate}]{.pre}
[`QP`{.docutils .literal .notranslate}]{.pre} handles, memory keys, and
ring buffer addresses into a [`doca_transport_ctx`{.docutils .literal
.notranslate}]{.pre} and provides
[`gpu_roce_launch_unified_dispatch`{.docutils .literal
.notranslate}]{.pre} as the launch function (compiled into
[`libcudaq-realtime-bridge-gpu-roce.so`{.docutils .literal
...
-   Transport-agnostic API -- the transport provides a pluggable launch
    function and opaque context (e.g., [`GpuRoceTransceiver`{.docutils
    .literal .notranslate}]{.pre}/[`DOCA`{.docutils .literal
    .notranslate}]{.pre} supplies
    [`gpu_roce_launch_unified_dispatch`{.docutils .literal
    .notranslate}]{.pre})
...
::: {#wiring-example-unified-mode-with-gpurocetransceiver .section}
### Wiring Example (Unified Mode with GpuRoceTransceiver)[¶](#wiring-example-unified-mode-with-gpurocetransceiver "Permalink to this heading"){.headerlink}
...
ring buffers. The transport mechanism (e.g., GpuRoceTransceiver RX/TX
kernels, [`libibverbs`{.docutils .literal .notranslate}]{.pre} threads,
or proprietary transport) places incoming RPC messages into RX slots and
...
-   [`config`{.docutils .literal .notranslate}]{.pre}: filled
    ...
    -   [`vp_id`{.docutils .literal .notranslate}]{.pre} (default 0):
        ...
        and NIC port/IP binding are configured in the
        GpuRoceTransceiver, not in this API.
    ...
    -   [`dispatch_mode`{.docutils .literal .notranslate}]{.pre}
        ...
        -   [`CUDAQ_DISPATCH_HOST_CALL`{.docutils .literal
            .notranslate}]{.pre}: plain C++ host callback
            ([`cudaq_host_rpc_fn_t`{.docutils .literal
            .notranslate}]{.pre}), invoked synchronously on the host
            dispatch path ([`CUDAQ_DISPATCH_PATH_HOST`{.docutils
            .literal .notranslate}]{.pre}); not processed by the device
            path
...
would do when populating RX slots in a GpuRoceTransceiver deployment.
...
Note: this host-side snippet emulates what the external device/FPGA
would do when consuming TX slots in a GpuRoceTransceiver deployment.
...
::: {#gpurocetransceiver-3-kernel-workflow-primary .section}
## GpuRoceTransceiver 3-Kernel Workflow (Primary)[¶](#gpurocetransceiver-3-kernel-workflow-primary "Permalink to this heading"){.headerlink}
...
::: {#nic-free-testing-no-gpurocetransceiver-no-connectx-7 .section}
## NIC-Free Testing (No GpuRoceTransceiver / No ConnectX-7)[¶](#nic-free-testing-no-gpurocetransceiver-no-connectx-7 "Permalink to this heading"){.headerlink}
