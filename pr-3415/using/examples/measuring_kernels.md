...
`mz`{.code .docutils .literal .notranslate}, `mx`{.code .docutils
.literal .notranslate}, and `my`{.code .docutils .literal .notranslate}.
...
.literal .notranslate}. Then, qubit 0 can be reset and used later in the
computation. In this case it is flipped to a 1. Finally, an if statement
performs a Hadamard on qubit 1 if `b0`{.code .docutils .literal
.notranslate} is 1.
