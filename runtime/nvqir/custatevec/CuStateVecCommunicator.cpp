...
public:
  CuStateVecCommunicatorProvider(CommunicatorProvider provider,
                                 const std::string &mpiLibrary)
      : m_type(communicatorType(provider)),
        m_libraryPath(provider == CommunicatorProvider::Cudaq ? ""
                                                              : mpiLibrary) {
    ...
    const custatevecStatus_t status = custatevecExCommunicatorInitialize(
        /*communicatorType=*/m_type,
        /*libraryPath=*/m_libraryPath.empty() ? nullptr : m_libraryPath.c_str(),
        /*argc=*/nullptr, /*argv=*/nullptr,
        /*exCommStatus=*/&communicatorStatus);
  ...
  ~CuStateVecCommunicatorProvider() {
    custatevecExCommunicatorStatus_t communicatorStatus =
        CUSTATEVEC_EX_COMMUNICATOR_STATUS_SUCCESS;
  ...
  bool matches(CommunicatorProvider provider,
               const std::string &mpiLibrary) const {
    const std::string libraryPath =
        provider == CommunicatorProvider::Cudaq ? "" : mpiLibrary;
    return m_type == communicatorType(provider) && m_libraryPath == libraryPath;
  }

private:
  custatevecCommunicatorType_t m_type;
  std::string m_libraryPath;
};

struct SharedProviderState {
  std::mutex mutex;
  std::shared_ptr<CuStateVecCommunicatorProvider> provider;
};

// cuStateVecEx permits multiple communicator descriptors, but its provider is
// initialized once per process. CUDA-Q may create a communicator from each
// thread-local simulator, so share one provider across their descriptors.
static SharedProviderState &sharedProviderState() {
  static SharedProviderState state;
  return state;
}
...
void CuStateVecCommunicator::initializeProvider(const std::string &mpiLibrary) {
  auto &state = sharedProviderState();
  std::scoped_lock lock(state.mutex);
  if (!state.provider)
    state.provider = std::make_shared<CuStateVecCommunicatorProvider>(
        m_providerType, mpiLibrary);
  else if (!state.provider->matches(m_providerType, mpiLibrary))
    throw std::runtime_error(
        "cuStateVec communicator provider is already initialized with a "
        "different configuration.");
  m_provider = state.provider;
}
