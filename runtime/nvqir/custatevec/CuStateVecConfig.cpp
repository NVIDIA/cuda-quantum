...
CuStateVecConfig CuStateVecConfig::fromEnvironment() {
  ...
  if (const auto value = getEnvironment("CUDAQ_BATCH_SIZE")) {
    ...
    if (config.trajectoryBatchSize &&
        !std::has_single_bit(
            static_cast<std::size_t>(*config.trajectoryBatchSize)))
      throw std::invalid_argument(
          "CUDAQ_BATCH_SIZE must be a positive power of two, NONE, or NULL.");
  }
