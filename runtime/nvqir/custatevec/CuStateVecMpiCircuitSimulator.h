  ...
  CuStateVecMpiCircuitSimulator()
      : m_communicator([this] {
        ...
        m_worldRank(m_communicator->provider() == CommunicatorProvider::Cudaq
                        ? cudaq::mpi::rank()
                        : m_communicator->rank()) {
    // Built-in MPI providers issue barriers when destroying a state vector.
    if (m_communicator->provider() != CommunicatorProvider::Cudaq)
      m_config.forceAllocateState = true;
...
protected:
  // Every rank follows the same configured GPU or CPU path, then rank zero
  // provides the sequence used by the distributed state.
  std::vector<double> generateRandomNumbers(std::size_t count) override {
    ...
    auto result = Base::generateRandomNumbers(count);
  ...
  void ensureState() override {
    ...
    if (m_communicatorSize > 1 &&
      ...
      throw std::runtime_error(
          "cuStateVecEx requires a power-of-two communicator size.");
