...
// Copy a borrowed state to host without transferring descriptor ownership.
template <typename Scalar>
void CuStateVecSimulationState<Scalar>::copyStateToHost(
    CuStateVecState<Scalar> &source, std::complex<Scalar> *data,
    std::size_t numElementsRequested) {
  if (!data)
    throw std::invalid_argument("Destination state pointer cannot be null.");
  source.normalizeWireOrdering();

  const std::size_t numElements = std::size_t{1} << source.numWires();
  if (source.distributionType() !=
    ...
    source.getState(data, 0, numElementsRequested);
    source.synchronize();
    return;
  }

  // Gather the current rank's sub-state vectors into one contiguous host
  // vector, ordered by the sub-state indices this rank holds.
  const std::size_t subStateSize = std::size_t{1} << source.numLocalWires();
  const auto indices = source.subStateIndices();
  std::vector<std::complex<Scalar>> local(subStateSize * indices.size());
  for (std::size_t position = 0; position < indices.size(); ++position) {
    const std::size_t begin =
        static_cast<std::size_t>(indices[position]) * subStateSize;
    source.getState(local.data() + position * subStateSize, begin,
                    begin + subStateSize);
  }
  source.synchronize();
  ...
  const auto communicator = source.communicator();
  if (!communicator)
...
void CuStateVecSimulationState<Scalar>::copyToHost(
  ...
  copyStateToHost(*m_state, data, numElementsRequested);
}
