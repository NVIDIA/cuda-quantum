...
__global__ void compute_frobenius_norm_kernel(
...
int compute_matrix_exp(cuDoubleComplex *d_A, cuDoubleComplex *d_expA, int N,
  ...
  if (!cublasH || !cusolverH) {
    throw std::runtime_error("compute_matrix_exp requires valid cuBLAS and cuSOLVER handles");
  }
  ...
  if (N <= 16) {
    // Use custom CUDA norm for small matrices (avoids host-side dependency issues or small-vec bugs)
    // Use d_workspace as scratch for output (first double)
    double *d_norm = reinterpret_cast<double *>(d_workspace);
    int nElems = N * N;
    int block = 256;
    int grid = 1;
    // Kernel defined above
    compute_frobenius_norm_kernel<<<grid, block, block * sizeof(double), stream>>>(
        reinterpret_cast<const cuDoubleComplex *>(d_A), nElems, d_norm);
    
    MATRIX_EXP_CUDA_CHECK(cudaGetLastError());
    MATRIX_EXP_CUDA_CHECK(cudaMemcpy(&norm_A, d_norm, sizeof(double), cudaMemcpyDeviceToHost));
  ...
  MATRIX_EXP_CUDA_CHECK(cudaMemcpy(d_A_scaled, d_A, N * N * sizeof(cuDoubleComplex),
  ...
  // A2 = A_scaled^2
  alpha = one;
  MATRIX_EXP_CUBLAS_CHECK(cublasZgemm(cublasH, CUBLAS_OP_N, CUBLAS_OP_N,
                           N, N, N,
                           &alpha,
                           d_A_scaled, N,
                           d_A_scaled, N,
  ...
  MATRIX_EXP_CUBLAS_CHECK(cublasZaxpy(cublasH, nn, &alpha, d_tmp, 1, d_tmp2, 1));
  ...
  alpha = one;
  beta = one;
  MATRIX_EXP_CUBLAS_CHECK(cublasZgeam(cublasH, CUBLAS_OP_N, CUBLAS_OP_N,
                           N, N,
                           &alpha, d_V, N,
                           &beta, d_U, N,
                           d_tmp2, N));
  ...
  MATRIX_EXP_CUSOLVER_CHECK(
      cusolverDnZgetrs(cusolverH, CUBLAS_OP_N, N, N, d_V, N, d_pivots,
                       d_tmp2, N, d_info));
  ...
  if (current != d_expA) {
    MATRIX_EXP_CUDA_CHECK(cudaMemcpy(d_expA, current, N * N * sizeof(cuDoubleComplex), cudaMemcpyDeviceToDevice));
  }
...
int apply_unitary_to_density_matrix(const cuDoubleComplex *d_U,
  ...
  if (!cublasH) {
    throw std::runtime_error("apply_unitary_to_density_matrix requires valid cuBLAS handle");
  }
  ...
  MATRIX_EXP_CUBLAS_CHECK(cublasZgemm(cublasH, CUBLAS_OP_N, CUBLAS_OP_C,
                           N, N, N,
                           &alpha,
                           d_temp, N,
                           d_U, N,
