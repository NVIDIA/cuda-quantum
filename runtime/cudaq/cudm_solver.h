...
struct Config {
    ...
    std::variant<InitialState, std::vector<std::complex<double>>> initial_state;  // Initial state
...
public:
    ...
    void evolve(cudm_state &state, cudensitymatOperator_t &liouvillian, const std::vector<cudensitymatOperator_t> &obervable_ops, evolve_result &result);

    evolve_result evolve_dynamics();

    cudensitymatOperator_t construct_liouvillian(cudensitymatHandle_t handle, const cudensitymatOperator_t &hamiltonian, const std::vector<cudensitymatOperator_t> &collapse_operators, bool me_solve);
