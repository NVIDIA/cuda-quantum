...
void allocateShots(
    std::span<cudaq::KrausTrajectory> trajectories,
    std::size_t total_shots,
    const ShotAllocationStrategy& strategy) {
    
    if (trajectories.empty()) {
        throw std::invalid_argument("Cannot allocate shots to empty trajectory list");
    }
    ...
    switch (strategy.type) {
        case ShotAllocationStrategy::Type::PROPORTIONAL: {
            ...
            if (total_probability <= 0.0) {
                throw std::invalid_argument("Total probability must be positive for proportional allocation");
            }
        ...
        case ShotAllocationStrategy::Type::LOW_WEIGHT_BIAS: {
            ...
            if (total_weight <= 0.0) {
                throw std::invalid_argument("Total weight must be positive for biased allocation");
            }
            
            for (std::size_t i = 0; i < trajectories.size(); ++i) {
                trajectories[i].num_shots = static_cast<std::size_t>(
                    (weights[i] / total_weight) * total_shots);
            }
            break;
        }
