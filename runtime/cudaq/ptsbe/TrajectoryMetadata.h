...
struct TrajectoryMetadata {
  ...
  TrajectoryMetadata(std::size_t id, std::vector<cudaq::KrausSelection> sels,
                     double prob, std::size_t shots)
      : trajectory_id(id), kraus_selections(std::move(sels)), probability(prob),
        num_shots(shots) {}
  ...
  explicit TrajectoryMetadata(const cudaq::KrausTrajectory &traj)
      : trajectory_id(traj.trajectory_id),
        kraus_selections(traj.kraus_selections), probability(traj.probability),
