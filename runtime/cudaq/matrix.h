...
public:
  ...
  complex_matrix operator+(const complex_matrix &other) const;
  ...
  std::vector<value_type> get_row(std::size_t i) const;

  void set_row(std::size_t i, const std::vector<value_type> &row);
};
