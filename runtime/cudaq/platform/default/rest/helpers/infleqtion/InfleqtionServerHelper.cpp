...
{
  std::string escaped_string = input_string;
  // Escape backslashes, double quotes, and newlines
  std::string::size_type pos = 0;
  while ((pos = escaped_string.find("\\", pos)) != std::string::npos)
  {
  ...
  pos = 0;
  while ((pos = escaped_string.find("\n", pos)) != std::string::npos)
  {
    escaped_string.replace(pos, 1, "\\n");
    pos += 2; // Move past the inserted escape sequence
  }
  // Wrap the formatted string in a JSON array
...
namespace cudaq
{
  ...
  /// @brief The InfleqtionServerHelper class extends the ServerHelper class to handle
  /// interactions with the Infleqtion server for submitting and retrieving quantum
  /// computation jobs.
  ...
  void InfleqtionServerHelper::initialize(BackendConfig config)
  {
    ...
    if (!keyExists("token"))
    {
      backendConfig["token"] = getEnvVar("SUPERSTAQ_API_KEY", "0", isTokenRequired);
    }

    // Construct the API job path
    backendConfig["job_path"] = backendConfig["url"] + '/' +
                                backendConfig["version"] + "/jobs";
  ...
  RestHeaders InfleqtionServerHelper::getHeaders()
  {
    ...
    if (!keyExists("token") || !keyExists("user_agent"))
      throw std::runtime_error("Required keys 'token' or 'user_agent' not found in backendConfig.");
  ...
  ServerJobPayload InfleqtionServerHelper::createJob(std::vector<KernelExecution> &circuitCodes)
  {
    // Check if the necessary keys exist in the configuration
    if (!keyExists("target") || !keyExists("job_path"))
      throw std::runtime_error("Key doesn't exist in backendConfig.");

    // For initial implementation, only submit one circuit
    if (circuitCodes.size() != 1)
    {
      throw std::runtime_error("Only single circuit submission is supported.");
    }
    ...
    auto ret = std::make_tuple(backendConfig.at("job_path"), headers, std::vector<ServerMessage>{job});
    return ret;
  }
  ...
  std::string InfleqtionServerHelper::constructGetJobPath(ServerMessage &postResponse)
  {
    // Extract job ID
  ...
  std::string InfleqtionServerHelper::constructGetJobPath(std::string &jobId)
  {
    if (!keyExists("url") || !keyExists("version"))
      throw std::runtime_error("Keys 'url' or 'version' don't exist in backendConfig.");
  ...
  bool InfleqtionServerHelper::jobIsDone(ServerMessage &getJobResponse)
  {
    ...
    if (!getJobResponse.contains("status"))
      throw std::runtime_error("ServerMessage doesn't contain 'status' key.");
  ...
  cudaq::sample_result InfleqtionServerHelper::processResults(ServerMessage &getJobResponse,
                                                              std::string &jobId)
  {
  ...
  std::string InfleqtionServerHelper::getEnvVar(const std::string &key, const std::string &defaultVal,
                                                const bool isRequired) const
  {
  ...
  std::string InfleqtionServerHelper::getValueOrDefault(const BackendConfig &config,
                                                        const std::string &key,
                                                        const std::string &defaultValue) const
  {
...
CUDAQ_REGISTER_TYPE(cudaq::ServerHelper, cudaq::InfleqtionServerHelper, infleqtion)