...
/// @brief The QuantinuumLegacyServerHelper implements the ServerHelper
/// interface to map Job requests and Job result retrievals actions from the
/// calling Executor to the specific schema required by the remote Quantinuum
/// REST server. This is temporary till we switch to the Nexus-based
class QuantinuumLegacyServerHelper : public ServerHelper {
protected:
  /// @brief The base URL
...
public:
  ...
  const std::string name() const override { return "quantinuum_legacy"; }
...
ServerJobPayload QuantinuumLegacyServerHelper::createJob(
...
std::string
QuantinuumLegacyServerHelper::extractJobId(ServerMessage &postResponse) {
  return postResponse["job"].get<std::string>();
}

std::string
QuantinuumLegacyServerHelper::constructGetJobPath(ServerMessage &postResponse) {
  return baseUrl + "job/" + extractJobId(postResponse);
}

std::string
QuantinuumLegacyServerHelper::constructGetJobPath(std::string &jobId) {
  return baseUrl + "job/" + jobId;
}

bool QuantinuumLegacyServerHelper::jobIsDone(ServerMessage &getJobResponse) {
...
cudaq::sample_result
QuantinuumLegacyServerHelper::processResults(ServerMessage &postJobResponse,
                                             std::string &jobId) {
...
QuantinuumLegacyServerHelper::generateRequestHeader() const {
...
RestHeaders QuantinuumLegacyServerHelper::getHeaders() {
  return generateRequestHeader();
}

/// Refresh the api key and refresh-token
void QuantinuumLegacyServerHelper::refreshTokens(bool force_refresh) {
  std::mutex m;
...
CUDAQ_REGISTER_TYPE(cudaq::ServerHelper, cudaq::QuantinuumLegacyServerHelper,
                    quantinuum_legacy)
