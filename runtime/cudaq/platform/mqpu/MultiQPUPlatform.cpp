    ...
    if (!qpuSubType.empty()) {
      ...
      if (qpuSubType == "NvcfSimulatorQPU") {
        ...
        if (option.empty()) {
          ...
          configStr = fmt::format("target;nvqc;simulator;{};option;{}", simName,
                                  option);
        }
        ...
        if (simName.find("nvidia") != std::string::npos && numQpus > 1 &&
            (option.empty() || option.find("mqpu") != std::string::npos)) {
        ...
        // URL list and options list. If one simulator name is provided,
        // assuming that all the URL should be using the same simulator.
        if (sims.size() > 1 && sims.size() != urls.size() &&
            sims.size() != options.size())
          throw std::runtime_error(fmt::format(
              "Invalid number of remote backend simulators and/or options "
              "provided: "
              "receiving simulators {} and options {}, expecting equal number "
              "of simulators and options (if provided) which is {}.",
        ...
        for (std::size_t qId = 0; qId < urls.size(); ++qId) {
          ...
          const auto optionName =
              options.size() == 1 ? options.front() : options[qId];
          // Populate the information and add the QPUs
          auto qpu = cudaq::registry::get<cudaq::QPU>("RemoteSimulatorQPU");
          qpu->setId(qId);
          const std::string configStr =
              fmt::format("url;{};simulator;{};option;{}", formatUrl(urls[qId]),
                          simName, optionName);
          qpu->setTargetBackend(configStr);
          threadToQpuId[std::hash<std::thread::id>{}(
