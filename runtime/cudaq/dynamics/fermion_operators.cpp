...
void fermion_operator::validate_opcode() const {
  ...
  assert(std::find(valid_op_codes.cbegin(), valid_op_codes.cend(),
                   std::abs(this->op_code)) != valid_op_codes.cend());
  // While -8 should be fine for all of the implemented logic, I believe it can
  // never occur within this representation. A value of -9 on the other hand
  // should also never occur, and if it does then something is wrong with the
  // code and/or my reasoning.
...
std::string fermion_operator::to_string(
