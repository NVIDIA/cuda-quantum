...
namespace cudaq {
cudensitymatState_t initialize_state(cudensitymatHandle_t handle, cudensitymatStatePurity_t purity, int num_modes, const std::vector<int64_t> &mode_extents) {
    try {
        cudensitymatState_t state;
        auto status = cudensitymatCreateState(handle, purity, num_modes, mode_extents.data(), 1, CUDA_R_64F, &state);
...
void scale_state(cudensitymatHandle_t handle, cudensitymatState_t state, double scale_factor, cudaStream_t stream) {
    try {
        auto status = cudensitymatStateComputeScaling(handle, state, &scale_factor, stream);
        if (status != CUDENSITYMAT_STATUS_SUCCESS) {
...
cudensitymatOperator_t compute_lindblad_operator(cudensitymatHandle_t handle, const std::vector<matrix_2> &c_ops, const std::vector<int64_t> &mode_extents) {
    try {
        if (c_ops.empty()) {
        ...
        auto status = cudensitymatCreateOperator(handle, static_cast<int32_t>(mode_extents.size()), mode_extents.data(), &lindblad_op);
        if (status != CUDENSITYMAT_STATUS_SUCCESS) {
        ...
        for (const auto &c_op : c_ops) {
            ...
            status = cudensitymatCreateOperatorTerm(handle, static_cast<int32_t>(mode_extents.size()), mode_extents.data(), &term);
            if (status != CUDENSITYMAT_STATUS_SUCCESS) {
            ...
            status = cudensitymatOperatorAppendTerm(handle, lindblad_op, term, 0, {1.0}, scalarCallback);
            if (status != CUDENSITYMAT_STATUS_SUCCESS) {
...
cudensitymatOperator_t convert_to_cudensitymat_operator(cudensitymatHandle_t handle, const std::map<std::string, std::complex<double>> &parameters, const operator_sum &op, const std::vector<int64_t> &mode_extents) {
    try {
        cudensitymatOperator_t operator_handle;
        auto status = cudensitymatCreateOperator(handle, static_cast<int32_t>(mode_extents.size()), mode_extents.data(), &operator_handle);
        if (status != CUDENSITYMAT_STATUS_SUCCESS) {
        ...
        status = cudensitymatOperatorTermAppendGeneralProduct(handle, term, 
        num_elem_operators, &num_operator_modes, &operator_mode_extents, 
        &operator_mode_strides, state_modes_acted_on, nullptr, 
        CUDA_C_64F, &tensor_data, &tensorCallback, coefficient, scalarCallback);
        if (status != CUDENSITYMAT_STATUS_SUCCESS) {
            cudensitymatDestroyOperatorTerm(term);
            cudensitymatDestroyOperator(operator_handle);
            throw std::runtime_error("Failed to attach flat_matrix to operator term.");
        }

        status = cudensitymatOperatorAppendTerm(handle, operator_handle, term, 0, coefficient, scalarCallback);
        if (status != CUDENSITYMAT_STATUS_SUCCESS) {
            cudensitymatDestroyOperatorTerm(term);
            cudensitymatDestroyOperator(operator_handle);
        ...
        std::cerr << "Error in convert_to_cudensitymat_operator: " << e.what() << std::endl;
        throw;
    }
}

cudensitymatOperator_t construct_liovillian(cudensitymatHandle_t handle, const cudensitymatOperator_t &hamiltonian, const std::vector<cudensitymatOperator_t> &collapse_operators, double gamma) {
    try {
        ...
        cudensitymatWrappedScalarCallback_t scalarCallback = {nullptr, nullptr};
        status = cudensitymatOperatorAppendTerm(handle, liouvillian, hamiltonian, 0, {1.0, 0.0}, scalarCallback);
        if (status != CUDENSITYMAT_STATUS_SUCCESS) {
        ...
        for (const auto &c_op : collapse_operators) {
            status = cudensitymatOperatorAppendTerm(handle, liouvillian, c_op, 0, coefficient, scalarCallback);
            if (status != CUDENSITYMAT_STATUS_SUCCESS) {
                cudensitymatDestroyOperator(liouvillian);
