...
// // Can be removed
// matrix_2 taylor_series_expm(const matrix_2 &op_matrix,
//                                              int order = 20) {
//     matrix_2 result = matrix_2(op_matrix.get_rows(), op_matrix.get_columns());
//     matrix_2 op_matrix_n = matrix_2(op_matrix.get_rows(), op_matrix.get_columns());
...
//         throw std::runtime_error("GPU-based matrix exponentiation not implemented.");
//     } else {
...
//         try {
//             noise_model.add_all_qubit_channel(step_kernel_name, kraus_channel({std::move(M0), std::move(M1)}));
...
// // evolve_result launch_analog_hamiltonian_kernel(const std::string &target_name,
...
// //         std::map<std::string, std::complex<double>> parameters = {{"time", t.value()}};

// //         amp_ts.emplace_back(hamiltonian.get_amplitude().evaluate(parameters).real(), t.value().real());
// //         ph_ts.emplace_back(hamiltonian.get_phase().evaluate(parameters).real(), t.value().real());
// //         dg_ts.emplace_back(hamiltonian.get_delta_global().evaluate(parameters).real(), t.value().real());
