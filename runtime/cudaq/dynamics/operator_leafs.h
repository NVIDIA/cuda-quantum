...
private:
  ...
  // The factor that should be applied when exchanging two operators with the same
  // group id that act on different degrees of freedom. E.g. for fermion relations
  // {a†(k), a(q)} = 0 for k != q, the exchange factor is -1.
  ...
  constexpr commutation_relations(int group_id) 
  : id(group_id) {}
...
public:
  ...
  /// Default commutation relations mean that two operator always commute as long as they
  /// act on different degrees of freedom.
  static constexpr commutation_relations default_commutation_relations = commutation_relations(-1);
  static constexpr commutation_relations fermion_commutation_relations = commutation_relations(-2);
  static constexpr commutation_relations boson_commutation_relations = default_commutation_relations;
  static commutation_relations custom_commutation_relations(uint id);

  static constexpr commutation_relations commutation_group = default_commutation_relations;
