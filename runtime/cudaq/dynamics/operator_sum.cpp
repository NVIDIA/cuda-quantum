...
EvalTy
operator_sum<HandlerTy>::evaluate(OperatorArithmetics<EvalTy> arithmetics,
                                  bool pad_sum_terms,
                                  bool pad_product_terms) const {
  ...
  if (pad_sum_terms) {
    ...
    EvalTy sum =
        padded_term.template evaluate<EvalTy>(arithmetics, pad_product_terms);
    ...
    EvalTy sum =
        terms[0].template evaluate<EvalTy>(arithmetics, pad_product_terms);
    for (auto term_idx = 1; term_idx < terms.size(); ++term_idx) {
      EvalTy term_eval = terms[term_idx].template evaluate<EvalTy>(
          arithmetics, pad_product_terms);
      sum = arithmetics.add(std::move(sum), std::move(term_eval));
    }
    return sum;
  }
...
#define INSTANTIATE_SUM_EVALUATE_METHODS(HandlerTy, EvalTy)                    \
  ...
  template EvalTy operator_sum<HandlerTy>::evaluate(                           \
      OperatorArithmetics<EvalTy> arithmetics, bool pad_sum,                   \
      bool pad_products) const;
...
matrix_2 operator_sum<HandlerTy>::to_matrix(
    ...
    const {
  return std::move(this->evaluate(OperatorArithmetics<EvaluatedMatrix>(
...
matrix_2 operator_sum<spin_operator>::to_matrix(
  ...
  auto evaluated = this->evaluate(
      OperatorArithmetics<EvaluatedCanonicalized>(dimensions, parameters), true,
