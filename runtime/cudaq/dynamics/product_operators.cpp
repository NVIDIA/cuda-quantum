...
void product_operator<HandlerTy>::insert(T &&other, bool update_id) {
  ...
  if (pos != this->operators.begin() && (pos - 1)->target == other.target)
    this->coefficient *=
        this->operators.erase(pos - 1, pos - 1)
            ->inplace_mult(
                other); // erase: constant time conversion to non-const iterator
  else
...
product_operator<HandlerTy>::m_evaluate(MatrixArithmetics arithmetics,
  ...
  auto padded_op = [&arithmetics,
    ...
    for (const auto &degree : degrees) {
      if (std::find(op_degrees.cbegin(), op_degrees.cend(), degree) ==
        ...
        padded.push_back(EvaluatedMatrix(
            identity.degrees(), identity.to_matrix(arithmetics.m_dimensions)));
      }
    }
    ...
    return arithmetics.tensor(
        std::move(ids),
        EvaluatedMatrix(op_degrees, op.to_matrix(arithmetics.m_dimensions,
  ...
  if (this->operators.size() > 0) {
    if (pad_terms) {
      ...
      return EvaluatedMatrix(std::move(prod.degrees()),
                             coefficient * prod.matrix());
    } else {
      EvaluatedMatrix prod(
          this->operators[0].degrees(),
          this->operators[0].to_matrix(arithmetics.m_dimensions,
                                       arithmetics.m_parameters));
      for (auto op_idx = 1; op_idx < this->operators.size(); ++op_idx) {
        auto mat = this->operators[op_idx].to_matrix(arithmetics.m_dimensions,
                                                     arithmetics.m_parameters);
        prod = arithmetics.mul(
            std::move(prod),
            EvaluatedMatrix(this->operators[op_idx].degrees(), mat));
      }
      return EvaluatedMatrix(std::move(prod.degrees()),
                             coefficient * prod.matrix());
    }
...
#define INSTANTIATE_PRODUCT_PRIVATE_METHODS(HandlerTy)                         \
  ...
  template void product_operator<HandlerTy>::aggregate_terms(                  \
      HandlerTy &&item1, HandlerTy &&item2, HandlerTy &&item3);                \
                                                                               \
  template EvaluatedMatrix product_operator<HandlerTy>::m_evaluate(            \
...
#define INSTANTIATE_PRODUCT_PROPERTIES(HandlerTy)                              \
                                                                               \
  template std::vector<int> product_operator<HandlerTy>::degrees() const;      \
                                                                               \
...
product_operator<HandlerTy>::product_operator(
    ...
    : coefficient(std::move(coefficient)), operators(atomic_operators),
      term_id(term_id) {
  assert(!HandlerTy::can_be_canonicalized || this->is_canonicalized());
}
...
product_operator<HandlerTy>::product_operator(
    ...
    std::string &&term_id)
    : coefficient(std::move(coefficient)),
      operators(std::move(atomic_operators)), term_id(std::move(term_id)) {
  assert(!HandlerTy::can_be_canonicalized || this->is_canonicalized());
}
...
product_operator<HandlerTy>::product_operator(
    ...
    : coefficient(std::move(other.coefficient)),
      term_id(std::move(other.term_id)) {
...
bool product_operator<HandlerTy>::operator==(
  ...
  return this->term_id == other.term_id &&
         this->coefficient == other.coefficient;
}
...
product_operator<HandlerTy> product_operator<HandlerTy>::operator*(
  ...
  product_operator<HandlerTy> self(this->coefficient, std::move(terms),
                                   this->term_id);
  return self *= other;
}
...
#define PRODUCT_ADDITION_PRODUCT(op)                                           \
  ...
  operator_sum<HandlerTy> product_operator<HandlerTy>::operator op(            \
      const product_operator<HandlerTy> &other) const {                        \
    return operator_sum<HandlerTy>(op other,                                   \
                                   product_operator<HandlerTy>(*this));        \
  }
