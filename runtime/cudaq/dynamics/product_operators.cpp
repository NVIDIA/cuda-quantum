...
EvalTy
product_operator<HandlerTy>::evaluate(OperatorArithmetics<EvalTy> arithmetics,
  ...
  auto padded_op = [&arithmetics,
    ...
    // to ensure that no reordering needs to happen during computing the tensor
    // of the ids.
...
matrix_2 product_operator<spin_operator>::to_matrix(
  ...
  auto evaluated = this->evaluate(
      OperatorArithmetics<EvaluatedCanonicalized>(dimensions, parameters),
      false);
  // FIXME: ASSERTS
