...
cudm_state cudm_solver::initialize_state() {
    ...
    return cudm_state::create_initial_state(config_.initial_state, mode_extents, !config_.collapse_operators.empty());
}

cudensitymatOperator_t cudm_solver::construct_liouvillian(cudensitymatHandle_t handle, const cudensitymatOperator_t &hamiltonian, const std::vector<cudensitymatOperator_t> &collapse_operators, bool me_solve) {
    return construct_liovillian(handle, hamiltonian, collapse_operators, me_solve ? 1.0 : 0.0);
}

void cudm_solver::evolve(cudm_state &state, cudensitymatOperator_t &liouvillian, const std::vector<cudensitymatOperator_t> &observable_ops, evolve_result &result) {
