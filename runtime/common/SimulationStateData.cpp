...
//   mlir::ModuleOp moduleOp, mlir::func::FuncOp func, const void* args, std::size_t startingArgIdx) {
...
template<typename T>
static void addStateData(cudaq::state* state, cudaq::opt::SimulationStateDataStore& store) {
  ...
  if (state->is_on_gpu()) {
    ...
    std::cout << std::endl;
    store.addData(hostData, numElements, sizeof(T), [](void* ptr) { delete static_cast<T*>(ptr); } );
  } else {
...
cudaq::opt::SimulationStateDataStore readSimulationStateData(
  std::pair<std::size_t, std::vector<std::size_t>> &argumentLayout, const void* args) {
  cudaq::opt::SimulationStateDataStore dataStore;

  auto offsets = argumentLayout.second;
  std::cout << "Reading simulation data into data store: " <<  offsets.size() << std::endl;
