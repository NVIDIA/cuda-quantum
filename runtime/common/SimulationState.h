...
class EmptySimulationState : public SimulationState {
public:
  Tensor getTensor(std::size_t tensorIdx) const override {
    throw std::runtime_error(
        "EmptySimulationState does not contain any tensors.");
  }

  virtual std::vector<Tensor> getTensors() const override { return {}; }

  std::size_t getNumTensors() const override { return 0; }

  std::size_t getNumQubits() const override { return 0; }

  std::complex<double> overlap(const SimulationState &other) override {
    throw std::runtime_error(
        "EmptySimulationState does not support overlap computation.");
  }

  std::complex<double>
  getAmplitude(const std::vector<int> &basisState) override {
    throw std::runtime_error(
        "EmptySimulationState does not support amplitude computation.");
  }

  void dump(std::ostream &os) const override {
    os << "EmptySimulationState: no data." << std::endl;
  }

  precision getPrecision() const override {
    throw std::runtime_error(
        "EmptySimulationState does not support precision query.");
  }

  void destroyState() override {
    // No resources to free for an empty state.
  }
...
protected:
  ...
  createFromSizeAndPtr(std::size_t, void *, std::size_t dataType) override {
    throw std::runtime_error(
        "EmptySimulationState cannot create a new state from data.");
  }
};
