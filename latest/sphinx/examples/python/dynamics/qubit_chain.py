...
for i in qubits:
    H += .5 * 2 * np.pi * nu_x * spin.x(i)
    H += .5 * 2 * np.pi * nu_z * spin.z(i)

    if N > 1:
        j = i + 1 if i < (
            N -
            1) else 0  # Nearest neighbors, with periodic boundary conditions
        H += .5 * 2 * np.pi * J * spin.x(i) * spin.x(j)
        H += .5 * 2 * np.pi * J * spin.y(i) * spin.y(j)

# Collapse operators
L_ops = []
for i in qubits:
    L_ops.append(np.sqrt(Gamma) * spin.plus(i))
