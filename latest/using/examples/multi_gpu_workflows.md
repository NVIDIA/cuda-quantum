...
-   [Examples](examples.html){.reference .internal}
    ...
    -   [Multi-GPU Workflows](#){.current .reference .internal}
        ...
        -   [Pooling the memory of multiple GPUs ([`mgpu`{.code
            .docutils .literal
            .notranslate}]{.pre})](#pooling-the-memory-of-multiple-gpus-mgpu){.reference
            .internal}
        -   [Parallel execution over multiple QPUs ([`mqpu`{.code
            .docutils .literal
            .notranslate}]{.pre})](#parallel-execution-over-multiple-qpus-mqpu){.reference
            .internal}
...
[`qpp-cpu`{.code .docutils .literal .notranslate}]{.pre} backend. This
is helpful for debugging code and testing small circuits.
...
[`nvidia`{.code .docutils .literal .notranslate}]{.pre} backend, you can
...
## Pooling the memory of multiple GPUs ([`mgpu`{.code .docutils .literal .notranslate}]{.pre})[¶](#pooling-the-memory-of-multiple-gpus-mgpu "Permalink to this heading"){.headerlink}

As [`N`{.code .docutils .literal .notranslate}]{.pre} gets larger, the
size of the state vector that needs to be stored in memory increases
...
qubits will quickly exceed the memory of as single GPU. The
[`mqpu`{.code .docutils .literal .notranslate}]{.pre} backend solved
this problem by pooling the memory of multiple GPUs across multiple
nodes to perform a single state vector simulation.
...
[`mpiexec`{.code .docutils .literal .notranslate}]{.pre}` `{.code
.docutils .literal .notranslate}[`-np`{.code .docutils .literal
.notranslate}]{.pre}` `{.code .docutils .literal .notranslate}[`n`{.code
.docutils .literal .notranslate}]{.pre}` `{.code .docutils .literal
.notranslate}[`python3`{.code .docutils .literal
.notranslate}]{.pre}` `{.code .docutils .literal
.notranslate}[`program.py`{.code .docutils .literal
.notranslate}]{.pre}` `{.code .docutils .literal
.notranslate}[`--target`{.code .docutils .literal
.notranslate}]{.pre}` `{.code .docutils .literal
.notranslate}[`nvidia`{.code .docutils .literal
.notranslate}]{.pre}` `{.code .docutils .literal
.notranslate}[`--target-option`{.code .docutils .literal
.notranslate}]{.pre}` `{.code .docutils .literal
.notranslate}[`mgpu`{.code .docutils .literal .notranslate}]{.pre}

This code will execute in an MPI context and provide additional memory
to simulation much larger state vectors.

You can also set [`cudaq.set_target('nvidia',`{.code .docutils .literal
.notranslate}]{.pre}` `{.code .docutils .literal
.notranslate}[`option='mgpu')`{.code .docutils .literal
.notranslate}]{.pre} within the file to select the target.
:::

::: {#parallel-execution-over-multiple-qpus-mqpu .section}
## Parallel execution over multiple QPUs ([`mqpu`{.code .docutils .literal .notranslate}]{.pre})[¶](#parallel-execution-over-multiple-qpus-mqpu "Permalink to this heading"){.headerlink}
...
.docutils .literal .notranslate}]{.pre} backend uses multiple GPUs to
simulate QPUs so you can accelerate quantum applications with
parallelization.
...
Hamiltonian. A standard [`observe`{.code .docutils .literal
.notranslate}]{.pre} call will run the program on a single GPU. Adding
the argument [`execution=cudaq.parallel.thread`{.code .docutils .literal
.notranslate}]{.pre} or [`execution=cudaq.parallel.mpi`{.code .docutils
.literal .notranslate}]{.pre} will automatically distribute the
Hamiltonian terms across multiple GPUs on a single node or multiple GPUs
on multiple nodes, respectively.

The code is executed with [`mpiexec`{.code .docutils .literal
.notranslate}]{.pre}` `{.code .docutils .literal
.notranslate}[`-np`{.code .docutils .literal
.notranslate}]{.pre}` `{.code .docutils .literal .notranslate}[`n`{.code
.docutils .literal .notranslate}]{.pre}` `{.code .docutils .literal
.notranslate}[`python3`{.code .docutils .literal
.notranslate}]{.pre}` `{.code .docutils .literal
...
.notranslate}[`--target-option`{.code .docutils .literal
.notranslate}]{.pre}` `{.code .docutils .literal
.notranslate}[`mqpu`{.code .docutils .literal .notranslate}]{.pre} where
[\\(n\\)]{.math .notranslate .nohighlight} is the number of GPUs
available.
...
.notranslate}]{.pre} backend is to batch circuit evaluations across
multiple simulated QPUs.
...
:code"[`observe`{.code .docutils .literal .notranslate}]{.pre} call and
run by default on a single GPU. The code below times this entire
process.
...
([`asyncresults`{.code .docutils .literal .notranslate}]{.pre}) and
retrieved later with the [`get`{.code .docutils .literal
.notranslate}]{.pre} command. The following loops over the parameter
batches, and the sets of parameters in each batch. The parameter sets
are provided as inputs to [`observe_async`{.code .docutils .literal
.notranslate}]{.pre} along with specification of a [`qpu_id`{.code
.docutils .literal .notranslate}]{.pre} which designates the GPU (of the
four available) which will run computation. A speedup of up to 4x can be
expected with results varying by problem size.
