    ...
    -   [Cloud](../cloud.html){.reference .internal}
        ...
        -   [Scaleway QaaS (scaleway)](scaleway.html){.reference
            ...
            -   [Manage your QPU
                ...
                .internal}
        -   [qBraid](#){.current .reference .internal}
...
Previous](scaleway.html "Scaleway Quantum as a Service"){.btn
...
Generate an API key from your [qBraid
account](https://account.qbraid.com/){.reference .external} and export
...
Alternatively, the API key can be passed directly to
[`cudaq.set_target`{.docutils .literal .notranslate}]{.pre} via the
[`api_key`{.docutils .literal .notranslate}]{.pre} argument (see below).
:::
...
([`qbraid:qbraid:sim:qir-sv`{.docutils .literal .notranslate}]{.pre}).

To specify a different qBraid device, set the [`machine`{.docutils
.literal .notranslate}]{.pre} parameter to its qBraid device ID.
...
[`QBRAID_API_KEY`{.docutils .literal .notranslate}]{.pre} environment
variable.
...
qBraid devices are cloud-hosted, so local emulation via the
[`emulate`{.docutils .literal .notranslate}]{.pre} flag is not supported
--- all jobs are executed on the qBraid service. To run without
submitting to real hardware, select one of the qBraid simulator devices
(for example, [`qbraid:qbraid:sim:qir-sv`{.docutils .literal
.notranslate}]{.pre}) via the [`machine`{.docutils .literal
...
[`cudaq.observe`{.docutils .literal .notranslate}]{.pre}. The default is
1000.
...
.literal .notranslate}[`qbraid`{.docutils .literal .notranslate}]{.pre}
to the [`nvq++`{.docutils .literal .notranslate}]{.pre} compiler. By
default jobs are submitted to the qBraid state vector simulator
([`qbraid:qbraid:sim:qir-sv`{.docutils .literal .notranslate}]{.pre}).
...
To execute kernels on a different device, pass
[`--qbraid-machine`{.docutils .literal .notranslate}]{.pre} with the
qBraid device ID:
...
The API key can be passed explicitly with [`--qbraid-api_key`{.docutils
.literal .notranslate}]{.pre} instead of being read from
[`QBRAID_API_KEY`{.docutils .literal .notranslate}]{.pre}:
...
qBraid devices are cloud-hosted, so the [`--emulate`{.docutils .literal
.notranslate}]{.pre} flag is not supported for this target --- all jobs
are executed on the qBraid service. To run without submitting to real
hardware, pass [`--qbraid-machine`{.docutils .literal
.notranslate}]{.pre} with a qBraid simulator device ID (for example,
[`qbraid:qbraid:sim:qir-sv`{.docutils .literal .notranslate}]{.pre}).
:::
:::
