    ...
    -   [Circuit Simulation](../simulators.html){.reference .internal}
        ...
        -   [Multi-QPU Simulators](mqpusims.html){.reference .internal}
            ...
            -   [Multi-QPU with Multi-Node Multi-GPU
                ...
                .internal}
        -   [Noisy Simulators](#){.current .reference .internal}
            -   [Trajectory Noisy
                Simulation](#trajectory-noisy-simulation){.reference
                .internal}
            -   [Density Matrix](#density-matrix){.reference .internal}
            -   [Stim](#stim){.reference .internal}
...
Previous](mqpusims.html "Multiple QPUs"){.btn .btn-neutral .float-left
accesskey="p"} [Next []{.fa .fa-arrow-circle-right
aria-hidden="true"}](photonics.html "Photonics Simulators"){.btn
...
::: {#noisy-simulators .section}
# Noisy Simulators[¶](#noisy-simulators "Permalink to this heading"){.headerlink}

::: {#trajectory-noisy-simulation .section}
## Trajectory Noisy Simulation[¶](#trajectory-noisy-simulation "Permalink to this heading"){.headerlink}

CUDA-Q GPU simulator backends, [`nvidia`{.code .docutils .literal
.notranslate}]{.pre}, [`tensornet`{.code .docutils .literal
.notranslate}]{.pre}, and [`tensornet-mps`{.code .docutils .literal
.notranslate}]{.pre}, supports noisy quantum circuit simulations using
quantum trajectory method.

When a [`noise_model`{.code .docutils .literal .notranslate}]{.pre} is
provided to CUDA-Q, the backend target will incorporate quantum noise
into the quantum circuit simulation according to the noise model
specified, as shown in the below example.
...
In the case of bit-string measurement sampling as in the above example,
each measurement 'shot' is executed as a trajectory, whereby Kraus
operators specified in the noise model are sampled.

::: {#unitary-mixture-vs-general-noise-channel .section}
### Unitary Mixture vs. General Noise Channel[¶](#unitary-mixture-vs-general-noise-channel "Permalink to this heading"){.headerlink}

Quantum noise channels can be classified into two categories:

1.  Unitary mixture

The noise channel can be defined by a set of unitary matrices along with
list of probabilities associated with those matrices. The depolarizing
channel is an example of unitary mixture, whereby [`I`{.code .docutils
.literal .notranslate}]{.pre} (no noise), [`X`{.code .docutils .literal
.notranslate}]{.pre}, [`Y`{.code .docutils .literal
.notranslate}]{.pre}, or [`Z`{.code .docutils .literal
.notranslate}]{.pre} unitaries may occur to the quantum state at
pre-defined probabilities.

2.  General noise channel

The channel is defined as a set of non-unitary Kraus matrices,
satisfying the completely positive and trace preserving (CPTP)
condition. An example of this type of channels is the amplitude damping
noise channel.

In trajectory simulation method, simulating unitary mixture noise
channels is more efficient than general noise channels since the
trajectory sampling of the latter requires probability calculation based
on the immediate quantum state.

::: {.admonition .note}
Note

CUDA-Q noise channel utility automatically detects whether a list of
Kraus matrices can be converted to the unitary mixture representation
for more efficient simulation.
:::

+-------------+--------------------+-----------------------------------+
| Backend     | Unitary Mixture    | General Channel                   |
+-------------+--------------------+-----------------------------------+
| [`nv        | YES                | YES                               |
| idia`{.code |                    |                                   |
| .docutils   |                    |                                   |
...
| ate}]{.pre} |                    |                                   |
+-------------+--------------------+-----------------------------------+
| [`tensornet | YES                | YES (number of qubits \> 1)       |
| -mps`{.code |                    |                                   |
| .docutils   |                    |                                   |
| .literal    |                    |                                   |
| .notransl   |                    |                                   |
...
: [**Noise Channel
Support**]{.caption-text}[¶](#id2 "Permalink to this table"){.headerlink}
:::

::: {#trajectory-expectation-value-calculation .section}
### Trajectory Expectation Value Calculation[¶](#trajectory-expectation-value-calculation "Permalink to this heading"){.headerlink}

In trajectory simulation method, the statistical error of observable
expectation value estimation scales asymptotically as
[\\(1/\\sqrt{N\_{trajectories}}\\)]{.math .notranslate .nohighlight},
where [\\(N\_{trajectories}\\)]{.math .notranslate .nohighlight} is the
number of trajectories. Hence, depending on the required level of
accuracy, the number of trajectories can be specified accordingly.
...
In the above example, as we increase the number of trajectories, the
result of CUDA-Q [`observe`{.code .docutils .literal
.notranslate}]{.pre} approaches the true value.

::: {.admonition .note}
Note

With trajectory noisy simulation, the result of CUDA-Q [`observe`{.code
.docutils .literal .notranslate}]{.pre} is inherently stochastic. For a
small number of qubits, the true expectation value can be simulated by
the [[density matrix]{.std
.std-ref}](#density-matrix-cpu-backend){.reference .internal} simulator.
:::
:::

::: {#batched-trajectory-simulation .section}
### Batched Trajectory Simulation[¶](#batched-trajectory-simulation "Permalink to this heading"){.headerlink}

On the [`nvidia`{.code .docutils .literal .notranslate}]{.pre} target,
when simulating many trajectories with small state vectors, the
simulation is batched for optimal performance.

::: {.admonition .note}
Note

Batched trajectory simulation is only available on the single-GPU
execution mode of the [`nvidia`{.code .docutils .literal
.notranslate}]{.pre} target.

If batched trajectory simulation is not activated, e.g., due to problem
size, number of trajectories, or the nature of the circuit (dynamic
circuits with mid-circuit measurements and conditional branching), the
required number of trajectories will be executed sequentially.
:::

The following environment variable options are applicable to the
[`nvidia`{.code .docutils .literal .notranslate}]{.pre} target for
batched trajectory noisy simulation. Any environment variables must be
set prior to setting the target or running "[`import`{.code .docutils
...
+-------------+--------------------+-----------------------------------+
| [`CUDAQ     | positive power of  | The number of state vectors in    |
| _BATCH_SIZE | two or             | the batched mode. The value must  |
| `{.docutils | [`NONE`{.code      | be a power of two. If             |
| .literal    | .docutils .literal | [`NONE`{.code .docutils .literal  |
| .notransl   | .n                 | .notranslate}]{.pre}, the batch   |
| ate}]{.pre} | otranslate}]{.pre} | size will be calculated based on  |
|             |                    | the available device memory.      |
|             |                    | Default is [`NONE`{.code          |
|             |                    | .docutils .literal                |
|             |                    | .notranslate}]{.pre}.             |
+-------------+--------------------+-----------------------------------+
| [`CUDAQ_BA  | positive integer   | The number of trajectory branches |
| TCHED_SIM_M |                    | to be tracked simultaneously in   |
| AX_BRANCHES |                    | the gate fusion. Default is 16.   |
| `{.docutils |                    |                                   |
| .literal    |                    |                                   |
| .notransl   |                    |                                   |
| ate}]{.pre} |                    |                                   |
+-------------+--------------------+-----------------------------------+
| [`CUDAQ_    | positive integer   | The max number of qubits for      |
| BATCHED_SIM |                    | batching. If the qubit count in   |
| _MAX_QUBITS |                    | the circuit is more than this     |
| `{.docutils |                    | value, batched trajectory         |
| .literal    |                    | simulation will be disabled. The  |
| .notransl   |                    | default value is 20.              |
| ate}]{.pre} |                    |                                   |
+-------------+--------------------+-----------------------------------+
| [           | positive integer   | The minimum number of             |
| `CUDAQ_BATC |                    | trajectories for batching. If the |
| HED_SIM_MIN |                    | number of trajectories is less    |
| _BATCH_SIZE |                    | than this value, batched          |
| `{.docutils |                    | trajectory simulation will be     |
| .literal    |                    | disabled. Default value is 4.     |
| .notransl   |                    |                                   |
| ate}]{.pre} |                    |                                   |
+-------------+--------------------+-----------------------------------+

: [**Additional environment variable options for trajectory
simulation**]{.caption-text}[¶](#id3 "Permalink to this table"){.headerlink}

::: {.admonition .note}
Note

The default batched trajectory simulation parameters have been chosen
for optimal performance.
:::

In the below example, we demonstrate the use of these parameters to
control trajectory batching.
...
The [`CUDAQ_LOG_LEVEL`{.code .docutils .literal .notranslate}]{.pre}
[[environment
variable]{.doc}](../../basics/troubleshooting.html){.reference
.internal} can be used to view detailed logs of batched trajectory
simulation, e.g., the batch size.
:::
:::
:::

::: {#density-matrix .section}
## Density Matrix[¶](#density-matrix "Permalink to this heading"){.headerlink}

Density matrix simulation is helpful for understanding the impact of
noise on quantum applications. Unlike state vectors simulation which
manipulates the [\\(2\^n\\)]{.math .notranslate .nohighlight} state
vector, density matrix simulations manipulate the [\\(2\^n x
2\^n\\)]{.math .notranslate .nohighlight} density matrix which defines
an ensemble of states. To learn how you can leverage the
[`density-matrix-cpu`{.code .docutils .literal .notranslate}]{.pre}
backend to study the impact of noise models on your applications, see
the [example
here](https://nvidia.github.io/cuda-quantum/latest/examples/python/noisy_simulations.html){.reference
.external}.

The [Quantum Volume
notebook](https://nvidia.github.io/cuda-quantum/latest/applications/python/quantum_volume.html){.reference
.external} also demonstrates a full application that leverages the
[`density-matrix-cpu`{.code .docutils .literal .notranslate}]{.pre}
backend.

To execute a program on the [`density-matrix-cpu`{.code .docutils
.literal .notranslate}]{.pre} target, use the following commands:
...
::: {#stim .section}
## Stim[¶](#stim "Permalink to this heading"){.headerlink}

This backend provides a fast simulator for circuits containing *only*
Clifford gates. Any non-Clifford gates (such as T gates and Toffoli
gates) are not supported. This simulator is based on the
[Stim](https://github.com/quantumlib/Stim){.reference .external}
library.

To execute a program on the [`stim`{.code .docutils .literal
...
By default CUDA-Q executes kernels using a "shot-by-shot" execution
approach. This allows for conditional gate execution (i.e. full control
flow), but it can be slower than executing Stim a single time and
generating all the shots from that single execution. Set the
[`explicit_measurements`{.code .docutils .literal .notranslate}]{.pre}
flag with [`sample`{.code .docutils .literal .notranslate}]{.pre} API
