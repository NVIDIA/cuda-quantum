...
-   [Realtime](realtime.html){.reference .internal}
    -   [Installation](realtime/installation.html){.reference .internal}
    ...
    -   [Messaging Protocol](realtime/protocol.html){.reference
        .internal}
        -   [Scope](realtime/protocol.html#scope){.reference .internal}
        -   [RPC Header /
    ...
    -   [CPU RoCE Transport](realtime/cpu_transport.html){.reference
        ...
        -   [Two-phase bring-up ([`setup`{.docutils .literal
            ...
            .internal}
        -   [TX modes](realtime/cpu_transport.html#tx-modes){.reference
            .internal}
...
-   [Extending](extending/extending.html){.reference .internal}
    -   [Compiler development](extending/compiler/index.html){.reference
        .internal}
        -   [Compiler IR](extending/compiler/cudaq_ir.html){.reference
            .internal}
            -   [CUDA-Q
    ...
    -   [Add a hardware backend](extending/backend.html){.reference
        ...
        -   [Example
            ...
            .internal}
        -   [Next Steps](extending/backend.html#next-steps){.reference
            .internal}
...
Previous](../index.html "CUDA-Q"){.btn .btn-neutral .float-left
...
[`LD_LIBRARY_PATH`{.docutils .literal .notranslate}]{.pre}. To avoid
having to set them every time you launch a new shell, we recommend
adding them to [`~/.profile`{.docutils .literal .notranslate}]{.pre}
(create the file if it does not exist), and to
...
[`00`{.code .docutils .literal .notranslate}]{.pre} or [`11`{.code
.docutils .literal .notranslate}]{.pre}. The program samples, meaning it
executes, the kernel 1000 times and prints how many times each output
was measured. On average, the values [`00`{.code .docutils .literal
.notranslate}]{.pre} and [`11`{.code .docutils .literal
.notranslate}]{.pre} should be observed around 500 times each.
...
.notranslate}]{.pre}, containing the following code:
...
.notranslate}]{.pre} compiler and run the built application with the
following command:
...
.docutils .literal .notranslate}]{.pre} target ([[GPU-accelerated
...
.docutils .literal .notranslate}]{.pre} ([[OpenMP parallelized CPU-only
statevector simulator]{.std
...
.notranslate}]{.pre}, the program simply seems to hang; that is because
it takes a long time for the CPU-only backend to simulate 28+ qubits!
...
<a href="../_images/widget-recording.gif"
class="reference internal image-reference"><img
src="../_images/widget-recording.gif" style="width: 500px;"
alt="../_images/widget-recording.gif" /></a>
<figcaption><p><span class="caption-text">Example of the interactive
material within CUDA-Q Academic.</span><a href="#id2" class="headerlink"
title="Permalink to this image">¶</a></p></figcaption>
</figure>

The courses can be run using a local installation of CUDA-Q or via the
cloud using [`qBraid`{.code .docutils .literal .notranslate}]{.pre},
[`CoCalc`{.code .docutils .literal .notranslate}]{.pre}, or
[`Google`{.code .docutils .literal .notranslate}]{.pre}` `{.code
.docutils .literal .notranslate}[`Colab`{.code .docutils .literal
.notranslate}]{.pre}.
