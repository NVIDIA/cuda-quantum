...
-   [Integration](integration.html){.reference .internal}
    -   [Downstream CMake Integration](#){.current .reference .internal}
...
Previous](integration.html "Integration with other Software Tools"){.btn
.btn-neutral .float-left accesskey="p"} [Next []{.fa
.fa-arrow-circle-right
aria-hidden="true"}](cuda_gpu.html "Using CUDA and CUDA-Q in a Project"){.btn
...
::: {#cuda-q-and-cmake .section}
# CUDA-Q and CMake[¶](#cuda-q-and-cmake "Permalink to this heading"){.headerlink}

CUDA-Q provides a full compiler toolchain for building out
quantum-classical applications. Since most C++ projects provide some
sort of integration with CMake, it becomes imperative for our toolchain
to integrate well with the CMake build system.
...
leverage [`nvq++`{.code .docutils .literal .notranslate}]{.pre} for
compilation of CUDA-Q source files.

To see how this works, let's create a trivial example. Suppose we have a
folder named [`myCudaQApp`{.code .docutils .literal .notranslate}]{.pre}
and within that folder we have a simple CUDA-Q source file, like
[`ghz.cpp`{.code .docutils .literal .notranslate}]{.pre}. With a local
CUDA-Q installation present on the system, we can create the following
top-level [`CMakeLists.txt`{.code .docutils .literal
.notranslate}]{.pre} file to build this file with [`nvq++`{.code
...
To configure and build this file, all we need to do is execute the
following from the top level of the [`myCudaQApp`{.code .docutils
.literal .notranslate}]{.pre} folder:
...
Notice that by running the [`ninja`{.code .docutils .literal
.notranslate}]{.pre} command with [`--verbose`{.code .docutils .literal
.notranslate}]{.pre}, you'll see that [`nvq++`{.code .docutils .literal
.notranslate}]{.pre} was used to compile and link the executable.
:::
:::
:::
