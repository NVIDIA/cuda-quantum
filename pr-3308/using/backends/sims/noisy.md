...
.notranslate}, `tensornet`{.code .docutils .literal .notranslate}, and
`tensornet-mps`{.code .docutils .literal .notranslate}, supports noisy
quantum circuit simulations using quantum trajectory method.

When a `noise_model`{.code .docutils .literal .notranslate} is provided
to CUDA-Q, the backend target will incorporate quantum noise into the
quantum circuit simulation according to the noise model specified, as
...
.literal .notranslate} (no noise), `X`{.code .docutils .literal
.notranslate}, `Y`{.code .docutils .literal .notranslate}, or `Z`{.code
.docutils .literal .notranslate} unitaries may occur to the quantum
state at pre-defined probabilities.
...
result of CUDA-Q `observe`{.code .docutils .literal .notranslate}
...
.docutils .literal .notranslate} is inherently stochastic. For a small
number of qubits, the true expectation value can be simulated by the
[[density matrix]{.std
...
On the `nvidia`{.code .docutils .literal .notranslate} target, when
simulating many trajectories with small state vectors, the simulation is
...
execution mode of the `nvidia`{.code .docutils .literal .notranslate}
target.
...
`nvidia`{.code .docutils .literal .notranslate} target for batched
trajectory noisy simulation. Any environment variables must be set prior
to setting the target or running "`import cudaq`{.code .docutils
.literal .notranslate}".
...
| _BATCH_SIZE | or `NONE`{.code    | the batched mode. If `NONE`{.code |
| `{.docutils | .docutils .literal | .docutils .literal .notranslate}, |
| .literal    | .notranslate}      | the batch size will be calculated |
| .n          |                    | based on the available device     |
| otranslate} |                    | memory. Default is `NONE`{.code   |
...
| .n          |                    | default value is 20.              |
| otranslate} |                    |                                   |
+-------------+--------------------+-----------------------------------+
| `CUDAQ_BATC | positive integer   | The minimum number of             |
| HED_SIM_MIN |                    | trajectories for batching. If the |
| _BATCH_SIZE |                    | number of trajectories is less    |
| `{.docutils |                    | than this value, batched          |
| .literal    |                    | trajectory simulation will be     |
| .n          |                    | disabled. Default value is 4.     |
| otranslate} |                    |                                   |
+-------------+--------------------+-----------------------------------+
...
The `CUDAQ_LOG_LEVEL`{.code .docutils .literal .notranslate}
[[environment
...
`density-matrix-cpu`{.code .docutils .literal .notranslate} backend to
study the impact of noise models on your applications, see the [example
...
`density-matrix-cpu`{.code .docutils .literal .notranslate} backend.

To execute a program on the `density-matrix-cpu`{.code .docutils
.literal .notranslate} target, use the following commands:
...
`explicit_measurements`{.code .docutils .literal .notranslate} flag with
`sample`{.code .docutils .literal .notranslate} API for efficient
execution.
:::
:::
:::
