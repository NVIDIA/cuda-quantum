...
flag `--target braket`{.docutils .literal .notranslate} to the
`nvq++`{.docutils .literal .notranslate} compiler. By default jobs are
submitted to the state vector simulator, `SV1`{.code .docutils .literal
.notranslate}.
...
`--braket-machine`{.docutils .literal .notranslate} flag to the
...
.literal .notranslate} refers to IQM Garnet QPU.

To emulate the device locally, without submitting through the cloud, you
can also pass the `--emulate`{.docutils .literal .notranslate} flag to
`nvq++`{.docutils .literal .notranslate}.
...
To see a complete example for using Amazon Braket backends, take a look
at our [[C++ examples]{.std
.std-ref}](../../examples/examples.html#examples){.reference .internal}.
:::
...
`SV1`{.code .docutils .literal .notranslate}.
...
To emulate the device locally, without submitting through the cloud, you
can also set the `emulate`{.docutils .literal .notranslate} flag to
...
`cudaq.sample`{.docutils .literal .notranslate}. By default, the
