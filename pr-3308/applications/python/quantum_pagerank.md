    ...
    -   [Grover's Algorithm](grovers.html){.reference .internal}
        ...
        -   [Structure of Grover's
            ...
            -   [Steps 4 and 5: Iteration and
                ...
                .internal}
    -   [Quantum PageRank](#){.current .reference .internal}
        -   [Problem Definition](#Problem-Definition){.reference
            .internal}
        -   [Simulating Quantum PageRank by CUDA-Q
            dynamics](#Simulating-Quantum-PageRank-by-CUDA-Q-dynamics){.reference
            .internal}
        -   [Breakdown of Terms](#Breakdown-of-Terms){.reference
            .internal}
...
Previous](grovers.html "Grover’s Algorithm"){.btn .btn-neutral
.float-left} [Next []{.fa .fa-arrow-circle-right
aria-hidden="true"}](uccsd_wf_ansatz.html "The UCCSD Wavefunction ansatz"){.btn
...
::: {#Quantum-PageRank .section}
Quantum PageRank[¶](#Quantum-PageRank "Permalink to this heading"){.headerlink}
===============================================================================
...
Problem Definition[¶](#Problem-Definition "Permalink to this heading"){.headerlink}
-----------------------------------------------------------------------------------

**PageRank** is an algorithm used to rank nodes---such as web pages or
individuals---in directed or undirected graphs based on their relative
importance.

::: {.line-block}
::: {.line}
Given a graph [\\(G = (V, E)\\)]{.math .notranslate .nohighlight}:
:::

::: {.line}
\- [\\(V\\)]{.math .notranslate .nohighlight} is the set of nodes (e.g.,
web pages, individuals).
:::

::: {.line}
\- [\\(E\\)]{.math .notranslate .nohighlight} is the set of edges (e.g.,
hyperlinks/connections between pages/individuals).
:::
:::

The goal is to assign an importance score to each node based on the
number and importance of incoming links.

------------------------------------------------------------------------

PageRank has broad applications across various fields: ranking web pages
in search engines, identifying influential individuals in social
networks, recommending products in recommendation systems, and measuring
the impact of research papers in scientific citation networks.

Here, we use the Karate Club graph as an example network to rank the
importance of each member within the social structure:
...
The mathematical foundations of the PageRank algorithm include **Link
matrix** [\\(\\mathbf{L}\\)]{.math .notranslate .nohighlight} and
**PageRank Matrix** [\\(\\mathbf{P}\\)]{.math .notranslate
.nohighlight}. Let's explore their definitions and meanings: For
**Social Network Analysis (SNA)**, the Link matrix
[\\(\\mathbf{L}\\)]{.math .notranslate .nohighlight} is used to
represent how influence, information, or relationships flow between
people (nodes). The entry [\\(L\_{ij}\\)]{.math .notranslate
.nohighlight} in row [\\(i\\)]{.math .notranslate .nohighlight}, column
[\\(j\\)]{.math .notranslate .nohighlight} of the matrix shows how much
influence person [\\(j\\)]{.math .notranslate .nohighlight} gives to
person [\\(i\\)]{.math .notranslate .nohighlight} with the following
definition:

::: {.math .notranslate .nohighlight}
\\\[\\begin{split}L\_{ij} = \\begin{cases}
\\frac{1}{d\_j\^{\\text{out}}} & \\text{if there is a link from node } j
\\text{ to node } i \\\\ 0 & \\text{otherwise}
\\end{cases}\\end{split}\\\]
:::

The Normalization [\\(\\frac{1}{d\_j\^{\\text{out}}}\\)]{.math
.notranslate .nohighlight} by the out-degree of node [\\(j\\)]{.math
.notranslate .nohighlight} can be interpreted as the normalized
distribution of influence across the network. That is to say, if person
[\\(j\\)]{.math .notranslate .nohighlight} follows a few people, then
their "vote of importance" is evenly split among those [\\(m\\)]{.math
.notranslate .nohighlight} people. So, [\\(L\_{ij} =
\\frac{1}{m}\\)]{.math .notranslate .nohighlight} for each connection.
...
To describe the dynamics of a random walker moving through the network,
the **PageRank Matrix :math:\`mathbf{P}\`** is introduced as below:

::: {.math .notranslate .nohighlight}
\\\[\\mathbf{P} = \\alpha \\mathbf{L} + (1 - \\alpha) \\frac{1}{N}
\\mathbf{E}\\\]
:::

The matrix models the walker's behavior by capturing the probabilities
of moving between nodes (with damping factor [\\(\\alpha\\)]{.math
.notranslate .nohighlight}), considering both the network's link
structure (via link matrix [\\(\\mathbf{L}\\)]{.math .notranslate
.nohighlight}) and the possibility of random jumps through the
teleportation matrix [\\(\\mathbf{E}\\)]{.math .notranslate
.nohighlight}, which ensures unbiased exploration of all nodes with a
probability of [\\(1/N\\)]{.math .notranslate .nohighlight}.
...
::: {#Simulating-Quantum-PageRank-by-CUDA-Q-dynamics .section}
Simulating Quantum PageRank by CUDA-Q dynamics[¶](#Simulating-Quantum-PageRank-by-CUDA-Q-dynamics "Permalink to this heading"){.headerlink}
-------------------------------------------------------------------------------------------------------------------------------------------

Instead of computing PageRank iteratively by [the power
method](https://en.wikipedia.org/wiki/PageRank#Power_method){.reference
.external}, we aim to introduce a [quantum version of
PageRank](https://www.nature.com/articles/srep00605){.reference
.external} that solves the ranking problem by leveraging the principles
of quantum mechanics, specifically [Quantum Stochastic Walk
(QSW)](https://journals.aps.org/pra/abstract/10.1103/PhysRevA.81.022323){.reference
.external}, which generalizes classical random walks by incorporating
both **coherent quantum evolution** and **incoherent stochastic
dynamics**, described by the **Lindblad master equation**:

::: {.math .notranslate .nohighlight}
\\\[\\frac{d\\rho}{dt} = -i(1-\\omega)\[H, \\rho\] + \\omega
\\sum\_{k\>0} \\gamma\_k \\left( L\_k \\rho L\_k\^\\dagger -
\\frac{1}{2} \\{L\_k\^\\dagger L\_k, \\rho\\} \\right)\\\]
:::
:::

::: {#Breakdown-of-Terms .section}
Breakdown of Terms[¶](#Breakdown-of-Terms "Permalink to this heading"){.headerlink}
-----------------------------------------------------------------------------------
...
-   [\\(H\\)]{.math .notranslate .nohighlight} is the Hamiltonian,
    governing coherent unitary evolution.

-   The factor [\\(1 - \\omega\\)]{.math .notranslate .nohighlight}
    determines the extent of quantum behavior in the system.

**Dissipative Evolution (incoherent):**

::: {.math .notranslate .nohighlight}
\\\[\\omega \\sum\_{k \> 0} \\gamma\_k \\left( L\_k \\rho
L\_k\^{\\dagger} - \\frac{1}{2} \\{ L\_k\^{\\dagger} L\_k, \\rho\\}
\\right)\\\]
:::

-   [\\(\\gamma\_k\\)]{.math .notranslate .nohighlight} are rates
    associated with each dissipative process.

-   [\\(L\_k\\)]{.math .notranslate .nohighlight} are Lindblad operators
    modeling incoherent jumps between states.

**Parameter :math:\`omega\` controls the interpolation:**

-   [\\(\\omega = 0\\)]{.math .notranslate .nohighlight} → Pure Quantum
    Walk (QW), where only unitary evolution occurs.

-   [\\(\\omega = 1\\)]{.math .notranslate .nohighlight} → Pure
    Classical Random Walk (CRW), where only stochastic transitions
    occur.

-   [\\(0 \< \\omega \< 1\\)]{.math .notranslate .nohighlight} → Quantum
    Stochastic Walk (QSW), a hybrid of both.

In brief, this framework naturally extends PageRank, where the
dissipators [\\(L\_k\\)]{.math .notranslate .nohighlight} encode
stochastic teleportation, similar to the random jumps in classical
PageRank. The **Quantum PageRank (QPR)** model based on quantum spatial
walks can reveal hidden structures for some networks, enhance ranking
sensitivity, and achieve comparable or faster convergence compared to
its classical counterpart.

Given the time-dependent nature of quantum stochastic walks, CUDA-Q
Dynamic leverages GPU acceleration and provides a suite of APIs for
efficiently solving time-evolved states. Let's get started:
...
The **Lindblad operators** can be defined for a quantum system
represented by a density matrix [\\(\\rho\\)]{.math .notranslate
.nohighlight}. They facilitate the incorporation of both quantum
coherence and incoherent processes into the dynamics.

For Quantum PageRank, the Lindblad operators shown below have been
demonstrated to converge to a stationary solution:

::: {.math .notranslate .nohighlight}
\\\[L(i,j) = \|i\\rangle \\langle j\| \\quad\\\]
:::

where the indices [\\(i\\)]{.math .notranslate .nohighlight} and
[\\(j\\)]{.math .notranslate .nohighlight} label the nodes in the
network as well as the Lindblad operators. This notation indicates that
the operator [\\(L(i,j)\\)]{.math .notranslate .nohighlight} describes a
**transition** from state [\\(\|j\\rangle\\)]{.math .notranslate
.nohighlight} to state [\\(\|i\\rangle\\)]{.math .notranslate
.nohighlight}.
...
Simply simulate the dynamic system via the `evolve`{.docutils .literal
.notranslate} API with `"dynamics"`{.docutils .literal .notranslate}
backend of CUDA-Q:
...
Note that the ranking results obtained by the Quantum PageRank algorithm
show smoother ranking distribution, which reduces the dominance of major
hubs, allowing less connected nodes to be fairly recognized. This
facilitates the identification of secondary hubs and reveals finer
structural details within the network, offering a more nuanced and
continuous scale of node importance for complex networks.

------------------------------------------------------------------------

Finally, let's visualize the ranking results, representing the influence
of each node, from the Quantum PageRank using the
`CUDA-Q Dynamic`{.docutils .literal .notranslate} simulation:
