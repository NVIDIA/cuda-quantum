...
-   [Examples](../../using/examples/examples.html){.reference .internal}
    ...
    -   [Dynamics
        ...
        .internal}
        -   [Cavity
            QED](../../using/examples/dynamics_examples.html#cavity-qed){.reference
            .internal}
        -   [Cross
            Resonance](../../using/examples/dynamics_examples.html#cross-resonance){.reference
            .internal}
        -   [Gate
            Calibration](../../using/examples/dynamics_examples.html#gate-calibration){.reference
            .internal}
        -   [Heisenberg
            Model](../../using/examples/dynamics_examples.html#heisenberg-model){.reference
            .internal}
        -   [Ion
            Trap](../../using/examples/dynamics_examples.html#ion-trap){.reference
            .internal}
        -   [Landau
            Zener](../../using/examples/dynamics_examples.html#landau-zener){.reference
            .internal}
        -   [Pulse](../../using/examples/dynamics_examples.html#pulse){.reference
            .internal}
        -   [Qubit
            Control](../../using/examples/dynamics_examples.html#qubit-control){.reference
            .internal}
        -   [Qubit
            Dynamics](../../using/examples/dynamics_examples.html#qubit-dynamics){.reference
            .internal}
        -   [Silicon Spin
            Qubit](../../using/examples/dynamics_examples.html#silicon-spin-qubit){.reference
            .internal}
        -   [Tensor
            Callback](../../using/examples/dynamics_examples.html#tensor-callback){.reference
            .internal}
        -   [Transmon
            Resonator](../../using/examples/dynamics_examples.html#transmon-resonator){.reference
            .internal}
        -   [Initial State (Multi-GPU
            Multi-Node)](../../using/examples/dynamics_examples.html#initial-state-multi-gpu-multi-node){.reference
            .internal}
        -   [Heisenberg Model (Multi-GPU
            Multi-Node)](../../using/examples/dynamics_examples.html#heisenberg-model-multi-gpu-multi-node){.reference
            .internal}
-   [Applications](../../using/applications.html){.reference .internal}
    ...
    -   [Multi-reference Quantum Krylov Algorithm - [\\(H\_2\\)]{.math
        .notranslate .nohighlight} Molecule](#){.current .reference
        .internal}
        -   [Setup](#Setup){.reference .internal}
        -   [Computing the matrix
            elements](#Computing-the-matrix-elements){.reference
            .internal}
...
-   Multi-reference Quantum Krylov Algorithm - [\\(H\_2\\)]{.math
    .notranslate .nohighlight} Molecule
-   
...
::: {#Multi-reference-Quantum-Krylov-Algorithm---H_2-Molecule .section}
Multi-reference Quantum Krylov Algorithm - [\\(H\_2\\)]{.math .notranslate .nohighlight} Molecule[¶](#Multi-reference-Quantum-Krylov-Algorithm---H_2-Molecule "Permalink to this heading"){.headerlink}
=======================================================================================================================================================================================================

The multireference selected quantum Krylov (MRSQK) algorithm is defined
in [this paper](https://arxiv.org/pdf/1911.05163){.reference .external}
and was developed as a low-cost alternative to quantum phase estimation.
This tutorial will demonstrate how this algorithm can be implemented in
CUDA-Q and accelerated using multiple GPUs. The [CUDA-Q Hadamard test
tutorial](https://nvidia.github.io/cuda-quantum/latest/applications/python/hadamard_test.html){.reference
.external} might provide helpful background information for
understanding this tutorial.

The algorithm works by preparing an initial state, and then defining
this state in a smaller subspace constructed with a basis that
corresponds to Trotter steps of the initial state. This subspace can be
diagonalized to produce an approximate energy for the system without
variational optimization of any parameters.

In the example below, the initial guess is the ground state of the
diagonalized Hamiltonian for demonstration purposes. In practice one
could use a number of heuristics to prepare the ground state such as
Hartree Fock or CISD. A very promising ground state preparation method
which can leverage quantum computers is the linear combination of
unitaries (LCU). LCU would allow for the state preparation to occur
completely on the quantum computer and avoid storing an inputting the
exponentially large state vector.

Regardless of the method used for state preparation, the procedure
begins by selecting a [\\(d\\)]{.math .notranslate
.nohighlight}-dimensional basis of reference states [\\({\\Phi\_0
\\cdots \\Phi\_d},\\)]{.math .notranslate .nohighlight} where each is a
linear combination of Slater determinants:

::: {.math .notranslate .nohighlight}
\\\[\\ket{\\Phi\_I} = \\sum\_{\\mu} d\_{\\mu I}\\ket{\\phi\_{\\mu}}.\\\]
:::

From this, a non-orthogonal Krylov Space [\\(\\mathcal{K} =
\\{\\psi\_{0} \\cdots \\psi\_{N}\\}\\)]{.math .notranslate .nohighlight}
is constructed by applying a family of [\\(s\\)]{.math .notranslate
.nohighlight} unitary operators on each of the [\\(d\\)]{.math
.notranslate .nohighlight} reference states resulting in [\\(d\*s =
N\\)]{.math .notranslate .nohighlight} elements in the Krylov space
where

::: {.math .notranslate .nohighlight}
\\\[\\ket{\\psi\_{\\alpha}} \\equiv \\ket{\\psi\_I\^{(n)}} =
\\hat{U}\_n\\ket{\\Phi\_I},\\\]
:::

Therefore, the general quantum state that we originally set out to
describe is

::: {.math .notranslate .nohighlight}
\\\[\\ket{\\Psi} = \\sum\_{\\alpha} c\_{\\alpha}\\ket{\\psi\_{\\alpha}}
= \\sum\_{I=0}\^d \\sum\_{n=0}\^s
c\_I\^{(n)}\\hat{U}\_n\\ket{\\Phi\_I}.\\\]
:::

The energy of this state can be obtained by solving the generalized
eigenvalue problem
...
::: {.math .notranslate .nohighlight}
\\\[S\_{\\alpha \\beta} = \\braket{\\psi\_{\\alpha}\|\\psi\_{\\beta}} =
\\braket{\\Phi\_I\|\\hat{U}\_m\^{\\dagger}\\hat{U}\_n\|\\Phi\_J}\\\]
:::
...
::: {.math .notranslate .nohighlight}
\\\[H\_{\\alpha \\beta} =
\\braket{\\psi\_{\\alpha}\|\\hat{H}\|\\psi\_{\\beta}} =
\\braket{\\Phi\_I\|\\hat{U}\_m\^{\\dagger}\\hat{H}\\hat{U}\_n\|\\Phi\_J}.\\\]
:::

The matrix elements for [\\(S\\)]{.math .notranslate .nohighlight} are
computed with the Hadamard test with a circuit shown below for the case
of the overlap matrix elements.

![Htest](../../_images/krylovcircuit.png)

The [\\(2\\sigma\_+\\)]{.math .notranslate .nohighlight} term refers to
measurement of the expectation value of this circuit with the
[\\(X+iY\\)]{.math .notranslate .nohighlight} operator.

The Hamiltonian matrix elements are computed with a circuit that
includes controlled application of the Hamiltonian. Once the
[\\(H\\)]{.math .notranslate .nohighlight} and [\\(S\\)]{.math
.notranslate .nohighlight} matrices are constructed, the diagonalization
is performed classically to produce an estimate for the ground state in
question.
...
The molecule is defined below and its Hamiltonian is extracted as a
matrix. The matrix is diagonalized to produce the ground state. The
corresponding state vector will be used as the initial state to ensure
good results for this demonstration.
...
The functions below take the original Hamiltonian defined above, and
strip it into a list of Pauli words and a list of coefficients which
will be uses later.
...
In this example, the unitary operators that build the Krylov subspace
are first-order Trotter operations at different time steps. The
performance here could potentially be improved by increasing the size of
the time step, using a higher order Trotter approximation, or using
other sorts of approximations. The CUDA-Q kernels below define the
unitary operations that construct the [\\(\\psi\\)]{.math .notranslate
.nohighlight} basis. Each receives the target qubits, the time step, and
...
The auxillary function below takes a Pauli word, and converts it to a
list of integers which informs applications of this word to a circuit
with the `apply_pauli`{.docutils .literal .notranslate} kernel.
...
Finally, the time step for the unitary operations that build the Krylov
space is defined as well as the dimension of the Krylov space.
...
::: {#Computing-the-matrix-elements .section}
Computing the matrix elements[¶](#Computing-the-matrix-elements "Permalink to this heading"){.headerlink}
---------------------------------------------------------------------------------------------------------

The cell below computes the overlap matrix. This can be done in serial
or in parallel, depending on the `multi_gpu`{.docutils .literal
.notranslate} specification. First, an operator is built to apply the
identity to the overlap matrix circuit when `apply_pauli`{.docutils
.literal .notranslate} is called. Next, the `wf_overlap`{.docutils
.literal .notranslate} array is constructed which will hold the matrix
elements.

Next, a pair of nested loops iterate over the time steps defined by the
dimension of the subspace. Each m,n combination corresponds to
computation of an off-diagonal matrix element of the overlap matrix
[\\(S\\)]{.math .notranslate .nohighlight} using the Hadamard test. This
is accomplished by calling the CUDA-Q `observe`{.docutils .literal
.notranslate} function with the X and Y operators, along with the time
steps, the components of the Hamiltonian matrix, and the initial state
vector `vec`{.docutils .literal .notranslate}.

The observe function broadcasts over the two provided operators
[\\(X\\)]{.math .notranslate .nohighlight} and [\\(Y\\)]{.math
.notranslate .nohighlight} and returns a list of results. The
`expectation`{.docutils .literal .notranslate} function returns the
expectation values which are summed and stored in the matrix.

The multi-gpu case completes the same steps, except the
`observe_async`{.docutils .literal .notranslate} command is used. This
allows for the [\\(X\\)]{.math .notranslate .nohighlight} and
[\\(Y\\)]{.math .notranslate .nohighlight} observables to be evaluated
at the same time on two different simulated QPUs. In this case, the
results are stored in lists corresponding to the real and imaginary
parts. These are then accessed later with the `get`{.docutils .literal
.notranslate} command to build [\\(S\\)]{.math .notranslate
.nohighlight}.
...
The Hamiltonian matrix elements are computed in the same way, except
this time the Hamiltonian is applied as part of the circuit. This is
accomplished with the extra for loop, which iterates through the terms
in the Hamiltonian, computing an expectation value for each one and then
summing the results to produce one matrix element.
...
::: {#Determining-the-ground-state-energy-of-the-subspace .section}
Determining the ground state energy of the subspace[¶](#Determining-the-ground-state-energy-of-the-subspace "Permalink to this heading"){.headerlink}
-----------------------------------------------------------------------------------------------------------------------------------------------------

The final step is to solve the generalized eigenvaulue problem with the
overlap and Hamiltonian matrices constructed using the quantum computer.
The procedure begins by diagonalizing [\\(S\\)]{.math .notranslate
.nohighlight} with the transform

::: {.math .notranslate .nohighlight}
\\\[S = U\\Sigma U\^{\\dagger}\\\]
:::

The eigenvectors [\\(v\\)]{.math .notranslate .nohighlight} and
eigenvalues [\\(s\\)]{.math .notranslate .nohighlight} are used to
construct a new matrix [\\(X\':\\)]{.math .notranslate .nohighlight}

::: {.math .notranslate .nohighlight}
\\\[X\' = S \^{\\frac{-1}{2}} = \\sum\_k v\_{ki} \\frac{1}{\\sqrt{s\_k}}
v\_{kj}.\\\]
:::

The matrix [\\(X\'\\)]{.math .notranslate .nohighlight} diagonalizes
[\\(H:\\)]{.math .notranslate .nohighlight}

::: {.math .notranslate .nohighlight}
\\\[X\'\^{\\dagger}HX\' = ES\^{\\frac{1}{2}}C.\\\]
:::

Using the eigenvectors of [\\(H\'\\)]{.math .notranslate .nohighlight},
([\\(\^{\\frac{1}{2}}C\\)]{.math .notranslate .nohighlight}), the
original eigenvectors to the problem can be found by left multiplying by
[\\(S\^{\\frac{-1}{2}}C.\\)]{.math .notranslate .nohighlight}
