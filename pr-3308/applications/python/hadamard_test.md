...
-   [Applications](../../using/applications.html){.reference .internal}
    ...
    -   [Using the Hadamard Test to Determine Quantum Krylov Subspace
        Decomposition Matrix Elements](#){.current .reference .internal}
        -   [Numerical result as a
            reference:](#Numerical-result-as-a-reference:){.reference
            .internal}
        -   [Using `Sample`{.docutils .literal .notranslate} to perform
            the Hadamard
            test](#Using-Sample-to-perform-the-Hadamard-test){.reference
            .internal}
        -   [Multi-GPU evaluation of QKSD matrix elements using the
            Hadamard
            Test](#Multi-GPU-evaluation-of-QKSD-matrix-elements-using-the-Hadamard-Test){.reference
            .internal}
            -   [Classically Diagonalize the Subspace
                Matrix](#Classically-Diagonalize-the-Subspace-Matrix){.reference
                .internal}
...
-   Using the Hadamard Test to Determine Quantum Krylov Subspace
    Decomposition Matrix Elements
-   

::: {.rst-breadcrumbs-buttons role="navigation" aria-label="Sequential page navigation"}
[[]{.fa .fa-arrow-circle-left aria-hidden="true"}
Previous](hybrid_quantum_neural_networks.html "Hybrid Quantum Neural Networks"){.btn
.btn-neutral .float-left} [Next []{.fa .fa-arrow-circle-right
aria-hidden="true"}](logical_aim_sqale.html "Anderson Impurity Model ground state solver on Infleqtion’s Sqale"){.btn
...
::: {#Using-the-Hadamard-Test-to-Determine-Quantum-Krylov-Subspace-Decomposition-Matrix-Elements .section}
Using the Hadamard Test to Determine Quantum Krylov Subspace Decomposition Matrix Elements[¶](#Using-the-Hadamard-Test-to-Determine-Quantum-Krylov-Subspace-Decomposition-Matrix-Elements "Permalink to this heading"){.headerlink}
===================================================================================================================================================================================================================================

The Hadamard test is a quantum algorithm for estimating expectation
values and is a useful subroutine for a number of quantum applications
ranging from estimation of molecular ground state energies to quantum
semidefinite programming. This tutorial will briefly introduce the
Hadamard test, demonstrate how it can be implemented in CUDA-Q, and then
parallelized for a Quantum Krylov Subspace Diagonalization application.

The Hadamard test is performed using a register with an ancilla qubit in
the [\\(\\ket{0}\\)]{.math .notranslate .nohighlight} state and a
prepared quantum state [\\(\\ket{\\psi}\\)]{.math .notranslate
.nohighlight}, the following circuit can be used to extract the
expectation value from measurement of the ancilla.

![Htest](../../_images/htest.png)

The key insight is that

::: {.math .notranslate .nohighlight}
\\\[P(0) = \\frac{1}{2} \\left\[ I + Re \\bra{\\psi} O \\ket{\\phi}
\\right\]\\\]
:::
...
::: {.math .notranslate .nohighlight}
\\\[P(1) = \\frac{1}{2} \\left\[ I - Re \\bra{\\psi} O \\ket{\\phi}
\\right\]\\\]
:::
...
More details and a short derivation can be found
[here](https://en.wikipedia.org/wiki/Hadamard_test){.reference
.external}.

What if you want to perform the Hadamard test to compute an expectation
value like [\\(\\bra{\\psi} O \\ket{\\phi}\\)]{.math .notranslate
.nohighlight}, where [\\(\\ket{\\psi}\\)]{.math .notranslate
.nohighlight} and [\\(\\ket{\\phi}\\)]{.math .notranslate .nohighlight}
are different states and [\\(O\\)]{.math .notranslate .nohighlight} is a
Pauli Operator? This is a common subroutine for the QKSD, where matrix
elements are determined by computing expectation values between
different states.

Defining [\\(O\\)]{.math .notranslate .nohighlight} as
...
::: {.math .notranslate .nohighlight}
\\\[\\ket{\\psi} = U\_{\\psi}\\ket{0} \\qquad \\ket{\\phi} =
U\_{\\phi}\\ket{0},\\\]
:::

we can combine the state preparation steps into the operator resulting
in

::: {.math .notranslate .nohighlight}
\\\[\\bra{\\psi}O\\ket{\\phi} = \\bra{0}U\_\\psi\^\\dagger O
U\_\\phi\\ket{0},\\\]
:::

which corresponds to the following circuit.
![Htest2](../../_images/htestfactored.png)

By preparing this circuit, and repeatedly measuring the ancilla qubit,
we estimate the expectation value as
...
::: {#Numerical-result-as-a-reference: .section}
Numerical result as a reference:[¶](#Numerical-result-as-a-reference: "Permalink to this heading"){.headerlink}
---------------------------------------------------------------------------------------------------------------

Before performing the Hadamard test, let's determine the exact
expectation value by performing the matrix multiplications explicitly.
The code below builds two CUDA-Q kernels corresponding to
[\\(\\ket{\\psi} = \\frac{1}{\\sqrt{2}}\\begin{pmatrix}1 \\\\ 0 \\\\ 1
\\\\ 0\\end{pmatrix}\\)]{.math .notranslate .nohighlight} and
[\\(\\ket{\\phi} = \\begin{pmatrix}0 \\\\ 1 \\\\ 0 \\\\
0\\end{pmatrix}.\\)]{.math .notranslate .nohighlight}
...
The state vectors can be accessed using the `get_state`{.docutils
.literal .notranslate} command and printed as numpy arrays:
...
The Hamiltonian operator ([\\(O\\)]{.math .notranslate .nohighlight} in
the derivation above) is defined as a CUDA-Q spin operator and converted
to a matrix with `to_matrix`{.docutils .literal .notranslate}. The
following line of code performs the explicit matrix multiplications to
produce the exact expectation value.
...
::: {#Using-Sample-to-perform-the-Hadamard-test .section}
Using `Sample`{.docutils .literal .notranslate} to perform the Hadamard test[¶](#Using-Sample-to-perform-the-Hadamard-test "Permalink to this heading"){.headerlink}
--------------------------------------------------------------------------------------------------------------------------------------------------------------------

Three CUDA-Q kernels are constructed below corresponding to
[\\(\\ket{\\psi}\\)]{.math .notranslate .nohighlight},
[\\(\\ket{\\phi}\\)]{.math .notranslate .nohighlight}, and the
Hamiltonian. A fourth kernel constructs the Hadamard test circuit and
completes with a measurement of the ancilla qubit in the computational
basis.
...
The CUDA-Q `sample`{.docutils .literal .notranslate} method computes
100000 sample ancilla measurements, and from them we can estimate the
expectation value. The standard error is provided as well. Try
increasing the sample size and note the convergence of the expectation
value and the standard error towards the numerical result.
...
::: {#Multi-GPU-evaluation-of-QKSD-matrix-elements-using-the-Hadamard-Test .section}
Multi-GPU evaluation of QKSD matrix elements using the Hadamard Test[¶](#Multi-GPU-evaluation-of-QKSD-matrix-elements-using-the-Hadamard-Test "Permalink to this heading"){.headerlink}
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

This example is small, but a more practical application of the Hadamard
test such as QKSD will require much larger circuits. The QKSD method
works by reducing the exponential [\\(2\^N\\)]{.math .notranslate
.nohighlight} Hilbert space into an exponentially smaller subspace using
a set of non-orthogonal states which are easy to prepare on a quantum
computer. The Hadamard test is used to compute the matrix elements of
this smaller subspace which is then diagonalized using a classical
method to produce the eigenvalues. [This
paper](https://www.osti.gov/servlets/purl/1962060){.reference .external}
described the method in more detail and is the source of the figure
below.

![Htest3](../../_images/QKSD.png)

This method can be easily parallelized, and multiple QPUs, if available,
could compute the matrix elements asynchronously. The CUDA-Q
`mqpu`{.docutils .literal .notranslate} backend allows you to simulate a
computation across multiple simulated QPUs. The code below demonstrates
how.

First, the Hadamard test circuit is defined, but this time the
[\\(\\ket{\\psi}\\)]{.math .notranslate .nohighlight} and
[\\(\\ket{\\phi}\\)]{.math .notranslate .nohighlight} states contain
parameterized rotations so that multiple states can be quickly
generated, for the sake of example.
...
Next, the `nvidia-mqpu`{.docutils .literal .notranslate} backend is
specified and the number of GPUs available is determined.
...
The `sample_async`{.docutils .literal .notranslate} command is then used
to distribute the Hadamard test computations across multiple simulated
QPUs. The results are saved in a list and accessed below.
...
The four matrix elements are shown below and can be classically
processed to produce the eigenvalues.
...
::: {#Classically-Diagonalize-the-Subspace-Matrix .section}
### Classically Diagonalize the Subspace Matrix[¶](#Classically-Diagonalize-the-Subspace-Matrix "Permalink to this heading"){.headerlink}

For a problem of this size, numpy can be used to diagonalize the
subspace and produce the eigenvalues and eigenvectors in the basis of
