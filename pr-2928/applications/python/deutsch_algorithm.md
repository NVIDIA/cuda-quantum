...
-   [Applications](../../using/applications.html){.reference .internal}
    ...
    -   [Cost Minimization](cost_minimization.html){.reference
        .internal}
    -   [Deutsch's Algorithm](#){.current .reference .internal}
        -   [XOR [\\(\\oplus\\)]{.math .notranslate
            .nohighlight}](#XOR-\oplus){.reference .internal}
        -   [Quantum oracles](#Quantum-oracles){.reference .internal}
        -   [Phase oracle](#Phase-oracle){.reference .internal}
        -   [Quantum parallelism](#Quantum-parallelism){.reference
            .internal}
        -   [Deutsch's Algorithm:](#Deutsch's-Algorithm:){.reference
            .internal}
...
Previous](cost_minimization.html "Cost Minimization"){.btn .btn-neutral
.float-left} [Next []{.fa .fa-arrow-circle-right
aria-hidden="true"}](divisive_clustering_coresets.html "Divisive Clustering With Coresets Using CUDA-Q"){.btn
...
::: {#Deutsch's-Algorithm .section}
Deutsch's Algorithm[¶](#Deutsch's-Algorithm "Permalink to this heading"){.headerlink}
=====================================================================================

Deutsch's Algorithm is a concise demonstration of the differences in
computational complexity between classical and quantum algorithms for
certain problems. For Desutch's algorithm, we begin with a function
which takes in a bit and outputs a bit. This can be represented as
[\\(f: \\{0,1\\} \\longrightarrow \\{0,1\\}\\)]{.math .notranslate
.nohighlight}. The function [\\(f\\)]{.math .notranslate .nohighlight}
has the property that it either constant or balanced. The goal of
Deutsch's Algorithm is to determine whether our given function is
constant or whether it is balanced.

A constant function is "A balanced function is a function such that the
outputs are the same regardless of the inputs, i.e., if [\\(f(0) =
0\\)]{.math .notranslate .nohighlight} then [\\(f(1) = 1\\)]{.math
.notranslate .nohighlight} or if [\\(f(0) = 1\\)]{.math .notranslate
.nohighlight} then [\\(f(1) = 0\\)]{.math .notranslate .nohighlight}. ",
the outputs are the same regardless of the inputs, i.e., in the case of
[\\(f: \\{0,1\\} \\longrightarrow \\{0,1\\}\\)]{.math .notranslate
.nohighlight}, there are are two ways in which this can occur: [\\(f(0)
= f(1) = 0\\)]{.math .notranslate .nohighlight} or [\\(f(0) = f(1) =
1\\)]{.math .notranslate .nohighlight}.

A balanced function is defined such that the ouputs are balanced across
their possibilities, i.e., if [\\(f(0) = 0\\)]{.math .notranslate
.nohighlight} then [\\(f(1) = 1\\)]{.math .notranslate .nohighlight} or
if [\\(f(0) = 1\\)]{.math .notranslate .nohighlight} then [\\(f(1) =
0\\)]{.math .notranslate .nohighlight}.

Classically, if we are given a function [\\(f: \\{0,1\\}
\\longrightarrow \\{0,1\\}\\)]{.math .notranslate .nohighlight}, we can
determine if it is constant or balanced by evaluating the function at
[\\(0\\)]{.math .notranslate .nohighlight} and at [\\(1\\)]{.math
.notranslate .nohighlight}. This is carried out in the code below:
...
Let us now classically find out if the function we defined above is
...
If you step through the `if`{.docutils .literal .notranslate} statements
above, you may notice that we require 2 calls to the function to
determine its property. That is, we have to query [\\(f\\)]{.math
.notranslate .nohighlight} twice.

The claim is that Deutsch's Algorithm can determine if a given function
is constant or balanced with just 1 function evalulation, demonstrating
quantum advantage.

Below we first outline Deutsch's Algorithm and work through the math to
verify that it does as promised. Then, we provide the implementation in
CUDA-Q.

::: {#XOR-\\oplus .section}
XOR [\\(\\oplus\\)]{.math .notranslate .nohighlight}[¶](#XOR-\oplus "Permalink to this heading"){.headerlink}
-------------------------------------------------------------------------------------------------------------

You may have seen the [\\(\\oplus\\)]{.math .notranslate .nohighlight}
symbol floating around. This is the exclusive OR or the XOR operation.
It follows the rules of addition modulo 2. For example,

1.  [\\(3\\oplus5 = (3+5)\_{\\%2} = 8\_{\\%2} = 0\\)]{.math .notranslate
    .nohighlight}

2.  [\\(5\\oplus4 = (5+4)\_{\\%2} = 9\_{\\%2} = 1\\)]{.math .notranslate
    .nohighlight}

A quick way to perform the [\\(\\oplus\\)]{.math .notranslate
.nohighlight} calculation is if the result of the addition is an even
number, the result is 0 otherwise 1.
:::
...
![fffc4baf45d44a619acf32182b5a1976](../../_images/oracle.png){.no-scaled-link}

Suppose we have [\\(f(x): \\{0,1\\} \\longrightarrow \\{0,1\\}\\)]{.math
.notranslate .nohighlight}. We can compute this function on a quantum
computer using oracles which we treat as black box functions that yield
the output with an appropriate sequence of logical gates.

Above you see an oracle represented as [\\(U\_f\\)]{.math .notranslate
.nohighlight} which allows us to transform the state
[\\(\\ket{x}\\ket{y}\\)]{.math .notranslate .nohighlight} into:

::: {.math .notranslate .nohighlight}
\\\[\\begin{aligned} U\_f\\ket{x}\\ket{y} = \\ket{x}\\ket{y \\oplus
f(x)} \\end{aligned}\\\]
:::

If [\\(y = 0\\)]{.math .notranslate .nohighlight}, then
[\\(U\_f\\ket{x}\\ket{y} = U\_f\\ket{x}\\ket{0} = \\ket{x}\\ket{0
\\oplus f(x)} = \\ket{x}\\ket{f(x)}\\)]{.math .notranslate
.nohighlight}, since [\\(f(x)\\)]{.math .notranslate .nohighlight} can
either be [\\(0\\)]{.math .notranslate .nohighlight} or [\\(1\\)]{.math
.notranslate .nohighlight} and [\\(0 \\oplus 0 = 0\\)]{.math
.notranslate .nohighlight} and [\\(0 \\oplus 1 = 1\\)]{.math
.notranslate .nohighlight}.

This is remarkable because by setting [\\(\\ket{y} = \\ket{0}\\)]{.math
.notranslate .nohighlight}, we can extract the value of
[\\(f(x)\\)]{.math .notranslate .nohighlight} by measuring the value of
the second qubit.
:::

::: {#Phase-oracle .section}
Phase oracle[¶](#Phase-oracle "Permalink to this heading"){.headerlink}
-----------------------------------------------------------------------

Since [\\(f(x)\\)]{.math .notranslate .nohighlight} can be either
[\\(0\\)]{.math .notranslate .nohighlight} or [\\(1\\)]{.math
.notranslate .nohighlight}, we have that [\\(0 \\oplus f(x) = 0 \\oplus
0 = 0 = f(x)\\)]{.math .notranslate .nohighlight} or [\\(0 \\oplus f(x)
= 0 \\oplus 1 = 1 = f(x)\\)]{.math .notranslate .nohighlight}.

Similarly, [\\(1 \\oplus f(x) = 1 \\oplus 0 = 1 =
\\overline{f(x)}\\)]{.math .notranslate .nohighlight} or [\\(1 \\oplus
f(x) = 1 \\oplus 1 = 0 = \\overline{f(x)}\\)]{.math .notranslate
.nohighlight} where the bar on top of [\\(f(x)\\)]{.math .notranslate
.nohighlight} denotes 'not [\\(f(x)\\)]{.math .notranslate
.nohighlight}'.

Let us now prove a result which we shall use later:

::: {.math .notranslate .nohighlight}
\\\[\\begin{split}\\begin{aligned} U\_f\\ket{x}\\ket{-} &= U\_f \\ket{x}
\\tfrac{1}{\\sqrt{2}}(\\ket{0} - \\ket{1}) \\\\ &= \\tfrac{1}{\\sqrt{2}}
(U\_f\\ket{x}\\ket{0} - U\_f\\ket{x}\\ket{1}) \\\\ &=
\\tfrac{1}{\\sqrt{2}} (\\ket{x}\\ket{0 \\oplus f(x)} - \\ket{x}\\ket{1
\\oplus f(x)}) \\\\ &= \\tfrac{1}{\\sqrt{2}} (\\ket{x}\\ket{f(x)} -
\\ket{x}\\ket{\\overline{f(x)}}) \\\\ &= \\begin{cases}
\\tfrac{1}{\\sqrt{2}} (\\ket{x}\\ket{0} - \\ket{x}\\ket{1}) =
\\ket{x}\\ket{-} & f(x) = 0 \\\\ \\tfrac{1}{\\sqrt{2}} (\\ket{x}\\ket{1}
- \\ket{x}\\ket{0}) = -\\ket{x}\\ket{-} & f(x) = 1 \\\\ \\end{cases}
\\\\ &= (-1)\^{f(x)}\\ket{x}\\ket{-} \\end{aligned}\\end{split}\\\]
:::

In summary, [\\(U\_f\\ket{x}\\ket{-} =
(-1)\^{f(x)}\\ket{x}\\ket{-}\\)]{.math .notranslate .nohighlight} where
the [\\(\\ket{-}\\)]{.math .notranslate .nohighlight} qubit was left
unchanged and a phase was applied to [\\(\\ket{x}\\)]{.math .notranslate
.nohighlight}.
:::

::: {#Quantum-parallelism .section}
Quantum parallelism[¶](#Quantum-parallelism "Permalink to this heading"){.headerlink}
-------------------------------------------------------------------------------------

Consider:

::: {.math .notranslate .nohighlight}
\\\[\\begin{split}\\begin{aligned} U\_f\\ket{+}\\ket{0} &= U\_f
\\tfrac{1}{\\sqrt{2}}(\\ket{0}+\\ket{1}) \\ket{0} \\\\ &= U\_f
\\tfrac{1}{\\sqrt{2}}(\\ket{0}\\ket{0}+\\ket{1}\\ket{0}) \\\\ &=
\\tfrac{1}{\\sqrt{2}}(U\_f \\ket{0}\\ket{0} + U\_f \\ket{1}\\ket{0})
\\\\ &= \\tfrac{1}{\\sqrt{2}}(U\_f \\ket{0}\\ket{0} + U\_f
\\ket{1}\\ket{0}) \\\\ &= \\tfrac{1}{\\sqrt{2}}(\\ket{0}\\ket{0 \\oplus
f(0)} + \\ket{1}\\ket{0 \\oplus f(1)}) \\\\ &=
\\tfrac{1}{\\sqrt{2}}(\\ket{0}\\ket{f(0)} + \\ket{1}\\ket{f(1)}) \\\\
\\end{aligned}\\end{split}\\\]
:::

We have calculated information about both [\\(f(0)\\)]{.math
.notranslate .nohighlight} and [\\(f(1)\\)]{.math .notranslate
.nohighlight} simultaneously. Quantum mechanics allows for parallelism
by exploiting the ability of superposition states.
:::
...
Our aim is to find out if [\\(f: \\{0,1\\} \\longrightarrow
\\{0,1\\}\\)]{.math .notranslate .nohighlight} is a constant or a
balanced function? If constant, [\\(f(0) = f(1)\\)]{.math .notranslate
.nohighlight}, and if balanced, [\\(f(0) \\neq f(1)\\)]{.math
.notranslate .nohighlight}.

We step through the circuit diagram below and follow the math after the
application of each gate.

![1bc76004777044d4b3e3179aae000426](../../_images/deutsch.png){.no-scaled-link}
...
::: {.math .notranslate .nohighlight}
\\\[\\ket{\\psi\_1} = H\_1H\_0\\ket{\\psi\_0} = H\_1H\_0\\ket{01} =
\\ket{+-} = \\frac{1}{\\sqrt{2}} (\\ket{0}+\\ket{1})\\ket{-}
\\tag{2}\\\]
:::

::: {.math .notranslate .nohighlight}
\\\[\\ket{\\psi\_2} = U\_f\\ket{\\psi\_1} = \\frac{1}{\\sqrt{2}}
(U\_f\\ket{0}\\ket{-} + U\_f\\ket{1}\\ket{-}) \\tag{3}\\\]
:::

using [\\(U\_f\\ket{x}\\ket{-} = (-1)\^{f(x)}\\ket{x}\\ket{-}\\)]{.math
.notranslate .nohighlight} which we proved in the phase oracle section
above:

::: {.math .notranslate .nohighlight}
\\\[\\begin{split}\\begin{aligned} \\ket{\\psi\_2} =
\\frac{1}{\\sqrt{2}} ((-1)\^{f(0)}\\ket{0}\\ket{-} +
(-1)\^{f(1)}\\ket{1}\\ket{-}) \\\\ = \\frac{1}{\\sqrt{2}}
((-1)\^{f(0)}\\ket{0} + (-1)\^{f(1)}\\ket{1}) \\ket{-} \\\\
\\end{aligned}\\end{split}\\\]
:::

We now drop the second, qubit in the [\\(\\ket{-}\\)]{.math .notranslate
.nohighlight} state from our derivations as the figure above has no
further gate operations acting on it. Remember our aim is to find out if
[\\(f(x)\\)]{.math .notranslate .nohighlight} is a constant or a
balanced function. We have the following options:

::: {.math .notranslate .nohighlight}
\\\[\\begin{split}\\begin{aligned} \\ket{\\psi\_{2}}= \\begin{cases}
\\tfrac{1}{\\sqrt{2}} (\\ket{0} + \\ket{1}) & f(0) = 0, f(1) = 0, f(x)
\\text{ is constant} \\\\ -\\tfrac{1}{\\sqrt{2}} (\\ket{0} + \\ket{1}) &
f(0) = 1, f(1) = 1, f(x) \\text{ is constant} \\\\ \\tfrac{1}{\\sqrt{2}}
(\\ket{0} - \\ket{1}) & f(0) = 0, f(1) = 1, f(x) \\text{ is balanced}
\\\\ -\\tfrac{1}{\\sqrt{2}} (\\ket{0} - \\ket{1}) & f(0) = 1, f(1) = 0,
f(x) \\text{ is balanced} \\\\ \\end{cases}
\\end{aligned}\\end{split}\\\]
:::

::: {.math .notranslate .nohighlight}
\\\[\\begin{split}\\begin{aligned} \\ket{\\psi\_{2}} = \\begin{cases}
\\pm \\tfrac{1}{\\sqrt{2}} (\\ket{0} + \\ket{1}) = \\pm \\ket{+} & f(x)
\\text{ is constant} \\\\ \\pm \\tfrac{1}{\\sqrt{2}} (\\ket{0} -
\\ket{1})= \\pm \\ket{-} & f(x) \\text{ is balanced} \\\\ \\end{cases}
\\end{aligned}\\end{split}\\\]
:::

::: {.math .notranslate .nohighlight}
\\\[\\begin{split}\\begin{aligned} \\ket{\\psi\_{3}}= H\\ket{\\psi\_{2}}
\\begin{cases} \\pm H\\ket{+} = \\pm \\ket{0} & f(x) \\text{ is
constant} \\\\ \\pm H\\ket{-} = \\pm \\ket{1} & f(x) \\text{ is
balanced} \\\\ \\end{cases} \\end{aligned}\\end{split}\\\]
:::

We can now measure the first qubit to yield either a [\\(0\\)]{.math
.notranslate .nohighlight} or a [\\(1\\)]{.math .notranslate
.nohighlight} to determine if [\\(f(x)\\)]{.math .notranslate
.nohighlight} is constant or balanced.

Deutsch's algorithm may not be practically useful but does demonstrate
quantum advantage since it uses one function call to determine the
property of [\\(f\\)]{.math .notranslate .nohighlight} in comparison to
two for classical methods.
...
This algorithm can be generalized to determine if a [\\(n\\)]{.math
.notranslate .nohighlight}-bit function [\\(f:{0,1}\^n\\longrightarrow
{0,1}\\)]{.math .notranslate .nohighlight} is constant or a balanced
with only [\\(\\frac{n}{2}\\)]{.math .notranslate .nohighlight} function
evaluations, for [\\(n\\)]{.math .notranslate .nohighlight} even. A
function if balanced if half of the inputs map to [\\(0\\)]{.math
.notranslate .nohighlight} and half map to [\\(1\\)]{.math .notranslate
.nohighlight}.

Here we must assume that the function that we are given is either
constant or balanced since there are [\\(n\\)]{.math .notranslate
.nohighlight}-bit functions that are neither constant, nor balanced. For
instance the [\\(2\\)]{.math .notranslate .nohighlight}-bit function
[\\(f(b\_0,b\_1) = \\max(b\_0,b\_1)\\)]{.math .notranslate .nohighlight}
is neither balanced, nor constant.

A hint on how you might approach this problem is to first solve the
problem for [\\(n=2\\)]{.math .notranslate .nohighlight} and see if you
can then use that approach to handle [\\(n\\)]{.math .notranslate
.nohighlight}-bit functions for larger values of [\\(n\\)]{.math
.notranslate .nohighlight}.
:::
:::
:::
