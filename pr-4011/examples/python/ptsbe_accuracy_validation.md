...
-   [Examples](../../using/examples/examples.html){.reference .internal}
    ...
    -   [PTSBE End-to-End
        ...
        -   [6. Two API
            ...
            .internal}
    -   [PTSBE Accuracy Validation](#){.current .reference .internal}
    -   [Constructing
...
Previous](ptsbe_end_to_end_workflow.html "PTSBE end-to-end workflow"){.btn
...
::: {#PTSBE-accuracy-validation .section}
# PTSBE accuracy validation[¶](#PTSBE-accuracy-validation "Permalink to this heading"){.headerlink}

This example checks that **PTSBE** (Pre-Trajectory Sampling with Batch
Execution) produces a measurement distribution that aligns closely with
**standard** (density-matrix) sampling on the same noisy circuit.

We run the **same circuit** with the **same noise model** and **same
number of shots** in two ways: 1. **Standard sample** --- full
density-matrix simulation with noise applied at each gate. 2. **PTSBE
sample** --- trajectory sampling: pre-sample noise realizations and run
batches of noiseless circuits.

We compare the two outcome distributions using the **Hellinger
fidelity** [\\(F\\)]{.math .notranslate .nohighlight}:

::: {.math .notranslate .nohighlight}
\\\[F = \\sum_x \\sqrt{p(x) \\, q(x)}\\\]
:::

where [\\(p\\)]{.math .notranslate .nohighlight} and [\\(q\\)]{.math
.notranslate .nohighlight} are the empirical probabilities from each
method. [\\(F \\in \[0,1\]\\)]{.math .notranslate .nohighlight} with
[\\(F=1\\)]{.math .notranslate .nohighlight} meaning identical
distributions. The **Hellinger distance** is [\\(H = \\sqrt{1 -
F}\\)]{.math .notranslate .nohighlight}. (See [Hellinger
distance](https://en.wikipedia.org/wiki/Hellinger_distance){.reference
.external} for the definitions and references.)

With a **large number of shots**, the two methods should agree to a high
degree (fidelity close to 1).
...
Use a **6-qubit circuit** with three layers of Hadamards and
controlled-X (CNOT) gates. For **single-qubit** gates (H) we use
**DepolarizationChannel** with a single qubit in the operand list. For
**CNOTs** we pass **qubit pairs** to the noise model and use
**Depolarization2** (two-qubit depolarization). We use
**ProbabilisticSamplingStrategy** (with a seed for reproducibility). A
moderate number of **shots** (e.g. 50k--1M) gives a meaningful fidelity
check across many bitstrings.
...
Run **standard** sampling (density-matrix with noise) and **PTSBE**
sampling with the same shot count.
...
Compute the **Hellinger fidelity** between the two empirical
distributions and report the result.
