...
that the gate we need is the controlled [\\(R\_k\\)]{.math .notranslate
.nohighlight} rotation where the [\\(R\_k\\)]{.math .notranslate
.nohighlight} gate is denoted by:

::: {.math .notranslate .nohighlight}
\\\[\\begin{split}\\begin{equation} R\_k = \\begin{bmatrix} 1 & 0 \\\\ 0
...
![qft](../../_images/qft.png){.no-scaled-link}
...
`quantum_fourier_transform2`{.docutils .literal .notranslate}. Pay
attention to how the kernel definition differs from the example above.
...
`adjoint`{.docutils .literal .notranslate} operation to create the
