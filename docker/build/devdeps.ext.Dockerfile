...
# Install cupy version 13.6.0
RUN apt-get update && apt-get install -y --no-install-recommends \
        python3 python3-pip && \
    apt-get autoremove -y && apt-get clean && rm -rf /var/lib/apt/lists/* && \
    python3 -m pip install --break-system-packages cupy-cuda$(echo $CUDA_VERSION | cut -d . -f1)x==13.6.0 cuquantum-cu$(echo $CUDA_VERSION | cut -d . -f1)==25.09 && \
