...
# This file builds a development environment that contains the necessary development 
# dependencies for building and testing CUDA Quantum, as well as additional dependencies for
# GPU components and backends. This image include a full CUDA and OpenMPI installation.
#
# Usage:
# Must be built from the repo root with:
#   docker build -t ghcr.io/nvidia/cuda-quantum-devdeps:${toolchain}-ext-latest -f docker/build/extdevenv.Dockerfile --build-arg toolchain=$toolchain .
#
# The variable $toolchain indicates which compiler toolchain to configure the environment with. 
# This toolchain is used to build the LLVM libraries that CUDA Quantum depends on, and needs to
# be used to build CUDA Quantum itself.Currently, llvm (default), clang15, and gcc11 are supported. 
...
FROM ghcr.io/nvidia/cuda-quantum-devdeps:${toolchain}-latest
SHELL ["/bin/bash", "-c"]
...
# Install tools for additional file and repo checks enforced as part of CI.
