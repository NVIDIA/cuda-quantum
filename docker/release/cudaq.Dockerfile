...
RUN mkdir /nvqc_scripts
ADD scripts/nvqc_proxy.py /nvqc_scripts

# NVQC Launch script: launch cudaq-qpud (nvcf mode) with MPI ranks == Number of NVIDIA GPUs
# IMPORTANT: 
...
RUN echo 'cat /opt/nvidia/cudaq/build_info.txt;' \
    ...
    'if [[ "$NUM_GPUS" == "$(nvidia-smi --list-gpus | wc -l)" ]]; then ' \
      ...
      'done; ' \
     'else echo "Invalid Deployment: Number of GPUs does not match the hardware" && exit 1; fi' > /nvqc_scripts/launch.sh
...
CMD ["/bin/bash", "-c", "if [ -n \"$NVQC\" ]; then bash -l /nvqc_scripts/launch.sh; else bash -l; fi"]
