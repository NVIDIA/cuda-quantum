...
The `spin_op`{.code .docutils .literal .notranslate} type provides an
abstraction for a general tensor product of Pauli spin operators, and
their sums.

Spin operators are constructed using the `spin.z()`{.code .docutils
.literal .notranslate}, `spin.y()`{.code .docutils .literal
.notranslate}, `spin.x()`{.code .docutils .literal .notranslate}, and
`spin.i()`{.code .docutils .literal .notranslate} functions,
corresponding to the [\\(Z\\)]{.math .notranslate .nohighlight},
[\\(Y\\)]{.math .notranslate .nohighlight}, [\\(X\\)]{.math .notranslate
.nohighlight}, and [\\(I\\)]{.math .notranslate .nohighlight} Pauli
operators. For example, `spin.z(0)`{.code .docutils .literal
.notranslate} corresponds to a Pauli Z operation acting on qubit 0. The
example below demonstrates how to construct the following operator 2
[\\(XYX\\)]{.math .notranslate .nohighlight} - 3 [\\(ZZY\\)]{.math
.notranslate .nohighlight}.
...
The `pauli_word`{.code .docutils .literal .notranslate} type specifies a
string of Pauli operations (e.g. `XYXZ`{.code .docutils .literal
.notranslate}) and is convenient for applying operations based on
exponentiated Pauli words. The code below demonstrates how a list of
Pauli words, along with their coefficients, are provided as kernel
inputs and converted into operators by the `exp_pauli`{.code .docutils
