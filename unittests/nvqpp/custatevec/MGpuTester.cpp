...
struct test_resizing {
  void operator()() __qpu__ {
    // Start with 26 qubits and grow to 28 below.
    cudaq::qvector q(26);
    cudaq::x(q);
    auto result = mz(q);
...
CUDAQ_TEST(MGPUTester, checkResizing) {
  auto counts = cudaq::sample(10, test_resizing{});
  EXPECT_EQ(1, counts.size());
  EXPECT_TRUE(counts.begin()->first == "1111111111111111111111111100");
  counts.dump();
}

CUDAQ_TEST(MGPUTester, checkNoise) {
  cudaq::set_random_seed(13);
  constexpr int numQubits = 20;
  // 2-qubit noise
...
CUDAQ_TEST(MGPUStateReuseTester, checkStateReuse) {
  cudaq::set_random_seed(13);
...
CUDAQ_TEST(MGPUStateReuseTester, checkNoisyCustomStateReuse) {
...
CUDAQ_TEST(MGPUStateReuseTester, checkNoisyCustomStateMultiTermObserve) {
  constexpr std::size_t numQubits = 6;
  constexpr std::size_t stateSize = std::size_t{1} << numQubits;
