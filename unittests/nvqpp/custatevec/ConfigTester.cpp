...
TEST(CuStateVecConfigTester, LeavesFusionSizesToCuStateVecExByDefault) {
  Environment legacy("CUDAQ_MGPU_FUSE", nullptr);
  Environment dense("CUDAQ_FUSION_MAX_QUBITS", nullptr);
  Environment diagonal("CUDAQ_FUSION_DIAGONAL_GATE_MAX_QUBITS", nullptr);
  ...
  EXPECT_FALSE(config.denseFusionQubits);
  EXPECT_EQ(config.diagonalFusionQubits, -1);
}

TEST(CuStateVecConfigTester, DiagonalMinusOneUsesCuStateVecExDefault) {
  Environment diagonal("CUDAQ_FUSION_DIAGONAL_GATE_MAX_QUBITS", "-1");
  const auto config = cudaq::cusv::CuStateVecConfig::fromEnvironment();
  EXPECT_EQ(config.diagonalFusionQubits, -1);
}
...
TEST(CuStateVecConfigTester, NonPositiveLegacyFusionDisablesUpdater) {
  Environment generic("CUDAQ_FUSION_MAX_QUBITS", nullptr);
  for (const char *value : {"0", "-1"}) {
    Environment setting("CUDAQ_MGPU_FUSE", value);
    const auto config = cudaq::cusv::CuStateVecConfig::fromEnvironment();
    EXPECT_EQ(config.gateMode, cudaq::cusv::GateExecutionMode::Direct);
    EXPECT_NE(dynamic_cast<cudaq::cusv::DirectGateEngine<double> *>(
                  cudaq::cusv::createGateEngine<double>(config).get()),
...
TEST(CuStateVecConfigTester, DefersDistributedEnvironmentParsing) {
  ...
  auto config = cudaq::cusv::CuStateVecConfig::fromEnvironment();
  EXPECT_ANY_THROW(config.applyDistributedEnvironment());
}
...
TEST(CuStateVecConfigTester, RejectsInvalidControls) {
  const auto rejects = [](const char *name, const char *value) {
    ...
    EXPECT_ANY_THROW(cudaq::cusv::CuStateVecConfig::fromEnvironment())
        << name << '=' << value;
  };
