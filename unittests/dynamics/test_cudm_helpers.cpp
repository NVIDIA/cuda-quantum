...
cudaq::operator_sum initialize_operator_sum() {
    ...
    auto prod_op_1 = 
        cudaq::scalar_operator(std::complex<double>(1.0, 0.0)) * pauli_x * pauli_z;
...
TEST_F(CuDensityMatTestFixture, ConvertToCuDensityMatOperator) {
    ...
    auto result = cudaq::convert_to_cudensitymat_operator(handle, {}, op_sum, mode_extents);
...
TEST_F(CuDensityMatTestFixture, InitializeState) {
    ...
    auto state = cudaq::initialize_state(handle, CUDENSITYMAT_STATE_PURITY_PURE, 2, mode_extents);
