...
TEST(EvolveTester, checkSimple) {
  ...
  integrator.dt = 0.001;
  integrator.order = 1;
  auto result = cudaq::evolveSingle(ham, dims, schedule, initialState,
...
TEST(EvolveTester, checkCompositeSystem) {
  ...
  auto result =
      cudaq::evolveSingle(hamiltonian, dims, schedule, initialState, integrator,
...
TEST(EvolveTester, checkCompositeSystemWithCollapse) {
  ...
  const auto timeSteps = cudaq::linspace(0.0, 0.5, num_steps);
