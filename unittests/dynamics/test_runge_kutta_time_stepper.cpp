...
protected:
  ...
  void SetUp() override {
    stepper = std::make_shared<cudaq::RungeKuttaTimeStepper<TestState>>(
        simple_derivative);
  }
};
...
TEST_F(RungeKuttaTimeStepperTest, SingleStep) {
  ...
  EXPECT_NEAR(state, expected, 1e-3)
      << "Single step Runge-Kutta integration failed!";
}
...
TEST_F(RungeKuttaTimeStepperTest, MultipleSteps) {
  ...
  EXPECT_NEAR(state, expected, 1e-2)
      << "Multiple step Runge-Kutta integration failed!";
}
...
TEST_F(RungeKuttaTimeStepperTest, Convergence) {
  ...
  EXPECT_NEAR(state, expected, 1e-3)
      << "Runge-Kutta integration does not converge!";
}

// // Integrating Sine function
// TEST_F(RungeKuttaTimeStepperTest, SineFunction) {
//     auto sine_stepper =
//     std::make_shared<cudaq::RungeKuttaTimeStepper<TestState>>(sine_derivative);
...
//     EXPECT_NEAR(state, expected, 1e-2) << "Runge-Kutta integration for sine
//     function failed!";
// }
...
TEST_F(RungeKuttaTimeStepperTest, SmallStepSize) {
  ...
  EXPECT_NEAR(state, expected, 1e-3)
      << "Runge-Kutta fails with small step sizes!";
}
...
TEST_F(RungeKuttaTimeStepperTest, LargeStepSize) {
  ...
  EXPECT_NEAR(state, expected, 1e-1)
      << "Runge-Kutta is unstable with large step sizes!";
}

// Constant derivative (dx/dt = 0)
TEST_F(RungeKuttaTimeStepperTest, ConstantFunction) {
  auto constant_stepper =
      std::make_shared<cudaq::RungeKuttaTimeStepper<TestState>>(
          [](const TestState &state, double t) { return 0.0; });
  ...
  EXPECT_NEAR(state, 5.0, 1e-6)
      << "Runge-Kutta should not change a constant function!";
}
