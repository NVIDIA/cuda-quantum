...
protected:
    ...
    void SetUp() override {
        ...
        auto value_function = [](const std::string &param, const std::complex<double> &step) {
            return step;
        };
...
TEST_F(RungeKuttaIntegratorTest, BasicIntegration) {
    ...
    EXPECT_NEAR(integrator->get_state().second, expected, 1e-3) << "Basic Runge-Kutta integration failed!";
}
...
TEST_F(RungeKuttaIntegratorTest, TimeEvolution) {
    ...
    EXPECT_NEAR(integrator->get_state().first, expected, 1e-5) << "Integrator did not correctly update time!";
}
...
TEST_F(RungeKuttaIntegratorTest, LargeStepSize) {
    ...
    EXPECT_NEAR(integrator->get_state().second, expected, 1e-1) << "Runge-Kutta integration failed for large step size!!";
}
...
//     EXPECT_NEAR(integrator->get_state().second, expected, 1e-3) << "Runge-Kutta integration for sine function failed!";
// }
...
TEST_F(RungeKuttaIntegratorTest, SmallStepIntegration) {
    ...
    EXPECT_NEAR(integrator->get_state().second, expected, 5e-4) << "Runge-Kutta integration for small step size failed!";
}
...
TEST_F(RungeKuttaIntegratorTest, LargeStepIntegration) {
    ...
    EXPECT_NEAR(integrator->get_state().second, expected, 1e-2) << "Runge-Kutta integration for large step size failed!";
}
