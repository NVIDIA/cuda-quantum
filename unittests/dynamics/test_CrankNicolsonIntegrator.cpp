...
class CrankNicolsonIntegratorTest : public ::testing::Test {
protected:
...
TEST_F(CrankNicolsonIntegratorTest, Initialization) {
  EXPECT_NO_THROW(cudaq::integrators::crank_nicolson cn1);
  EXPECT_NO_THROW(cudaq::integrators::crank_nicolson cn2(1));
  EXPECT_NO_THROW(cudaq::integrators::crank_nicolson cn3(3, 0.01));
  EXPECT_THROW(cudaq::integrators::crank_nicolson bad(0),
               std::invalid_argument);
  EXPECT_THROW(cudaq::integrators::crank_nicolson(2, 0.0),
               std::invalid_argument);
  EXPECT_THROW(cudaq::integrators::crank_nicolson(2, -0.01),
               std::invalid_argument);
}

TEST_F(CrankNicolsonIntegratorTest, IntegrateLandsExactlyOnSchedulePoints) {
  constexpr std::size_t numIntervals = 99;
  const double maxStepSize = 1.0 / numIntervals;
  cudaq::integrators::crank_nicolson integrator(/*num_corrector_steps=*/1,
                                                maxStepSize);
...
TEST_F(CrankNicolsonIntegratorTest, CheckEvolve) {
  const std::vector<std::complex<double>> initialStateVec = {{1.0, 0.0},
                                                             {0.0, 0.0}};
  const std::vector<int64_t> dims = {2};
  auto spin_op_x = cudaq::spin_op::x(0);
  cudaq::sum_op<cudaq::matrix_handler> ham(2.0 * M_PI * 0.1 * spin_op_x);
  SystemDynamics system(dims, ham);

  for (int nCorr : {1, 2, 3}) {
    std::cout << "Test CN corrector steps = " << nCorr << "\n";
    cudaq::integrators::crank_nicolson integrator(nCorr, 0.001);
...
TEST_F(CrankNicolsonIntegratorTest, CloneReproducesTrajectory) {
  const std::vector<std::complex<double>> initialStateVec = {{1.0, 0.0},
                                                             {0.0, 0.0}};
  const std::vector<int64_t> dims = {2};
...
TEST_F(CrankNicolsonIntegratorTest, ConvergenceOrderVerification) {
  const std::vector<std::complex<double>> initialStateVec = {{1.0, 0.0},
                                                             {0.0, 0.0}};
  const std::vector<int64_t> dims = {2};
  ...
  EXPECT_GE(ratio, 2.0)
      << "Crank-Nicolson should show at least 2nd-order convergence";
  EXPECT_GE(estimated_order, 1.5)
      << "Estimated order should be close to 2 for CN";
}

TEST_F(CrankNicolsonIntegratorTest, NormPreservationLargeSteps) {
  const std::vector<std::complex<double>> initialStateVec = {{1.0, 0.0},
                                                             {0.0, 0.0}};
  const std::vector<int64_t> dims = {2};
  cudaq::sum_op<cudaq::matrix_handler> ham(2.0 * M_PI * 0.1 *
