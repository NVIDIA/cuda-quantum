...
TEST(OperatorExpressions, checkCustomMatrixOps) {
    ...
    auto expected_product = cudaq::kronecker(product_matrices.begin(), product_matrices.end());
    auto expected_product_reverse = cudaq::kronecker(product_reverse_matrices.begin(), product_reverse_matrices.end());
    auto expected_sum_term0 = cudaq::kronecker(sum_matrices_term0.begin(), sum_matrices_term0.end());
    auto expected_sum_term1 = cudaq::kronecker(sum_matrices_term1.begin(), sum_matrices_term1.end());

    utils::checkEqual(op0.to_matrix(dimensions), matrix0); // *not* in canonical order; order as defined in custom op definition
    utils::checkEqual(op1.to_matrix(dimensions), matrix1); // *not* in canonical order; order as defined in custom op definition
    utils::checkEqual((op0 * op1).to_matrix(dimensions), expected_product); // now reordered in canonical order
    utils::checkEqual((op1 * op0).to_matrix(dimensions), expected_product_reverse); // now reordered in canonical order
    utils::checkEqual((op0 + op1).to_matrix(dimensions), expected_sum_term0 + expected_sum_term1); // now reordered in canonical order
    utils::checkEqual((op1 + op0).to_matrix(dimensions), expected_sum_term0 + expected_sum_term1); // now reordered in canonical order
}
...
TEST(OperatorExpressions, checkMatrixOpsWithComplex) {
  ...
  // `matrix_operator`
  {
    ...
    auto want_matrix = scaled_identity + utils::annihilate_matrix(3);
    auto want_matrix_reverse = utils::annihilate_matrix(3) + scaled_identity;
...
TEST(OperatorExpressions, checkMatrixOpsWithScalars) {
  ...
  // `matrix_operator + scalar_operator`
  {
    ...
    auto want_matrix =
        utils::annihilate_matrix(level_count) + scaled_identity;
    auto want_reverse_matrix =
        scaled_identity + utils::annihilate_matrix(level_count);
    utils::checkEqual(want_matrix, got_matrix);
  ...
  // `matrix_operator - scalar_operator`
  {
    ...
    auto want_reverse_matrix = scaled_identity - utils::number_matrix(level_count);
    utils::checkEqual(want_matrix, got_matrix);
    utils::checkEqual(want_reverse_matrix, got_reverse_matrix);
  }
  ...
  // `matrix_operator - scalar_operator`
  {
    ...
    auto want_reverse_matrix = scaled_identity - utils::position_matrix(level_count);
    utils::checkEqual(want_matrix, got_matrix);
    utils::checkEqual(want_reverse_matrix, got_reverse_matrix);
  }
  ...
  // `matrix_operator * scalar_operator`
  {
    ...
    auto got_reverse_matrix = reverse.to_matrix({{degree_index, level_count}}, {{"value", const_scale_factor}});
    auto want_matrix = utils::create_matrix(level_count) * scaled_identity;
    auto want_reverse_matrix =
        scaled_identity * utils::create_matrix(level_count);
    utils::checkEqual(want_matrix, got_matrix);
    utils::checkEqual(want_reverse_matrix, got_reverse_matrix);
  }
}
...
TEST(OperatorExpressions, checkMatrixOpsSimpleArithmetics) {
  ...
  // Addition, same DOF.
  {
    ...
    auto want_matrix = utils::annihilate_matrix(level_count) +
  ...
  // Addition, different DOF's.
  {
    ...
    auto create_full = cudaq::kronecker(utils::create_matrix(level_count),
                                        utils::id_matrix(level_count));
