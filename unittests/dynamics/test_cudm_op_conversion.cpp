...
//     converter =
//         std::make_unique<cudm_op_conversion>(handle, dimensions, schedule);
//   }
// };

// TEST_F(CuDmOpConversion, ConstructorValid) {
//   EXPECT_NO_THROW(cudm_op_conversion converter(handle, dimensions,
//   schedule));
// }

// TEST_F(CuDmOpConversion, ConstructorEmptyDimensions) {
//   std::map<int, int> empty_dimensions;
//   EXPECT_THROW(cudm_op_conversion converter(handle, empty_dimensions,
//   schedule),
...
//   EXPECT_THROW(
//       cudm_op_conversion converter(invalid_handle, dimensions, schedule),
//       std::runtime_error);
// }
...
//   EXPECT_EQ(std::get<std::complex<double>>(result),
...
//   ASSERT_TRUE(
//       std::holds_alternative<cudensitymatWrappedScalarCallback_t>(result));
// }
...
//   auto result = converter->add(converter->evaluate(scalar_op1),
//                                converter->evaluate(scalar_op2));
...
//   scalar_operator scalar_op(1.5);
//   auto scalar_result = converter->evaluate(scalar_op);

//   EXPECT_TRUE(std::holds_alternative<std::complex<double>>(scalar_result));

//   auto moved_result = std::move(scalar_result);

//   EXPECT_TRUE(std::holds_alternative<std::complex<double>>(moved_result));
//   EXPECT_EQ(std::get<std::complex<double>>(moved_result),
//             std::complex<double>(1.5, 0.0));

//   EXPECT_TRUE(scalar_result.index() != std::variant_npos);
// }
