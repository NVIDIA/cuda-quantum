 ...
 struct multi_round_kernel {
    void operator()(int num_qubits, int num_rounds,
      ...
      for (int round = 0; round < num_rounds; round++) {
		...
		// as well here, cudaq::snap_state();
        auto syndrome_z = syn_z(q);
    	auto syndrome_x = syn_x(q);
    	cudaq::save_state();
    	/* decoding at each round */
      }
      mz(q);
      for (int qi = 0; qi < num_qubits; qi++)
        reset(q[qi]);
  ...
  multi_round_kernel{}(num_qubits, num_rounds);
  ...
  assert(ctx_msm.recorded_states.size() == num_rounds && "Expected 3 state snapshots");

  for (std::size_t round = 0; round < ctx_msm.recorded_states.size(); ++round) {
    const auto &state = ctx_msm.recorded_states[round];

    assert(state.getNumQubits() == num_qubits && "Number of qubits must equal num_qubits");
