...
CUDAQ_TEST(SaveStateTester, checkStimState) {
 ...
 struct multi_round_kernel {
    void operator()(int num_qubits, int num_rounds,
      ...
      for (int round = 0; round < num_rounds; round++) {
        ...
        for (int qi = 0; qi < num_qubits; qi++)
          cudaq::apply_noise<cudaq::depolarization_channel>(noise_probability, q[qi]);
  ...
  assert(ctx_msm.get_recorded_states().size() == num_rounds && "Expected 3 state snapshots");

  for (std::size_t round = 0; round < ctx_msm.get_recorded_states().size(); ++round) {
