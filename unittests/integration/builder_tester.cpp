...
CUDAQ_TEST(BuilderTester, checkVecCharPtr) {
  {
    ...
    auto [kernel, paulis] = cudaq::make_kernel<std::vector<const char *>>();
    auto qubits = kernel.qalloc(2);
    kernel.exp_pauli(M_PI_2, qubits, paulis[0]);
    std::cout << kernel << "\n";
    std::vector<const char *> pauliVec{"XX", "YY"};
    kernel(pauliVec);
  }
  {
    // Check it compiles and executes
  ...
  }
  {
    ...
    auto [kernel, thetas, paulis] =
        cudaq::make_kernel<std::vector<double>, std::vector<const char *>>();
    auto qubits = kernel.qalloc(2);
    kernel.x(qubits[0]);
    kernel.for_loop(0, paulis.size(), [&](auto idx) {
    ...
    std::cout << kernel << "\n";
    using namespace cudaq::spin;
    std::vector<const char *> pauliVec{"XY", "YX"};
