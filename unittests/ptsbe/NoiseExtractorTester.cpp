...
TEST(NoiseExtractorTest, AmplitudeDampingIsNotUnitaryMixture) {
  ...
  EXPECT_THROW(
      { auto result = extractNoiseSites(trace, noise_model, true); (void)result; },
...
TEST(NoiseExtractorTest, GracefulValidation_InvalidChannels) {
  ...
  EXPECT_THROW(
      { auto r = extractNoiseSites(trace, noise_model, true); (void)r; },
      std::invalid_argument);
}
...
TEST(NoiseExtractorTest, LargeCircuit) {
  ...
  for (int i = 0; i < 100; ++i) {
    trace.appendInstruction("h", {}, {}, {{static_cast<std::size_t>(i % 10), 2}});
  }
