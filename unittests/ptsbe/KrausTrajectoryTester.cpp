...
static KrausTrajectory makeTrajectory(std::size_t id, double prob, std::size_t errors = 0) {
    std::vector<KrausSelection> selections;
    for (std::size_t i = 0; i < errors; ++i) {
        selections.push_back(KrausSelection(
            i, {0}, "h", KrausOperatorIndex{1}, "Error"));
    }
...
CUDAQ_TEST(KrausTrajectoryTest, WithResults) {
    ...
    traj.measurement_counts = std::map<std::string, std::size_t>{
        {"00", 60},
        {"11", 40}
    };
...
CUDAQ_TEST(KrausTrajectoryTest, CompleteScenario) {
    ...
    std::vector<KrausSelection> sels1 = {
        KrausSelection(0, {0}, "h", KrausOperatorIndex::IDENTITY, "No error on H"),
        KrausSelection(1, {0, 1}, "cx", KrausOperatorIndex::IDENTITY, "No error on CX")
    };
