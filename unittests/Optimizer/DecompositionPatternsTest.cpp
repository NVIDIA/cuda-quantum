...
// Test 1: Verify pattern names match getDebugName()
TEST_F(DecompositionPatternsTest, PatternNamesMatchDebugNames) {
  auto patternNames = cudaq::getAllDecompositionPatternNames();

  for (auto patternName : patternNames) {
    // Create the pattern
    auto pattern =
        cudaq::createDecompositionPattern(context.get(), patternName);
...
// Test 2: Verify metadata is consistent (source and target gates are valid)
TEST_F(DecompositionPatternsTest, MetadataConsistency) {
  auto patternNames = cudaq::getAllDecompositionPatternNames();

  for (auto patternName : patternNames) {
    auto [sourceGate, targetGates] =
        cudaq::getSourceAndTargetGates(patternName);

    // Source gate should not be empty
    EXPECT_FALSE(sourceGate.empty())
        << "Pattern '" << patternName.str() << "' has empty source gate";

    // Target gates should not be empty
    EXPECT_FALSE(targetGates.empty())
        << "Pattern '" << patternName.str() << "' has empty target gates";

    // All target gates should be non-empty
    for (auto targetGate : targetGates) {
      EXPECT_FALSE(targetGate.empty()) << "Pattern '" << patternName.str()
                                       << "' has empty target gate in list";
    }
  }
}
...
TEST_F(DecompositionPatternsTest, DecompositionProducesOnlyTargetGates) {
  ...
  for (auto patternName : patternNames) {
    ...
    ASSERT_TRUE(succeeded(result))
        << "Decomposition pass failed for pattern: " << patternName.str();
    module->dump();
    ...
    if (!unexpectedGates.empty()) {
      ...
      ADD_FAILURE() << "Pattern '" << patternName.str()
