...
class CuStateVecMpiCircuitSimulatorTester
...
public:
  ...
  std::shared_ptr<cudaq::cusv::CuStateVecCommunicator> activeCommunicator() {
...
std::unique_ptr<CuStateVecMpiCircuitSimulatorTester> processSimulator;

CuStateVecMpiCircuitSimulatorTester &getSimulator() {
  if (!processSimulator)
    processSimulator = std::make_unique<CuStateVecMpiCircuitSimulatorTester>();
  return *processSimulator;
}

class SimulatorEnvironment : public ::testing::Environment {
  void TearDown() override { processSimulator.reset(); }
};

[[maybe_unused]] auto *const simulatorEnvironment =
    ::testing::AddGlobalTestEnvironment(new SimulatorEnvironment);
...
int minimumVisibleGpuCount() {
  ...
  if (plugin->get()->Allreduce(plugin->getComm(), &localCount, &minimumCount, 1,
                               INT_32, MIN) != 0)
    return 0;
  return minimumCount;
}
...
std::shared_ptr<cudaq::cusv::CuStateVecCommunicator>
getSimulatorCommunicator() {
  auto &simulator = getSimulator();
  const auto qubits = simulator.allocateQubits(numQubits);
  auto communicator = simulator.activeCommunicator();
  simulator.deallocateQubits(qubits);
  return communicator;
}

bool usesCudaqCommunicator() {
  auto config = cudaq::cusv::CuStateVecConfig::fromEnvironment();
  config.applyDistributedEnvironment();
  return config.communicatorPlugin == cudaq::cusv::CommunicatorPlugin::Self ||
         (config.communicatorPlugin == cudaq::cusv::CommunicatorPlugin::Auto &&
          cudaq::mpi::available());
}

constexpr std::size_t ptsbeNumQubits = 6;
...
void runPtsbeOnDistributedState(bool desynchronizeRng) {
  auto &simulator = getSimulator();
  std::vector<std::size_t> qubits(ptsbeNumQubits);
  ...
  simulator.allocateQubits(ptsbeNumQubits);
...
TEST(MGpuTesterMultiProcesses, GetSimulationStateRejectsRepeatedCall) {
  ...
  EXPECT_ANY_THROW(sim.getSimulationState());
...
TEST(MGpuTesterMultiProcesses, DistributedGpuRandomAdvancesEveryRank) {
  ...
  auto *const plugin = cudaq::mpi::getMpiPlugin();
  ASSERT_EQ(
      plugin->get()->Allreduce(plugin->getComm(), local.data(), minimum.data(),
                               static_cast<int32_t>(count), FLOAT_64, MIN),
      0);
  for (std::size_t index = 0; index < count; ++index)
    EXPECT_NEAR(local[index], minimum[index], 1e-12);
}
...
TEST(MGpuTesterMultiProcesses, CompatibleStateInputStaysOffHost) {
  ...
  auto source = cudaq::cusv::CuStateVecState<cudaq::real>::createMultiProcess(
      ...
      std::size_t{1} << 26, getSimulatorCommunicator(), true);
...
TEST(MGpuTesterMultiProcesses, FailedCommunicatorRebindingRestoresDescriptor) {
  auto communicator = getSimulatorCommunicator();
  const int32_t expectedSize = communicator->size();
  const int32_t expectedRank = communicator->rank();
  const auto communicatorSize = cudaq::mpi::getMpiPlugin()->getComm()->commSize;

  EXPECT_ANY_THROW(communicator->setCommunicator(nullptr, communicatorSize));
...
TEST(MGpuTesterMultiProcesses, CommunicatorRebindingReinitializesState) {
  ...
  if (cudaq::mpi::num_ranks() != 4)
    GTEST_SKIP() << "This regression requires four MPI ranks.";
  if (!usesCudaqCommunicator())
    GTEST_SKIP() << "This regression requires the CUDA-Q communicator.";
  auto *const plugin = cudaq::mpi::getMpiPlugin();
  cudaqDistributedCommunicator_t *pair = nullptr;
  ASSERT_EQ(plugin->get()->CommSplit(plugin->getComm(), cudaq::mpi::rank() / 2,
                                     0, &pair),
            0);
  ASSERT_NE(pair, nullptr);
  ASSERT_TRUE(sim.setMpiCommunicator(pair->commPtr, pair->commSize));
...
TEST(MGpuTesterMultiProcesses, CommunicatorRebindingPreservesUserSeed) {
  ...
  cudaqDistributedCommunicator_t *singleton = nullptr;
  ASSERT_EQ(
      plugin->get()->CommSplit(activeWorld, cudaq::mpi::rank(), 0, &singleton),
      0);
  ...
  sim.allocateQubits(1);
  sim.deallocateQubits({0});
  ASSERT_TRUE(sim.setMpiCommunicator(singleton->commPtr, singleton->commSize));
...
TEST(MGpuTesterMultiProcesses, CustomCommunicatorKeepsWorldDeviceRank) {
  ...
  auto *const plugin = cudaq::mpi::getMpiPlugin();
  cudaqDistributedCommunicator_t *singleton = nullptr;
  ASSERT_EQ(plugin->get()->CommSplit(plugin->getComm(), cudaq::mpi::rank(), 0,
                                     &singleton),
            0);
  ASSERT_NE(singleton, nullptr);
  ASSERT_TRUE(sim.setMpiCommunicator(singleton->commPtr, singleton->commSize));
...
TEST(MGpuTesterMultiProcesses, CommunicatorRebindingLifecycle) {
  ...
  sim.allocateQubits(1);
  EXPECT_ANY_THROW(
      sim.setMpiCommunicator(singleton->commPtr, singleton->commSize));
  sim.deallocateQubits({0});

  EXPECT_TRUE(sim.setMpiCommunicator(singleton->commPtr, singleton->commSize));
  EXPECT_TRUE(
      sim.setMpiCommunicator(activeWorld->commPtr, activeWorld->commSize));
}
...
TEST(PtsbeMgpuTest, PtsbeDistributed) {
  if (minimumVisibleGpuCount() < 2)
    GTEST_SKIP() << "This regression requires two visible GPUs.";
...
TEST(PtsbeMgpuTest, PtsbeDistributedDesync) {
  if (minimumVisibleGpuCount() < 2)
    GTEST_SKIP() << "This regression requires two visible GPUs.";
  ASSERT_NE(std::getenv("CUDAQ_MGPU_NQUBITS_THRESH"), nullptr);
