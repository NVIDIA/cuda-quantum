...
leverage `nvq++`{.code .docutils .literal .notranslate} for compilation
of CUDA-Q source files.

To see how this works, let's create a trivial example. Suppose we have a
folder named `myCudaQApp`{.code .docutils .literal .notranslate} and
within that folder we have a simple CUDA-Q source file, like
`ghz.cpp`{.code .docutils .literal .notranslate}. With a local CUDA-Q
installation present on the system, we can create the following
top-level `CMakeLists.txt`{.code .docutils .literal .notranslate} file
to build this file with `nvq++`{.code .docutils .literal .notranslate}:
...
.notranslate} command with `--verbose`{.code .docutils .literal
.notranslate}, you'll see that `nvq++`{.code .docutils .literal
.notranslate} was used to compile and link the executable.
:::
:::
:::
