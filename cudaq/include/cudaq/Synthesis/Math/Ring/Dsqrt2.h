...
public:
  ...
  /// Equality modulo denominator exponent: both operands are normalized to
  ...
  ///     conj_sq2(alpha / `sqrt`(2)^k) = -conj_sq2(alpha) / `sqrt`(2)^k   (k
  ///     odd)
  ///                                 =  conj_sq2(alpha) / `sqrt`(2)^k   (k
  ///                                 even)
...
inline DSqrt2 mul_by_sqrt2_power(const DSqrt2 &x, const Integer &d) {
  ...
  if (d_mod_2)
    new_alpha =
        new_alpha * ZSqrt2(0, 1); // multiply by `sqrt`(2) in Z[`sqrt`(2)]
