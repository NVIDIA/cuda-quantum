...
/// but macro-expand to one: `quake.apply`, `quake.compute_action`,
/// `quake.apply_noise`, and `quake.call_by_ref`. Every other op only manages
/// `ref`/`veq` types (e.g., `quake.concat`, `quake.dealloc`,
/// `quake.relax_size`, `cc.instantiate_callable`, `cc.callable_closure`, and
/// `quake.evince`) and cannot alias or mutate a qubit we are tracking.
/// `quake.evince` is not an use in the execution sense.
