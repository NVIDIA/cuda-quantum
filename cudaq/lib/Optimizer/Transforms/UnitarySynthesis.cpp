...
/// unique_ptr before AnyDecomposer's full definition is available.
struct AnyDecomposer;
...
/// Plain-data struct for 1-qubit ZYZ synthesis.
///
...
struct OneQubitOpZYZ {
  ...
  OneQubitOpZYZ(const Eigen::Matrix2cd &mat) : targetMatrix(mat) {
...
///                    0  i −1  0
///                    1  0  0 −i
static const Eigen::Matrix4cd &MagicBasisMatrix() {
  using namespace std::complex_literals;
...
/// Helper function to convert a matrix into 'magic' basis
static const Eigen::Matrix4cd &MagicBasisMatrixAdj() {
...
///                +1 −1 −1 −1
///                +1 −1 +1 +1
static const Eigen::Matrix4cd &GammaFactor() {
...
static std::tuple<Eigen::Matrix4d, Eigen::Matrix4cd, Eigen::Matrix4d>
...
static std::tuple<Eigen::Matrix2cd, Eigen::Matrix2cd, std::complex<double>>
...
static Eigen::Matrix4cd canonicalVecToMatrix(double x, double y, double z) {
  using namespace std::complex_literals;
...
namespace {
/// Plain-data struct for 2-qubit KAK synthesis.
///
...
struct TwoQubitOpKAK {
  ...
  TwoQubitOpKAK(const Eigen::MatrixXcd &vec) : targetMatrix(vec) {
    using namespace std::complex_literals;
...
completeUnitaryBasis(const Eigen::MatrixXcd &definedCols, Eigen::Index n) {
...
static CSDComponents cosineSineDecomposition(const Eigen::MatrixXcd &u,
                                             bool &ok) {
...
static void demultiplex(const Eigen::MatrixXcd &a, const Eigen::MatrixXcd &b,
...
namespace {
/// Plain-data struct for N-qubit QSD synthesis.
///
/// Method bodies that construct or dereference AnyDecomposer are defined after
/// AnyDecomposer's full definition to break the mutual-dependency cycle.
///
...
struct NQubitOpQSD {
  ...
  /// The four `(n-1)`-qubit sub-unitary decomposers, built during construction
  /// so the whole recursion tree is validated before any IR is emitted.
  std::unique_ptr<AnyDecomposer> childWr, childVr, childWl, childVl;
  bool valid = true;
  ...
  std::unique_ptr<AnyDecomposer> makeChild(const Eigen::MatrixXcd &matrix);
...
/// Modern C++ type class concept for unitary decomposers. Satisfied by any
/// plain-data struct that exposes a `valid` flag and an `emitDecomposedFuncOp`
/// method. No base class or vtable is required.
template <typename T>
concept DecomposerType =
    requires(T &d, cudaq::quake::CustomUnitaryConstantOp customOp,
             OpBuilder &rewriter, std::string funcName) {
      { d.valid } -> std::convertible_to<bool>;
      d.emitDecomposedFuncOp(customOp, rewriter, std::move(funcName));
    };

static_assert(DecomposerType<OneQubitOpZYZ>);
static_assert(DecomposerType<TwoQubitOpKAK>);
static_assert(DecomposerType<NQubitOpQSD>);

namespace {
/// Type-erased sum of all decomposer instances. Dispatch is via `std::visit`
/// rather than virtual dispatch: no vtable, no mandatory heap allocation for
/// the 1- and 2-qubit base cases.
struct AnyDecomposer {
  using Variant = std::variant<OneQubitOpZYZ, TwoQubitOpKAK, NQubitOpQSD>;
  Variant inner;

  bool isValid() const {
    return std::visit([](const auto &d) -> bool { return d.valid; }, inner);
  }
  ...
  template <DecomposerType T>
  explicit AnyDecomposer(T &&t) : inner(std::forward<T>(t)) {}
};
} // namespace

// ---------------------------------------------------------------------------
// NQubitOpQSD out-of-line definitions (AnyDecomposer is now complete)
// ---------------------------------------------------------------------------

std::unique_ptr<AnyDecomposer>
NQubitOpQSD::makeChild(const Eigen::MatrixXcd &matrix) {
  size_t childQubits = numQubits - 1;
  if (childQubits == 1) {
    Eigen::Matrix2cd m2 = matrix;
    return std::make_unique<AnyDecomposer>(OneQubitOpZYZ(m2));
  }
  if (childQubits == 2)
    return std::make_unique<AnyDecomposer>(TwoQubitOpKAK(matrix));
  return std::make_unique<AnyDecomposer>(NQubitOpQSD(matrix));
}

void NQubitOpQSD::emitMux(const std::vector<double> &angles,
                          ArrayRef<Value> controls, Value target, bool isRy,
...
NQubitOpQSD::NQubitOpQSD(const Eigen::MatrixXcd &vec) : targetMatrix(vec) {
...
/// or return nullopt when the dimension is outside the supported synthesis
/// range (a power of two with `2 <= dimension <= 32`, i.e. 1-5 qubits). The
/// returned decomposer has already run its factorization; callers consult
/// `isValid()` to learn whether the reconstruction stayed within tolerance.
static std::optional<AnyDecomposer>
makeDecomposer(const Eigen::MatrixXcd &unitary, size_t dimension) {
  if (dimension == 2) {
    Eigen::Matrix2cd m2 = unitary;
    return AnyDecomposer{OneQubitOpZYZ(m2)};
  }
  if (dimension == 4)
    return AnyDecomposer{TwoQubitOpKAK(unitary)};
  if (dimension >= 8 && dimension <= 32 && (dimension & (dimension - 1)) == 0)
    return AnyDecomposer{NQubitOpQSD(unitary)};
  return std::nullopt;
}
...
struct DecompInfo {
  ...
  std::optional<AnyDecomposer>
      decomposer; ///< non-empty only when `Convertible`
