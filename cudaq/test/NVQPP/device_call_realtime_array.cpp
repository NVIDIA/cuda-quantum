...
__qpu__ int byteVectorKernel(std::vector<std::uint8_t> values) {
  return cudaq::device_call(0, sumByteVector, values, 3);
}
...
static bool runByRefOutputStdvecTests() {
  ...
  return incremented.size() == 4 && incremented[0] == 4 &&
         ...
         bits.size() == 10 && bits[0] && !bits[1] && bits[2] && bits[3] &&
