    ...
    -   [Quantum Hardware (QPUs)](../hardware.html){.reference
        ...
        -   [Photonic QPUs](photonic.html){.reference .internal}
            -   [ORCA
                ...
                .internal}
        -   [Quantum Control Systems](#){.current .reference .internal}
            -   [Quantum Machines](#quantum-machines){.reference
                .internal}
...
Previous](photonic.html "Photonic"){.btn .btn-neutral .float-left
...
::: {#quantum-control-systems .section}
# Quantum Control Systems[¶](#quantum-control-systems "Permalink to this heading"){.headerlink}

::: {#quantum-machines .section}
## Quantum Machines[¶](#quantum-machines "Permalink to this heading"){.headerlink}

Quantum Machines provides a unified quantum computing control system
that enables users to execute quantum programs on various QPU types
(hardware backends) through their Quantum Orchestration Platform (QOP) -
a platform that controls different quantum hardware implementations
through a unified software API.

For information about available hardware backends and their
capabilities, please consult the [Quantum Machines
website](https://www.quantum-machines.co/){.reference .external}.

::: {#setting-credentials .section}
### Setting Credentials[¶](#setting-credentials "Permalink to this heading"){.headerlink}

To use Quantum Machines with CUDA-Q, you need to have an API key and
access to the Quantum Machines server or their [`QOperator`{.code
.docutils .literal .notranslate}]{.pre} service. You can set it using an
...
Alternatively, you can provide it directly when setting the target in
your code.
:::
...
To specify which backend to use, set the [`executor`{.code .docutils
.literal .notranslate}]{.pre} parameter when configuring the target. The
available backends depend on your specific access rights and set up with
Quantum Machines. By default, a mock executor is used.

::: {.tab-set .docutils}
Python

::: {.tab-content .docutils}
To target Quantum Machines from Python, use the
[`cudaq.set_target()`{.docutils .literal .notranslate}]{.pre} function:
...
-   [`url`{.docutils .literal .notranslate}]{.pre}: The URL of the
    Quantum Machines server

-   [`executor`{.docutils .literal .notranslate}]{.pre}: The name of the
    executor/backend to use (defaults to "mock")

-   [`api_key`{.docutils .literal .notranslate}]{.pre}: Your API key
    (optional if set via environment variable)
:::

C++

::: {.tab-content .docutils}
To target quantum kernel code for execution on Quantum Machines, pass
the flag [`--target`{.docutils .literal
.notranslate}]{.pre}` `{.docutils .literal
.notranslate}[`quantum_machines`{.docutils .literal .notranslate}]{.pre}
...
-   [`--quantum-machines-url`{.docutils .literal .notranslate}]{.pre}:
    The URL of the QOperator server

-   [`--quantum-machines-executor`{.docutils .literal
    .notranslate}]{.pre}: The name of the executor/backend to use
    (defaults to "mock")

-   [`--quantum-machines-api-key`{.docutils .literal
    .notranslate}]{.pre}: Your API key (if not set via environment
    variable)
:::
:::

To see a complete example, take a look at [[Quantum Machines
examples]{.std
.std-ref}](../../examples/hardware_providers.html#quantum-machines-examples){.reference
.internal}.
:::
:::
:::
