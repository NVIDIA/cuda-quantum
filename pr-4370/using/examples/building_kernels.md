...
.docutils .literal .notranslate}]{.pre} builds a register consisting of
a single qubit, while [`cudaq.qvector`{.code .docutils .literal
.notranslate}]{.pre} builds a register of [\\(N\\)]{.math .notranslate
.nohighlight} qubits.
...
adding [`.ctrl`{.code .docutils .literal .notranslate}]{.pre} to the end
of any gate, followed by specification of the control qubit and the
target qubit.
...
::: {#multi-controlled-operations .section}
...
[`adj`{.code .docutils .literal .notranslate}]{.pre} designation.
...
[`cudaq.register_operation`{.code .docutils .literal
.notranslate}]{.pre}. A one-dimensional [`Numpy`{.code .docutils
.literal .notranslate}]{.pre} array specifies the unitary matrix to be
applied. The entries of the array read from top to bottom through the
rows.
...
how [`kernel_A`{.code .docutils .literal .notranslate}]{.pre} can be
called within [`kernel_B`{.code .docutils .literal .notranslate}]{.pre}
to perform CNOT operations.
