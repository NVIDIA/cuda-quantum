...
CUDA-Q also provides a [`pasqal`{.docutils .literal .notranslate}]{.pre}
target for routing Pasqal jobs through the vendor agnostic Quantum
Resource Management Interface (QRMI). This target enables integration
with resource managers like Slurm for scheduling. Select the Pasqal
