...
-   [Applications](../../using/applications.html){.reference .internal}
    ...
    -   [Entanglement Accelerates Quantum Simulation](#){.current
        .reference .internal}
        -   [2. Model Definition](#2.-Model-Definition){.reference
            .internal}
            -   [2.1 Initial product
                state](#2.1-Initial-product-state){.reference .internal}
            -   [2.2 QIMF Hamiltonian](#2.2-QIMF-Hamiltonian){.reference
                .internal}
            -   [2.3 First-Order Trotter Formula
                (PF1)](#2.3-First-Order-Trotter-Formula-(PF1)){.reference
                .internal}
            -   [2.4 PF1 step for the QIMF
                partition](#2.4-PF1-step-for-the-QIMF-partition){.reference
                .internal}
            -   [2.5 Hamiltonian
                helpers](#2.5-Hamiltonian-helpers){.reference .internal}
        -   [3. Entanglement
            metrics](#3.-Entanglement-metrics){.reference .internal}
        -   [4. Simulation workflow](#4.-Simulation-workflow){.reference
            .internal}
            -   [4.1 Single-step Trotter
                error](#4.1-Single-step-Trotter-error){.reference
                .internal}
            -   [4.2 Dual trajectory
                update](#4.2-Dual-trajectory-update){.reference
                .internal}
        -   [5. Reproducing the paper's Figure
            1a](#5.-Reproducing-the-paper’s-Figure-1a){.reference
            .internal}
            -   [5.1 Visualising the joint
                behaviour](#5.1-Visualising-the-joint-behaviour){.reference
                .internal}
            -   [5.2 Interpreting the
                result](#5.2-Interpreting-the-result){.reference
                .internal}
        -   [6. References and further
            reading](#6.-References-and-further-reading){.reference
            .internal}
...
Previous](skqd.html "Sample-Based Krylov Quantum Diagonalization (SKQD)"){.btn
.btn-neutral .float-left accesskey="p"} [Next []{.fa
.fa-arrow-circle-right
aria-hidden="true"}](ptsbe.html "Pre-Trajectory Sampling with Batch Execution (PTSBE)"){.btn
...
::: {#Entanglement-Accelerates-Quantum-Simulation .section}
# Entanglement Accelerates Quantum Simulation[¶](#Entanglement-Accelerates-Quantum-Simulation "Permalink to this heading"){.headerlink}

This notebook demonstrates how to reproduce the key numerical result
from [Entanglement Accelerates Quantum
Simulation](https://www.nature.com/articles/s41567-025-02945-2){.reference
.external} using NVIDIA CUDA-Q. We follow the workflow presented in the
paper for the quantum Ising model with mixed fields (QIMF) and show how
entanglement growth can *reduce* the observed Trotter error of the
first-order product formula (PF1).

Notebook prepared by Jonah So
([jonahso@ucsb.edu](mailto:jonahso%40ucsb.edu){.reference .external})
and Jue Xu
([xujue@connect.hku.hk](mailto:xujue%40connect.hku.hk){.reference
.external}).
...
## 2. Model Definition[¶](#2.-Model-Definition "Permalink to this heading"){.headerlink}

We focus on the [\\(n\\)]{.math .notranslate .nohighlight}-qubit quantum
Ising model with mixed fields (QIMF), the same Hamiltonian used
throughout the paper. The subsections below walk through the exact
construction, the PF1 Trotter step, and the helper routines needed for
later analysis.

::: {#2.1-Initial-product-state .section}
### 2.1 Initial product state[¶](#2.1-Initial-product-state "Permalink to this heading"){.headerlink}

We start from the [\\(\|0000\\ldots\\rangle\\)]{.math .notranslate
.nohighlight} state, matching the initial condition explored in the
paper. The helper kernel below prepares this state.
...
::: {#2.2-QIMF-Hamiltonian .section}
### 2.2 QIMF Hamiltonian[¶](#2.2-QIMF-Hamiltonian "Permalink to this heading"){.headerlink}

The paper studies rapid entanglement growth in the one-dimensional QIMF
Hamiltonian

::: {.math .notranslate .nohighlight}
\\\[H = h_x \\sum\_{j=1}\^N X_j + h_y \\sum\_{j=1}\^N Y_j + J
\\sum\_{j=1}\^{N-1} X_j X\_{j+1},\\\]
:::

where [\\(h_x\\)]{.math .notranslate .nohighlight} and [\\(h_y\\)]{.math
.notranslate .nohighlight} set the mixed fields and [\\(J\\)]{.math
.notranslate .nohighlight} couples nearest neighbours. Every term is
expressed in the Pauli basis, so we can assemble [\\(H\\)]{.math
.notranslate .nohighlight} with CUDA-Q's [`spin`{.docutils .literal
.notranslate}]{.pre} helpers. For later Trotterization we split the
Hamiltonian into two commuting groups where the terms in the same group
commute with each other:

-   [\\(A = h_x \\sum_j X_j + J \\sum_j X_j X\_{j+1}\\)]{.math
    .notranslate .nohighlight}

-   [\\(B = h_y \\sum_j Y_j\\)]{.math .notranslate .nohighlight}

The function below returns [\\((A, B, H)\\)]{.math .notranslate
.nohighlight}
...
::: {#2.3-First-Order-Trotter-Formula-(PF1) .section}
### 2.3 First-Order Trotter Formula (PF1)[¶](#2.3-First-Order-Trotter-Formula-(PF1) "Permalink to this heading"){.headerlink}

The paper benchmarks the first-order product formula as a baseline
algorithm. For a Hamiltonian decomposition [\\(H = \\sum_l H_l\\)]{.math
.notranslate .nohighlight}, the ideal evolution [\\(U(\\delta t) =
e\^{-i H \\delta t}\\)]{.math .notranslate .nohighlight} is approximated
by the ordered product of exponentials

::: {.math .notranslate .nohighlight}
\\\[\\mathscr{U}\_1(\\delta t) = \\overrightarrow{\\prod\_{l}} e\^{-i
H_l \\, \\delta t},\\\]
:::

which incurs an error of [\\(\\mathcal{O}(\\delta t\^2)\\)]{.math
.notranslate .nohighlight} whenever the terms fail to commute. The
authors show that once the simulated state has significant entanglement,
the empirical PF1 error drops sharply to the average-case performance.

The CUDA-Q kernel below implements a single PF1 step for two commuting
groups (the [\\(A\\)]{.math .notranslate .nohighlight} and
[\\(B\\)]{.math .notranslate .nohighlight} partitions defined above).
:::

::: {#2.4-PF1-step-for-the-QIMF-partition .section}
### 2.4 PF1 step for the QIMF partition[¶](#2.4-PF1-step-for-the-QIMF-partition "Permalink to this heading"){.headerlink}

Because each [\\(A\\)]{.math .notranslate .nohighlight} term commutes
with every other [\\(A\\)]{.math .notranslate .nohighlight} term (and
similarly for [\\(B\\)]{.math .notranslate .nohighlight}), the PF1
update factorises neatly into

::: {.math .notranslate .nohighlight}
\\\[\\mathscr{U}\_1(\\delta t) = e\^{-i A \\delta t} \\, e\^{-i B
\\delta t}.\\\]
:::

To simulate for a total time [\\(t\\)]{.math .notranslate .nohighlight},
we apply this kernel [\\(r\\)]{.math .notranslate .nohighlight} times
with [\\(\\delta t = t/r\\)]{.math .notranslate .nohighlight}. The paper
sets [\\(t=10\\)]{.math .notranslate .nohighlight} and
[\\(r=100\\)]{.math .notranslate .nohighlight}, which we adopt below so
the numerical values line up with Figure 1a.
...
::: {#2.5-Hamiltonian-helpers .section}
### 2.5 Hamiltonian helpers[¶](#2.5-Hamiltonian-helpers "Permalink to this heading"){.headerlink}

The simulation loop expects explicit coefficient lists and Pauli words.
The utility functions below wrap CUDA-Q's iterator API so that kernels
can consume plain Python lists. This also keeps the PF1 kernel agnostic
to the specific model --- any Hamiltonian decomposed into commuting
partitions can reuse the same workflow.
...
::: {#3.-Entanglement-metrics .section}
## 3. Entanglement metrics[¶](#3.-Entanglement-metrics "Permalink to this heading"){.headerlink}

To match the paper we track how much entanglement each subsystem grows
during time evolution. For a pure state [\\(\|\\psi\\rangle\\)]{.math
.notranslate .nohighlight}, the reduced density matrix of a kept
subsystem [\\(A\\)]{.math .notranslate .nohighlight} is
...
where [\\(B\\)]{.math .notranslate .nohighlight} denotes the qubits we
trace out. The von Neumann entropy

::: {.math .notranslate .nohighlight}
\\\[S(\\rho_A) = - \\sum_i \\lambda_i \\log_2 \\lambda_i\\\]
:::

with eigenvalues [\\(\\lambda_i\\)]{.math .notranslate .nohighlight}
provides a convenient single-number entanglement measure. When
[\\(S(\\rho_A)\\)]{.math .notranslate .nohighlight} approaches its
maximum the subsystem is almost maximally mixed, signalling strong
entanglement with the rest of the chain. The helpers below implement the
partial trace and entropy evaluation using CuPy.
...
::: {#4.-Simulation-workflow .section}
## 4. Simulation workflow[¶](#4.-Simulation-workflow "Permalink to this heading"){.headerlink}

With the building blocks in place, we compare the ideal evolution with
the PF1 approximation for a 12-qubit chain. The helpers below construct
the exact unitary [\\(U = e\^{-iH\\delta t}\\)]{.math .notranslate
.nohighlight} and step both trajectories forward so that we can track
their divergence and the associated entanglement.
...
::: {#4.1-Single-step-Trotter-error .section}
### 4.1 Single-step Trotter error[¶](#4.1-Single-step-Trotter-error "Permalink to this heading"){.headerlink}

Following Eq. (3) of the paper we evaluate the norm difference between
the ideal state and its PF1 counterpart after each segment:

::: {.math .notranslate .nohighlight}
\\\[\\epsilon(t) = \\big\\\|(U(\\delta t) - \\mathscr{U}\_1(\\delta
t))\\,\|\\psi(t)\\rangle\\big\\\|.\\\]
:::

Tracking [\\(\\epsilon(t)\\)]{.math .notranslate .nohighlight} alongside
entanglement lets us reproduce the result highlighted in Figure 1a.
...
::: {#4.2-Dual-trajectory-update .section}
### 4.2 Dual trajectory update[¶](#4.2-Dual-trajectory-update "Permalink to this heading"){.headerlink}

The driver below advances both trajectories for [\\(r\\)]{.math
.notranslate .nohighlight} steps:

-   **Ideal evolution** multiplies the state by [\\(U = e\^{-iH\\delta
    t}\\)]{.math .notranslate .nohighlight} at each step.

-   **PF1 evolution** applies the Trotter evolution [\\(e\^{-iA\\delta
    t}e\^{-iB\\delta t}\\)]{.math .notranslate .nohighlight} to the
    CUDA-Q state.

Storing the intermediate states allows us to probe entanglement and
Trotter error at every segment boundary, exactly as in the reference
study.
...
::: {#5.-Reproducing-the-paper’s-Figure-1a .section}
## 5. Reproducing the paper's Figure 1a[¶](#5.-Reproducing-the-paper’s-Figure-1a "Permalink to this heading"){.headerlink}

We now evolve the [\\(n=12\\)]{.math .notranslate .nohighlight} QIMF
chain for [\\(t=10\\)]{.math .notranslate .nohighlight} with
[\\(r=100\\)]{.math .notranslate .nohighlight} PF1 steps, exactly
matching the parameters in the paper. At every segment we store the PF1
error [\\(\\epsilon(t)\\)]{.math .notranslate .nohighlight} and the
entanglement entropy for contiguous subsystems of size 1 through 4.
...
::: {#5.1-Visualising-the-joint-behaviour .section}
### 5.1 Visualising the joint behaviour[¶](#5.1-Visualising-the-joint-behaviour "Permalink to this heading"){.headerlink}

The plot below overlays the PF1 Trotter error with the entropy of
increasingly large contiguous subsystems. This reproduces the
qualitative behaviour of Figure 1a from [Entanglement Accelerates
Quantum
Simulation](https://www.nature.com/articles/s41567-025-02945-2){.reference
.external}:

-   the PF1 error (blue) drops within the first few units of simulated
    time,

-   each entropy curve (coloured) rises toward the maximally mixed value
    for its subsystem size, and

-   larger subsystems take longer to saturate, mirroring the light-cone
    spreading of entanglement.

The key takeaway is that higher entanglement correlates with *lower*
observed Trotter error, counter to the classical intuition that
entanglement makes simulation harder.
...
::: {.output_area .docutils .container}
![](../../_images/applications_python_entanglement_acc_hamiltonian_simulation_23_0.png)
:::
:::
:::

::: {#5.2-Interpreting-the-result .section}
### 5.2 Interpreting the result[¶](#5.2-Interpreting-the-result "Permalink to this heading"){.headerlink}

Intuitively, the entangled state locally looks like an ensemble of
random states, leading to average-case Trotter errors that shrink as
entanglement increases. The tutorial demonstrates that CUDA-Q can
capture this effect faithfully for realistic system sizes. Try
experimenting with:

-   different subsystem cuts to probe entanglement propagation in more
    detail,

-   higher-order product formulas (PF2, PF4) to compare error scalings,
    or

-   alternative initial product states to see how quickly the system
    enters the high-entanglement regime.
:::
:::

::: {#6.-References-and-further-reading .section}
## 6. References and further reading[¶](#6.-References-and-further-reading "Permalink to this heading"){.headerlink}

-   [Entanglement Accelerates Quantum
    Simulation](https://arxiv.org/abs/2406.02379){.reference .external}

-   [Spin-Hamiltonian Simulation Using
    CUDA-Q](https://nvidia.github.io/cuda-quantum/latest/applications/python/hamiltonian_simulation.html){.reference
    .external}
:::
:::
:::
