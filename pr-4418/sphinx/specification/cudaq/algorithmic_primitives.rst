...
executions). The result of this task is a mapping of observed measurement bit
strings to the number of times each was observed (the *counts dictionary*).
...
This function takes as input a quantum kernel instance followed by the concrete
arguments at which the kernel should be invoked. Overloaded functions exist for
specifying the number of shots and the noise model to apply.
...
**[3]** The :code:`__global__` register in the returned :code:`sample_result`
contains the primary bitstring output. Its bit ordering is determined by two
rules:

- If the kernel contains one or more measurement operations (:code:`mz`,
  :code:`mx`, :code:`my`), the :code:`__global__` bitstring is formed in
  **measurement order** — the order the measurements appear in the kernel.
- If the kernel contains no measurement operations, CUDA-Q performs an implicit
  final measurement of all allocated qubits and forms the :code:`__global__`
...
produces :code:`{ 10:1000 }` — bit 0 corresponds to the first allocated qubit.
...
produces :code:`{ 01:1000 }` — bit 0 is from :code:`b` (measured first), bit 1
is from :code:`a` (measured second).

Named measurement registers
""""""""""""""""""""""""""""

When a measurement result is assigned to a named variable, that variable name
becomes a **named register** in the :code:`sample_result`. Each named register
holds the counts for the corresponding measurement independently from the
:code:`__global__` register.

In C++, the :code:`nvq++` compiler extracts the variable name from the
assignment and embeds it as the register name in the compiled IR:
...
In Python, named registers can be created either by variable assignment (as in
C++) or by passing the :code:`register_name` keyword argument to :code:`mz`:
...
The :code:`__global__` register always contains the full concatenated bitstring
(following the measurement-order rule above). Named registers give per-measurement
access to individual results.

.. deprecated::
   Named measurement registers in :code:`cudaq::sample` are deprecated. Use
   :code:`cudaq::run` to retrieve individual measurement results from each shot.

.. note::

  If you don't specify any measurements in your kernel and allow the
  :code:`nvq++` compiler to perform passes that introduce ancilla qubits into
  your kernel, it may be difficult to discern which qubits are the ancilla
  qubits vs which ones are your qubits. In this case, it is recommended that
  you provide explicit measurements in your kernel in order to only receive
  measurements from your qubits and silently discard the measurements from the
  ancillary qubits.

:code:`sample_result` type
^^^^^^^^^^^^^^^^^^^^^^^^^^

**[4]** CUDA-Q specifies the following structure for :code:`cudaq::sample_result`:
...
The :code:`sample_result` type tracks a unique global register named
:code:`__global__` plus any named measurement registers. The API provides:

- :code:`count(bitString)` — number of times a bit string was observed.
- :code:`to_map()` — full counts dictionary as :code:`std::unordered_map`.
- :code:`get_marginal(indices)` — a new :code:`sample_result` over a subset of
  measured qubits.
- :code:`sequential_data()` — bit string observations in shot order.
- :code:`expectation()` / :code:`probability(bitString)` — derived statistics.

Kernel requirements
^^^^^^^^^^^^^^^^^^^

**[5]** The kernel passed to :code:`sample` must be an entry-point kernel that
returns :code:`void`. This must be enforced by compiler implementations.

Asynchronous sampling
^^^^^^^^^^^^^^^^^^^^^

**[6]** CUDA-Q also provides :code:`cudaq::sample_async` which returns a
:code:`async_sample_result`:
...
Programmers can asynchronously launch sampling tasks on any :code:`qpu_id`. The
:code:`async_sample_result` wraps a :code:`std::future<sample_result>` and
exposes :code:`get()` to retrieve results after asynchronous execution.

For remote QPU systems with long queue times, the :code:`async_sample_result`
type encodes job ID information and can be persisted to file and loaded at a
later time. After loading from file, and when remote queue jobs are completed,
one can invoke :code:`get()` and the results will be retrieved and returned.

Implementation note: :code:`explicit_measurements`
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

**[7]** For each kernel, the runtime computes an internal
:code:`explicit_measurements` flag. This is an execution-planning signal for
backend implementations, not a user semantic override.

When measurements exist in the kernel, the flag tells the backend that it must
preserve user measurement order in the results. When no measurements exist (or
measurements trivially follow allocation order), the flag tells the backend it
can use more efficient strategies such as buffered-result collection or implicit
measure-all.

Earlier releases exposed this flag as a user-controlled runtime option. This
user control is now deprecated and silently ignored — the runtime computes the
effective behavior automatically. The :code:`explicit_measurements` field in
:code:`sample_options` remains only for source compatibility.
