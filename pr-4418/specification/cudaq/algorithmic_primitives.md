...
(circuit executions). The result of this task is a mapping of observed
measurement bit strings to the number of times each was observed (the
*counts dictionary*).
...
concrete arguments at which the kernel should be invoked. Overloaded
functions exist for specifying the number of shots and the noise model
to apply.
...
::: {#sampling-semantics .section}
### [12.1.1. ]{.section-number}Sampling semantics[¶](#sampling-semantics "Permalink to this heading"){.headerlink}

**\[3\]** The [`__global__`{.code .docutils .literal
.notranslate}]{.pre} register in the returned [`sample_result`{.code
.docutils .literal .notranslate}]{.pre} contains the primary bitstring
output. Its bit ordering is determined by two rules:

-   If the kernel contains one or more measurement operations
    ([`mz`{.code .docutils .literal .notranslate}]{.pre}, [`mx`{.code
    .docutils .literal .notranslate}]{.pre}, [`my`{.code .docutils
    .literal .notranslate}]{.pre}), the [`__global__`{.code .docutils
    .literal .notranslate}]{.pre} bitstring is formed in **measurement
    order** --- the order the measurements appear in the kernel.

-   If the kernel contains no measurement operations, CUDA-Q performs an
    implicit final measurement of all allocated qubits and forms the
    [`__global__`{.code .docutils .literal .notranslate}]{.pre}
    bitstring in **qubit allocation order**.

For example, a kernel with no measurements:
...
produces [`{`{.code .docutils .literal .notranslate}]{.pre}` `{.code
.docutils .literal .notranslate}[`10:1000`{.code .docutils .literal
.notranslate}]{.pre}` `{.code .docutils .literal .notranslate}[`}`{.code
.docutils .literal .notranslate}]{.pre} --- bit 0 corresponds to the
first allocated qubit.

A kernel where measurements appear in a different order than allocation:
...
produces [`{`{.code .docutils .literal .notranslate}]{.pre}` `{.code
.docutils .literal .notranslate}[`01:1000`{.code .docutils .literal
.notranslate}]{.pre}` `{.code .docutils .literal .notranslate}[`}`{.code
.docutils .literal .notranslate}]{.pre} --- bit 0 is from [`b`{.code
.docutils .literal .notranslate}]{.pre} (measured first), bit 1 is from
[`a`{.code .docutils .literal .notranslate}]{.pre} (measured second).

::: {#named-measurement-registers .section}
#### [12.1.1.1. ]{.section-number}Named measurement registers[¶](#named-measurement-registers "Permalink to this heading"){.headerlink}

When a measurement result is assigned to a named variable, that variable
name becomes a **named register** in the [`sample_result`{.code
.docutils .literal .notranslate}]{.pre}. Each named register holds the
counts for the corresponding measurement independently from the
[`__global__`{.code .docutils .literal .notranslate}]{.pre} register.

In C++, the [`nvq++`{.code .docutils .literal .notranslate}]{.pre}
compiler extracts the variable name from the assignment and embeds it as
the register name in the compiled IR:
...
::: highlight
    auto kernel = []() __qpu__ {
      cudaq::qvector q(2);
      x(q[0]);
      auto val = mz(q[1]);  // creates register "val"
    };

    auto counts = cudaq::sample(kernel);
    // counts.register_names() == {"__global__", "val"}
    // counts.count("0", "val") == 1000
:::
:::

In Python, named registers can be created either by variable assignment
(as in C++) or by passing the [`register_name`{.code .docutils .literal
.notranslate}]{.pre} keyword argument to [`mz`{.code .docutils .literal
.notranslate}]{.pre}:
...
::: highlight
    ...
    def kernel():
        q = cudaq.qvector(2)
        x(q[0])
        val = mz(q[1])  # creates register "val" (via variable name)

    @cudaq.kernel
    def kernel_explicit():
        q = cudaq.qvector(2)
        x(q[0])
        mz(q[1], register_name="val")  # creates register "val" (explicit)
...
The [`__global__`{.code .docutils .literal .notranslate}]{.pre} register
always contains the full concatenated bitstring (following the
measurement-order rule above). Named registers give per-measurement
access to individual results.

::: deprecated
[Deprecated since version Named: ]{.versionmodified
.deprecated}measurement registers in [`cudaq::sample`{.code .docutils
.literal .notranslate}]{.pre} are deprecated. Use [`cudaq::run`{.code
.docutils .literal .notranslate}]{.pre} to retrieve individual
measurement results from each shot.
:::
...
::: {#sample-result-type .section}
### [12.1.2. ]{.section-number}[`sample_result`{.code .docutils .literal .notranslate}]{.pre} type[¶](#sample-result-type "Permalink to this heading"){.headerlink}

**\[4\]** CUDA-Q specifies the following structure for
...
The [`sample_result`{.code .docutils .literal .notranslate}]{.pre} type
tracks a unique global register named [`__global__`{.code .docutils
.literal .notranslate}]{.pre} plus any named measurement registers. The
API provides:

-   [`count(bitString)`{.code .docutils .literal .notranslate}]{.pre}
    --- number of times a bit string was observed.

-   [`to_map()`{.code .docutils .literal .notranslate}]{.pre} --- full
    counts dictionary as [`std::unordered_map`{.code .docutils .literal
    .notranslate}]{.pre}.

-   [`get_marginal(indices)`{.code .docutils .literal
    .notranslate}]{.pre} --- a new [`sample_result`{.code .docutils
    .literal .notranslate}]{.pre} over a subset of measured qubits.

-   [`sequential_data()`{.code .docutils .literal .notranslate}]{.pre}
    --- bit string observations in shot order.

-   [`expectation()`{.code .docutils .literal .notranslate}]{.pre} /
    [`probability(bitString)`{.code .docutils .literal
    .notranslate}]{.pre} --- derived statistics.
:::

::: {#kernel-requirements .section}
### [12.1.3. ]{.section-number}Kernel requirements[¶](#kernel-requirements "Permalink to this heading"){.headerlink}

**\[5\]** The kernel passed to [`sample`{.code .docutils .literal
.notranslate}]{.pre} must be an entry-point kernel that returns
[`void`{.code .docutils .literal .notranslate}]{.pre}. This must be
enforced by compiler implementations.
:::

::: {#asynchronous-sampling .section}
### [12.1.4. ]{.section-number}Asynchronous sampling[¶](#asynchronous-sampling "Permalink to this heading"){.headerlink}

**\[6\]** CUDA-Q also provides [`cudaq::sample_async`{.code .docutils
.literal .notranslate}]{.pre} which returns a
...
[`qpu_id`{.code .docutils .literal .notranslate}]{.pre}. The
[`async_sample_result`{.code .docutils .literal .notranslate}]{.pre}
wraps a [`std::future<sample_result>`{.code .docutils .literal
.notranslate}]{.pre} and exposes [`get()`{.code .docutils .literal
.notranslate}]{.pre} to retrieve results after asynchronous execution.

For remote QPU systems with long queue times, the
[`async_sample_result`{.code .docutils .literal .notranslate}]{.pre}
type encodes job ID information and can be persisted to file and loaded
at a later time. After loading from file, and when remote queue jobs are
completed, one can invoke [`get()`{.code .docutils .literal
.notranslate}]{.pre} and the results will be retrieved and returned.
:::

::: {#implementation-note-explicit-measurements .section}
### [12.1.5. ]{.section-number}Implementation note: [`explicit_measurements`{.code .docutils .literal .notranslate}]{.pre}[¶](#implementation-note-explicit-measurements "Permalink to this heading"){.headerlink}

**\[7\]** For each kernel, the runtime computes an internal
[`explicit_measurements`{.code .docutils .literal .notranslate}]{.pre}
flag. This is an execution-planning signal for backend implementations,
not a user semantic override.

When measurements exist in the kernel, the flag tells the backend that
it must preserve user measurement order in the results. When no
measurements exist (or measurements trivially follow allocation order),
the flag tells the backend it can use more efficient strategies such as
buffered-result collection or implicit measure-all.

Earlier releases exposed this flag as a user-controlled runtime option.
This user control is now deprecated and silently ignored --- the runtime
computes the effective behavior automatically. The
[`explicit_measurements`{.code .docutils .literal .notranslate}]{.pre}
field in [`sample_options`{.code .docutils .literal .notranslate}]{.pre}
