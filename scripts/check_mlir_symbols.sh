...
# copy of every MLIR/LLVM symbol in the process. That only holds if libcudaqMLIR
# exports everything the consumers reference. A gap does not fail the link --
# shared libraries may have undefined symbols -- it fails much later, when the
# dynamic loader gives up:
#
#   ImportError: .../_quakeDialects...so: undefined symbol: _ZN4llvm6SHA256...
#
# in a wheel-validation job hours into CI. This turns that into a build error
# naming the symbol.
#
...
These would fail at load time, not at link time. Add the library that defines
them to cudaq/lib/Optimizer/mlir-libs-allowlist.txt so that libcudaqMLIR
whole-archives it. To find that library:
