...
# Need to know the top-level of the CUDAQ repo
working_dir=`pwd`
...
if [ ! "$?" -eq "0" ] || [ ! -d "$build_include_dir" ] || [ "${force_update,,}" = "python" ] || [ "${force_update,,}" = "py" ]; then
    ...
    if [ ! "$?" -eq "0" ] || [ ! "$cudaq_build_exit_code" -eq "0" ]; then
        echo "Failed to build and install the CUDAQ Python package needed for docs generation."
...
echo "Building CUDAQ documentation using Sphinx..."
cd "$repo_root/docs"
