...
struct GetMeasureOpRewrite : public OpConversionPattern<quake::GetMeasureOp> {
  ...
  matchAndRewrite(quake::GetMeasureOp getMeas, OpAdaptor adaptor,
    ...
    Value index;
    if (!adaptor.getIndex()) {
      index = arith::ConstantIntOp::create(rewriter, loc, getMeas.getRawIndex(),
                                           64);
    } else {
      index = adaptor.getIndex();
      if (isa<IndexType>(index.getType())) {
        index = arith::IndexCastOp::create(rewriter, loc, i64Ty, index);
    ...
    auto call =
        func::CallOp::create(rewriter, loc, TypeRange{ptrResultTy},
...
struct DiscriminateOpRewrite
  ...
  matchAndRewrite(quake::DiscriminateOp disc, OpAdaptor adaptor,
    ...
    if (isa<cudaq::cc::StdvecType>(disc.getResult().getType())) {
      ...
      Value arraySize =
          func::CallOp::create(rewriter, loc, i64Ty,
                               cudaq::opt::QIRArrayGetSize, ValueRange{m})
              .getResult(0);
      Value buff =
          cudaq::cc::AllocaOp::create(rewriter, loc, bufElemTy, arraySize);
      ...
      cudaq::opt::factory::createInvariantLoop(
          ...
          [&](OpBuilder &builder, Location loc, Region &, Block &block) {
            ...
            Value elemPtr =
                func::CallOp::create(builder, loc, ptrResultTy,
                                     cudaq::opt::QIRResultArrayGetElementPtr1d,
                                     ValueRange{m, iv})
                    .getResult(0);
            Value resultVal = cudaq::cc::LoadOp::create(builder, loc, elemPtr);
            Value bitPtr =
                cudaq::cc::CastOp::create(builder, loc, i1PtrTy, resultVal);
            Value bit = cudaq::cc::LoadOp::create(builder, loc, bitPtr);
            Value addr = cudaq::cc::ComputePtrOp::create(
                builder, loc, cudaq::cc::PointerType::get(bufElemTy), buff, iv);
            Value stored =
                (i1Ty != bufElemTy)
                    ? cudaq::cc::CastOp::create(builder, loc, bufElemTy, bit,
                                                cudaq::cc::CastOpMode::Unsigned)
                          .getResult()
    ...
    if (auto intTy = dyn_cast<IntegerType>(origResTy);
      ...
      auto bytePtr = cudaq::cc::CastOp::create(rewriter, loc, i8PtrTy, m);
      Value byteVal = cudaq::cc::LoadOp::create(rewriter, loc, bytePtr);
      loaded = cudaq::cc::CastOp::create(rewriter, loc, origResTy, byteVal);
    } else {
      auto ptrCast = cudaq::cc::CastOp::create(rewriter, loc, i1PtrTy, m);
      loaded = cudaq::cc::LoadOp::create(rewriter, loc, ptrCast);
    }
...
struct DiscriminateOpToCallRewrite
  ...
  matchAndRewrite(quake::DiscriminateOp disc, OpAdaptor adaptor,
    ...
    if constexpr (M::discriminateToClassical) {
      ...
      auto call = func::CallOp::create(rewriter, loc, i1Ty, readFn,
                                       adaptor.getOperands());
      ...
      if (auto intTy = dyn_cast<IntegerType>(origResTy);
        ...
        auto i8Ty = rewriter.getI8Type();
        auto i8PtrTy = cudaq::cc::PointerType::get(i8Ty);
        auto du = cudaq::cc::CastOp::create(rewriter, loc, i8PtrTy, unu);
        Value byteVal = cudaq::cc::LoadOp::create(rewriter, loc, du);
        loaded = cudaq::cc::CastOp::create(rewriter, loc, origResTy, byteVal);
      } else {
        auto ptrI1Ty = cudaq::cc::PointerType::get(i1Ty);
        auto du = cudaq::cc::CastOp::create(rewriter, loc, ptrI1Ty, unu);
        loaded = cudaq::cc::LoadOp::create(rewriter, loc, du);
      }
    }
    auto origResTy = disc.getResult().getType();
    if constexpr (M::discriminateToClassical) {
      if (auto intTy = dyn_cast<IntegerType>(origResTy);
          intTy && intTy.getWidth() > 1)
        loaded = arith::ExtUIOp::create(rewriter, loc, origResTy, loaded);
    }
    rewriter.replaceOp(disc, loaded);
    return success();
  }
...
private:
  ...
  rewriteMultiQubitMeasurement(quake::MzOp mz, OpAdaptor adaptor,
    ...
    Value totalQubits = arith::ConstantIntOp::create(rewriter, loc, 0, 64);
    for (auto [origTarget, convTarget] :
         llvm::zip(mz.getTargets(), adaptor.getTargets())) {
      if (isa<quake::RefType>(origTarget.getType())) {
        Value one = arith::ConstantIntOp::create(rewriter, loc, 1, 64);
        totalQubits = arith::AddIOp::create(rewriter, loc, totalQubits, one);
        veqSizes.push_back(Value{});
      } else {
        Value sz = func::CallOp::create(rewriter, loc, i64Ty,
                                        cudaq::opt::QIRArrayGetSize,
                                        ValueRange{convTarget})
                       .getResult(0);
        totalQubits = arith::AddIOp::create(rewriter, loc, totalQubits, sz);
        veqSizes.push_back(sz);
      }
    }

    // Allocate the result array.
    Value resultArray = func::CallOp::create(rewriter, loc, arrayTy,
                                             cudaq::opt::QIRResultArrayCreate,
    ...
    auto getResultSlot = [&](OpBuilder &builder, Location loc, Value array,
                             Value index) -> Value {
      return func::CallOp::create(builder, loc, ptrResultTy,
                                  cudaq::opt::QIRResultArrayGetElementPtr1d,
    ...
    for (auto [origTarget, convTarget] :
      ...
      if (isa<quake::RefType>(origTarget.getType())) {
        ...
        Value result =
            func::CallOp::create(rewriter, loc, resultTy, functionName, mzArgs)
                .getResult(0);
        Value slot = getResultSlot(rewriter, loc, resultArray, offset);
        cudaq::cc::StoreOp::create(rewriter, loc, result, slot);
        ...
        cudaq::opt::factory::createInvariantLoop(
            ...
            [&](OpBuilder &builder, Location loc, Region &, Block &block) {
              Value iv = block.getArgument(0);
              Value qubitPtr =
                  func::CallOp::create(builder, loc, ptrQubitTy,
                                       cudaq::opt::QIRArrayGetElementPtr1d,
                                       ValueRange{convTarget, iv})
                      .getResult(0);
              Value qubit = cudaq::cc::LoadOp::create(builder, loc, qubitPtr);
              SmallVector<Value> mzArgs{qubit};
              if (cstringGlobal)
                mzArgs.push_back(cstringGlobal);
              Value result = func::CallOp::create(builder, loc, resultTy,
                                                  functionName, mzArgs)
                                 .getResult(0);
              Value idx = arith::AddIOp::create(builder, loc, savedOffset, iv);
              Value slot = getResultSlot(builder, loc, resultArray, idx);
              cudaq::cc::StoreOp::create(builder, loc, result, slot);
            });
        offset = arith::AddIOp::create(rewriter, loc, offset, veqSize);
      }
    }
