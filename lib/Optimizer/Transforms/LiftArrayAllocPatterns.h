...
public:
  ...
  explicit AllocaPattern(mlir::MLIRContext *ctx, mlir::DominanceInfo &di, llvm::StringRef fn)
      : OpRewritePattern(ctx), dom(di), funcName(fn) {}

  mlir::LogicalResult matchAndRewrite(cudaq::cc::AllocaOp alloc,
                                mlir::PatternRewriter &rewriter) const override;

  // Determine if \p alloc is a legit candidate for promotion to a constant
  // array value. \p scoreboard is a vector of store mlir::Operations. Each element of
