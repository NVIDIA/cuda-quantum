...
bool QuakeBridgeVisitor::VisitVarDecl(clang::VarDecl *x) {
  ...
  if (auto vecType = dyn_cast<cc::StdvecType>(type)) {
    ...
    if (x->getInit()) {
      ...
      // interface directly) shapes; the spec allows either.
      ...
      // Assign registerName. Handle two producers: (a) a
      // `quake.discriminate` that consumes a measurement-interface
      // operand (`bool b = mz(q);` shape), and (b) a direct measurement
      // interface (`auto h = mz(q);` shape -- new spec).
      auto attachName = [&](quake::MeasurementInterface meas) {
      ...
      if (auto descr = initVec.getDefiningOp<quake::DiscriminateOp>()) {
        if (auto meas = descr.getMeasurement()
          ...
          attachName(meas);
                     ...
                     initVec.getDefiningOp<quake::MeasurementInterface>()) {
        attachName(meas);
      }
  ...
  // shapes reach here: `bool b = mz(q);` (discriminate-of-measure) and
  // `auto h = mz(q);` (measure-interface directly; new
  // `measure_handle` spec). Both route the same register name through
  // to the underlying measurement op via `MeasurementInterface`.
  auto attachName = [&](quake::MeasurementInterface meas) {
    meas.setRegisterName(builder.getStringAttr(x->getName()));
  };
  if (auto discr = initValue.getDefiningOp<quake::DiscriminateOp>()) {
    if (auto meas =
            discr.getMeasurement().getDefiningOp<quake::MeasurementInterface>())
      attachName(meas);
  } else if (auto meas =
                 initValue.getDefiningOp<quake::MeasurementInterface>()) {
    attachName(meas);
  }
