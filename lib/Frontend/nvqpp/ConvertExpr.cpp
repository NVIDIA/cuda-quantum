    ...
    if (funcName == "slice_vector") {
      ...
      if (eleTy == builder.getI1Type()) {
        ...
        auto scale = arith::ConstantIntOp::create(
            builder, loc, args[1].getType(), (int64_t)((bits + 7) / 8));
      ...
      auto devCall = [&]() {
        if (maybeGPULaunchParams) {
          ...
          return cc::DeviceCallOp::create(
              builder, loc, TypeRange(devFuncTy.getResults()), symbol,
  ...
  if (call.getNumResults() > 0) {
    ...
    if (auto vecTy =
      ...
      func::CallOp::create(builder, loc, TypeRange{},
                           "__nvqpp_vectorCopyToStack", ValueRange(copyArgs));
      Value newSpan =
...
bool QuakeBridgeVisitor::VisitCXXConstructExpr(clang::CXXConstructExpr *x) {
  ...
  if (isInNamespace(ctor, "cudaq")) {
          ...
          if (auto arrTy = dyn_cast<cc::ArrayType>(ptrTy.getElementType())) {
            if (arrTy.isUnknownSize()) {
              ...
              numQubits = arith::ConstantIntOp::create(
                  builder, loc, std::countr_zero(arraySize), 64);
            }
          }
    ...
    if (ctorName == "complex") {
      Value imag = popValue();
      Value real = popValue();
      return pushValue(complex::CreateOp::create(
