    ...
    if (auto vMeasTy = dyn_cast<quake::MeasurementsType>(vTy))
      if (auto kMeasTy = dyn_cast<quake::MeasurementsType>(kTy))
        if (vMeasTy.hasSpecifiedSize() && !kMeasTy.hasSpecifiedSize()) {
          auto relax = quake::RelaxSizeOp::create(builder, loc, kMeasTy, v);
          result.push_back(relax);
          continue;
        }
...
bool QuakeBridgeVisitor::VisitCastExpr(clang::CastExpr *x) {
  ...
  case clang::CastKind::CK_UserDefinedConversion: {
    ...
    if (sub.getType() == measTy) {
      auto i1Val = quake::DiscriminateOp::create(builder, loc, i1Type, sub);
      // Convert to `int`
      if (isa<IntegerType>(castToTy))
        return pushValue(cudaq::cc::CastOp::create(
            builder, loc, castToTy, i1Val, cudaq::cc::CastOpMode::Unsigned));
      // Convert to `float`
    ...
    if (isa<quake::MeasurementsType>(sub.getType()))
      return pushValue(quake::DiscriminateOp::create(
          builder, loc, cc::StdvecType::get(i1Type), sub));
    ...
    if (isa<quake::MeasurementsType>(svec.getType()) && funcName == "size")
      if (auto memberCall = dyn_cast<clang::CXXMemberCallExpr>(x))
        if (memberCall->getImplicitObjectArgument()) {
          [[maybe_unused]] auto calleeTy = popType();
          assert(isa<FunctionType>(calleeTy));
          return pushValue(quake::MeasurementsSizeOp::create(
              builder, loc, builder.getI64Type(), svec));
        }
    auto ext =
    ...
    if ((opKind == clang::OO_EqualEqual || opKind == clang::OO_ExclaimEqual) &&
      ...
      if (lhs.getType() == measTy || rhs.getType() == measTy) {
        ...
        if (lhs.getType() == measTy)
          lhs = quake::DiscriminateOp::create(builder, loc, i1Type, lhs);
        if (rhs.getType() == measTy)
          rhs = quake::DiscriminateOp::create(builder, loc, i1Type, rhs);
  ...
  if (isInNamespace(func, "cudaq")) {
    ...
    if (funcName == "toInteger" || funcName == "to_integer") {
      ...
      if (isa<quake::MeasurementsType>(arg.getType()))
        arg = quake::DiscriminateOp::create(builder, loc, boolVecTy, arg);
      ...
      auto i64Ty = builder.getI64Type();
      return pushValue(func::CallOp::create(builder, loc, i64Ty,
    ...
    if (funcName == "to_bool_vector") {
      ...
      auto i1Ty = builder.getI1Type();
      arg = quake::DiscriminateOp::create(builder, loc,
                                          cc::StdvecType::get(i1Ty), arg);
  ...
  if (auto typeNameOpt = isInterceptedSubscriptOperator(x)) {
    ...
    if (typeName == "vector") {
      ...
      if (isa<quake::MeasurementsType>(svec.getType())) {
        auto getMeas =
            quake::GetMeasureOp::create(builder, loc, svec, indexVar);
