...
bool QuakeBridgeVisitor::VisitCallExpr(clang::CallExpr *x) {
  ...
  if (isInNamespace(func, "cudaq")) {
    ...
    if (isInNamespace(func, "driver")) {
      auto *ctx = builder.getContext();
      auto i64Ty = builder.getI64Type();
      Type devPtrTy = cudaq::cc::StructType::get(
          ctx, "device_ptr", ArrayRef<Type>{i64Ty, i64Ty, i64Ty});
      if (funcName == "malloc") {
        IRBuilder irBuilder(ctx);
        [[maybe_unused]] auto result =
            irBuilder.loadIntrinsic(module, "__nvqpp_driver_malloc");
        auto tmp = builder.create<cudaq::cc::AllocaOp>(loc, devPtrTy);
        SmallVector<Value> mallocArgs{tmp};
        Type ty;
        if (func->isFunctionTemplateSpecialization()) {
          // Traversal should have pushed an extra type that corresponds to the
          // template type argument.
          assert(x->getNumArgs() == 0 && "size implied by template argument");
          ty = extraType;
          extraType = Type{};
          mallocArgs.push_back(
              builder.create<cudaq::cc::SizeOfOp>(loc, i64Ty, ty));
        } else {
          assert(x->getNumArgs() == 1 && "user must specify the size");
          mallocArgs.push_back(args[0]);
        }
        builder.create<func::CallOp>(loc, TypeRange{}, "__nvqpp_driver_malloc",
                                     mallocArgs);
        return pushValue(builder.create<cudaq::cc::LoadOp>(loc, tmp));
      }
      if (funcName == "malloc_set") {
        IRBuilder irBuilder(ctx);
        [[maybe_unused]] auto result =
            irBuilder.loadIntrinsic(module, "__nvqpp_driver_malloc_set");
        auto tmp = builder.create<cudaq::cc::AllocaOp>(loc, devPtrTy);
        SmallVector<Value> mallocArgs{tmp};
        const int initValOff = 0;
        auto tmp2 = builder.create<cudaq::cc::AllocaOp>(
            loc, args[initValOff].getType());
        builder.create<cudaq::cc::StoreOp>(loc, args[initValOff], tmp2);
        auto castTmp = builder.create<cudaq::cc::CastOp>(
            loc, cudaq::cc::PointerType::get(builder.getI8Type()), tmp2);
        mallocArgs.push_back(castTmp);
        auto size = builder.create<cudaq::cc::SizeOfOp>(
            loc, i64Ty, args[initValOff].getType());
        mallocArgs.push_back(size);
        mallocArgs.push_back(args[1]);
        builder.create<func::CallOp>(loc, TypeRange{},
                                     "__nvqpp_driver_malloc_set", mallocArgs);
        return pushValue(builder.create<cudaq::cc::LoadOp>(loc, tmp));
      }
      if (funcName == "memcpy") {
        IRBuilder irBuilder(ctx);
        [[maybe_unused]] auto result =
            irBuilder.loadIntrinsic(module, "__nvqpp_driver_memcpy_send");
        builder.create<func::CallOp>(loc, TypeRange{},
                                     cudaq::cc::isDevicePtr(args[0].getType())
                                         ? "__nvqpp_driver_memcpy_send"
                                         : "__nvqpp_driver_memcpy_receive",
                                     args);
        return true;
      }
      if (funcName == "free") {
        IRBuilder irBuilder(ctx);
        [[maybe_unused]] auto result =
            irBuilder.loadIntrinsic(module, "__nvqpp_driver_free");
        builder.create<func::CallOp>(loc, TypeRange{}, "__nvqpp_driver_free",
                                     args);
        return true;
      }
    }
