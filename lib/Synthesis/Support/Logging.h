...
/// (CUDAQ_SYNTH_LOG_LEVEL, CUDAQ_SYNTH_LOG_FILE, CUDAQ_SYNTH_LOG_FORMAT) which are applied
/// on top of whatever is passed to init().
...
/// first CUDAQ_SYNTH_LOG_* call, which will auto-initialize with defaults if init()
...
///
/// Thread-safe.  Called on every CUDAQ_SYNTH_LOG_* invocation; the fast path is a
/// single hash-map lookup after initialization.
