...
//
//   CUDAQ_SYNTH_LOG_INFO("synth.gridsynth", "theta={:.6f}, eps={:.2e}", theta, eps);
//   CUDAQ_SYNTH_LOG_DEBUG("synth.grid", "solve_tdgp: k={}", k);
...
//
// Guidelines:
//   CRITICAL / ERROR   Algorithm failures (degenerate inputs, unsolvable cases).
//   WARN               Recoverable issues (timeouts, fallbacks, skipped candidates).
//   INFO               High-level progress (function entry/exit, final T-count).
//   DEBUG              Per-iteration details (loop variables, candidate counts).
//   TRACE              Inner-loop diagnostics.  Always compiled out in release
//                      builds when CUDAQ_SYNTH_LOGGING_LEVEL=INFO or above is set.
//
