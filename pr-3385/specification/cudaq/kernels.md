...
CUDA-Q requires the `__qpu__`{.code .docutils .literal .notranslate}
function attribute for quantum kernel declarations. Other language
bindings may opt to use other language features to enable function
annotation or decoration (e.g. a `@cudaq.kernel()`{.code .docutils
.literal .notranslate} function decorator in Python).
...
>     `cudaq::control`{.code .docutils .literal .notranslate})
>
> -   Classical control flow constructs from the classical language
>     (`if`{.code .docutils .literal .notranslate}, `for`{.code
>     .docutils .literal .notranslate}, `while`{.code .docutils .literal
>     .notranslate}, etc.)
>
...
>     variables
>
> -   Coherent conditional execution -
>     `if ( boolExprFromQubitMeasurement ) { x (another_qubit); }`{.code
...
> <div>
>
> -   `T`{.code .docutils .literal .notranslate} such that
>     `std::is_arithmetic_v<T> == true`{.code .docutils .literal
>     .notranslate}
>
> -   `std::vector<T>`{.code .docutils .literal .notranslate} such that
>     `std::is_arithmetic_v<T> == true`{.code .docutils .literal
>     .notranslate}
>
> -   `std::span<T>`{.code .docutils .literal .notranslate} such that
>     `std::is_arithmetic_v<T> == true`{.code .docutils .literal
>     .notranslate}
>
> -   `std::vector<V>`{.code .docutils .literal .notranslate} such that
>     type `V`{.code .docutils .literal .notranslate} is a valid
>     `std::vector<T>`{.code .docutils .literal .notranslate} (possibly
>     recursively)
>
> -   `struct`{.code .docutils .literal .notranslate} types composed of
>     any valid CUDA-Q type.
>
> -   `std::tuple<Ts...>`{.code .docutils .literal .notranslate} such
>     that T is any allowed type.
>
> -   `std::pair<T,U>`{.code .docutils .literal .notranslate} such that
>     T is any allowed type.
>
> </div>
...
`std::vector<T>`{.code .docutils .literal .notranslate} for any allowed
type `T`{.code .docutils .literal .notranslate} can only be constructed
with known size. Vector-like variables cannot be default constructed and
later filled with type `T`{.code .docutils .literal .notranslate} data
...
**\[8\]** CUDA-Q kernel lambdas in C++ can capture variables of allowed
