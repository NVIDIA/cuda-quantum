...
[`1001`{.docutils .literal .notranslate}]{.pre} and [`1111`{.docutils
.literal .notranslate}]{.pre}.
:::
...
\|i\\rangle = \\frac{\\sqrt{t}}{\\sqrt{N}} \|G\\rangle +
\\frac{\\sqrt{N - t}}{\\sqrt{N}} \|B\\rangle.\\\]
:::
...
.notranslate}]{.pre} and [`1111`{.docutils .literal
.notranslate}]{.pre}. Here [\\(\\ket{\\xi} =
\\frac{1}{16}\\sum\_{k=0}\^{15}\\ket{k}\\)]{.math .notranslate
.nohighlight} since there are 4 qubits and 16 computational basis states
with [\\(\\ket{0} = \\ket{0000}, \\ket{1} = \\ket{0001}, \\cdots
\\ket{15} = \\ket{1111}\\)]{.math .notranslate .nohighlight}. The good
state is a equal superposition of the states [\\(\\ket{9}
...
![8f686b31210f436ca9a8a14977a05b14](../../_images/grovers-2D-plane.png)
:::
...
operation [\\(r_B\\)]{.math .notranslate .nohighlight}) and then this
...
>
> ::: {.math .notranslate .nohighlight}
> \\\[r_0 = 2 \|0\^{\\otimes 4} \\rangle \\langle 0\^{\\otimes 4} \| -
> \\text{Id}\\\]
> :::
>
...
.notranslate}]{.pre} to instead reflect about the equal superposition
state. To do this we'll wrap the [`all_zero_reflection`{.docutils
.literal .notranslate}]{.pre} kernel with Hadamard gates which will
transform the qubits from the equal superposition state to the all zero
state and back.
...
we're only left with defining [\\(r_B\\)]{.math .notranslate
...
.nohighlight}, which is exactly what we need for [\\(r_B\\)]{.math
.notranslate .nohighlight}.
:::
...
::: {.math .notranslate .nohighlight}
\\\[\\mathcal{G} = r\_\\xi \\circ r_B = H\^{\\otimes n} \\big(
...
::: {style="text-align: center;"}
![e0cb5b51b9f14d5589987194b9940807](../../_images/grovers-steps1-3.png)
:::
...
::: {style="text-align: center;"}
![cff2210c2b8d48099d0bd48f060708ed](../../_images/grovers-full-rotation.png)
:::
...
[`1001`{.docutils .literal .notranslate}]{.pre} and [`1111`{.docutils
.literal .notranslate}]{.pre} in the resulting state have been amplified
...
Edit the [`num_iterations`{.docutils .literal .notranslate}]{.pre}
variable in the code block below to compute the angle between the
resulting state of [`num_iterations`{.docutils .literal
.notranslate}]{.pre} of [\\(\\mathcal{G}\\)]{.math .notranslate
.nohighlight} and the state [\\(\\ket{G}\\)]{.math .notranslate
.nohighlight} along with the histogram of the resulting sampling
distribution. Notice how increasing the number of iterations beyond a
certain point produces states with lower probability amplitudes of the
marked states than desired. Why might this happen? What number of
iterations results in a better chance of sampling a marked state? How
does this compare from the number of iterations that are prescribed by
the formula: \$ m [\\approx
...
As an exercise, adapt the code to search for a different set of unmarked
