...
.literal .notranslate}]{.pre} command and printed as numpy arrays:
...
to a matrix with [`to_matrix`{.docutils .literal .notranslate}]{.pre}.
The following line of code performs the explicit matrix multiplications
to produce the exact expectation value.
...
## Using [`Sample`{.docutils .literal .notranslate}]{.pre} to perform the Hadamard test[¶](#Using-Sample-to-perform-the-Hadamard-test "Permalink to this heading"){.headerlink}
...
The CUDA-Q [`sample`{.docutils .literal .notranslate}]{.pre} method
computes 100000 sample ancilla measurements, and from them we can
estimate the expectation value. The standard error is provided as well.
Try increasing the sample size and note the convergence of the
expectation value and the standard error towards the numerical result.
...
[`mqpu`{.docutils .literal .notranslate}]{.pre} backend allows you to
simulate a computation across multiple simulated QPUs. The code below
demonstrates how.
...
Next, the [`nvidia-mqpu`{.docutils .literal .notranslate}]{.pre} backend
is specified and the number of GPUs available is determined.
...
The [`sample_async`{.docutils .literal .notranslate}]{.pre} command is
then used to distribute the Hadamard test computations across multiple
simulated QPUs. The results are saved in a list and accessed below.
