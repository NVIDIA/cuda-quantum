...
.notranslate}]{.pre} and two CUDA-Q kernels, one performing an SU4
operation and another building the entire QV circuit. This example is
constructed for an even number of qubits for simplicity.
...
The [`percent_heavy_sampled`{.docutils .literal .notranslate}]{.pre}
function takes the random circuit parameters and permutations and the
error rate and returns the percent of heavy bitstrings produced by a
noisy circuit sample.
...
[`error_rate`{.docutils .literal .notranslate}]{.pre}.

Next, the noiseless simulation is performed on a GPU simulated with the
[`nvidia`{.docutils .literal .notranslate}]{.pre} backend to obtain the
state vector. The [`density-matrix-cpu`{.docutils .literal
.notranslate}]{.pre} backend is used to sample the noisy circuit.
...
[`percent`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal
.notranslate}[`_heavy_sampled`{.docutils .literal .notranslate}]{.pre}
function for [`n_circuit`{.docutils .literal .notranslate}]{.pre}number
of times and prints if the test is passed and returns the average.
