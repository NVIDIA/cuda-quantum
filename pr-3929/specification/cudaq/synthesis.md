...
.notranslate}]{.pre} function takes as input an instantiated pure device
quantum kernel, a std::range of control qubits ([`cudaq::qvector`{.code
.docutils .literal .notranslate}]{.pre} or [`cudaq::qview`{.code
.docutils .literal .notranslate}]{.pre}), and the remaining arguments
...
.notranslate}]{.pre} [[negated polarity
...
.notranslate}]{.pre} function takes as input an pure device quantum
kernel and the remaining arguments for the kernel.
:::
:::
:::
