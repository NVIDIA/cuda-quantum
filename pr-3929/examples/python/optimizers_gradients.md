...
-   [Examples](../../using/examples/examples.html){.reference .internal}
    ...
    -   [Visualizing Kernels](visualization.html){.reference .internal}
        ...
        -   [Kernel
            ...
            .internal}
        -   [Interactive Circuit
            Builder](visualization.html#Interactive-Circuit-Builder){.reference
            .internal}
    -   [Executing
...
shift gradient with shift [`epsilon`{.docutils .literal
.notranslate}]{.pre}. The first step of the function builds
[`xplus`{.docutils .literal .notranslate}]{.pre} and [`xminus`{.docutils
.literal .notranslate}]{.pre} , arrays consisting of the shifted
parameters.

Next, a for loop iterates over all of the parameters and uses the
[`cudaq.observe_async`{.docutils .literal .notranslate}]{.pre} to
compute the expectation value. This command also takes
[`qpu_id`{.docutils .literal .notranslate}]{.pre} as an in out which
specifies the GPU that will be used to simulate the ith QPU. In the
example below, four GPUs (simulated QPUs) are available so the gradient
is batched over four devices.

The results are saved in the [`g_plus`{.docutils .literal
.notranslate}]{.pre} and [`g_minus`{.docutils .literal
.notranslate}]{.pre} lists, the elements of which are accessed with
commands like [`g_plus[1].expectation()`{.docutils .literal
.notranslate}]{.pre} to compute the finite differences and construct the
final gradient.
