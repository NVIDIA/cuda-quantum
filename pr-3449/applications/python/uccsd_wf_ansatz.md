...
Here, [\\(T\_1\\)]{.math .notranslate .nohighlight} and
[\\(T\_2\\)]{.math .notranslate .nohighlight} represent the single and
double excitation operators, respectively.
...
Note: the
`cudaq.kernels.uccsd(qubits, thetas, electron_count, qubits_num)`{.docutils
.literal .notranslate} available in cudaq.kernels uses cnot and rotation
gates to implement the exponential of pauli operators in the UCCSD, see
this
...
`exp_pauli()`{.docutils .literal .notranslate} gate available in CUDA-Q.
Using `exp_pauli()`{.docutils .literal .notranslate} gate instead of
decomposing it when running on classical simulator reduces the number of
gates dramatically leading to improvements in perfomance.
