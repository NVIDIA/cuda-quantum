...
// RUN: nvq++ %cpp_std -target iqm --iqm-machine "Crystal 5" -emulate -fkernel-exec-kind=2 %s -o %t && %t | FileCheck %s
// RUN: nvq++ %cpp_std -target iqm --iqm-machine "Crystal 20" -emulate %s -fkernel-exec-kind=2 -o %t && %t | FileCheck %s
// clang-format on
