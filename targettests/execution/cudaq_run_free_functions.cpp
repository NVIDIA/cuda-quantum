...
bar_test_result bar_test(int x) __qpu__ {
  cudaq::qvector q(5);
  h(q[0]);
  for (int i = 1; i < 5; i++) {
    cx(q[i - 1], q[i]);
...
baz_test_result baz_test(int x) __qpu__ {
  cudaq::qvector q(5);
  h(q[0]);
  for (int i = 1; i < 5; i++) {
    cx(q[i - 1], q[i]);
...
int main() {
  ...
  {
    const auto results = cudaq::run(4, bar_test, 2 * 3);
    for (auto i : results)
      printf("%d\t", (int)i.a);
    printf("\n");
  }
  {
    const auto results = cudaq::run(3, baz_test, 4 - 3);
    for (auto i : results)
