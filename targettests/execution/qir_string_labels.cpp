...
// clang-format off
// RUN: nvq++ %cpp_std -v %s -o %t -fno-enable-cudaq-run --target quantinuum --emulate && CUDAQ_DUMP_JIT_IR=1 %t |& FileCheck --check-prefixes=CHECK,QIR_ADAPTIVE %s
// RUN: nvq++ %cpp_std -v %s -o %t -fno-enable-cudaq-run --target ionq --emulate && CUDAQ_DUMP_JIT_IR=1 %t |& FileCheck --check-prefixes=CHECK,IONQ %s
// RUN: if %qci_avail; then nvq++ %cpp_std -v %s -o %t -fno-enable-cudaq-run --target qci --emulate && CUDAQ_DUMP_JIT_IR=1 %t |& FileCheck --check-prefixes=CHECK,QIR_ADAPTIVE %s; fi
