...
// RUN: nvq++ %cpp_std -fkernel-exec-kind=2 -target iqm --iqm-machine "Crystal 5" --emulate %s -o %t && %t | FileCheck %s
// RUN: nvq++ %cpp_std -fkernel-exec-kind=2 -target iqm --iqm-machine "Crystal 20" --emulate %s -o %t && %t | FileCheck %s
// clang-format on
