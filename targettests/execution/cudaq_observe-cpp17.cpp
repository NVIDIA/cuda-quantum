...
// REQUIRES: c++17
// clang-format off
// RUN: if %infleqtion_avail; then nvq++ %cpp_std --target infleqtion               --emulate %s -o %t && %t | FileCheck %s; fi
...
// RUN: if %quantinuum_avail; then nvq++ %cpp_std --target quantinuum               --emulate %s -o %t && %t | FileCheck %s; fi
// clang-format on
