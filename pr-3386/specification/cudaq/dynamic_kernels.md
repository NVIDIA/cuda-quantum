...
.notranslate} abstraction facilitates the dynamic definition of quantum
kernels that can optionally be parameterized by user-defined input
arguments.

The `kernel_builder`{.code .docutils .literal .notranslate} takes the
following structure
...
factory functions (`make_kernel`{.code .docutils .literal .notranslate})
...
new `Value`{.code .docutils .literal .notranslate} instance which can be
leveraged in the construction of the kernel. The intended mechanism for
