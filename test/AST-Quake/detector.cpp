...
struct testDetectorVecWithLoad {
  int operator()() __qpu__ {
    cudaq::qubit q0, q1;
    mz(q0);
    mz(q1);
    std::vector<std::int64_t> indices = {-1, -2};
    cudaq::detector(indices);
    return indices[0];
  }
};

// clang-format off
// CHECK-LABEL:   func.func @__nvqpp__mlirgen__testDetectorVecWithLoad() -> i32 attributes {"cudaq-entrypoint", "cudaq-kernel"} {
...
// CHECK:           %[[VAL_5:.*]] = quake.mz %[[VAL_3]] : (!quake.ref) -> !quake.measure
// CHECK:           %[[VAL_6:.*]] = quake.mz %[[VAL_4]] : (!quake.ref) -> !quake.measure
// CHECK:           %[[VAL_7:.*]] = cc.alloca !cc.array<i64 x 2>
// CHECK-ALIVE:     %[[VAL_8:.*]] = cc.cast %[[VAL_7]] : (!cc.ptr<!cc.array<i64 x 2>>) -> !cc.ptr<!cc.array<i64 x ?>>
// CHECK:           %[[VAL_9:.*]] = cc.cast %[[VAL_7]] : (!cc.ptr<!cc.array<i64 x 2>>) -> !cc.ptr<i64>
// CHECK:           cc.store %[[VAL_1]], %[[VAL_9]] : !cc.ptr<i64>
// CHECK:           %[[VAL_10:.*]] = cc.compute_ptr %[[VAL_7]][1] : (!cc.ptr<!cc.array<i64 x 2>>) -> !cc.ptr<i64>
