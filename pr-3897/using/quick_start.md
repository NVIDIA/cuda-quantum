...
be installed. We recommend using [[`conda`{.docutils .literal
.notranslate}]{.pre}](https://docs.conda.io/en/latest/){.reference
.external} to do so. If you are not already using [`conda`{.docutils
.literal .notranslate}]{.pre}, you can install a minimal version
following [[`miniconda`{.docutils .literal .notranslate}]{.pre}
instructions here](https://docs.anaconda.com/miniconda/){.reference
.external}. The following commands will create and activate a complete
environment for CUDA-Q with all its dependencies:
...
errors, please switch to one of the Python versions 3.10.15, 3.11.10, or
3.12.7 as per the issue
