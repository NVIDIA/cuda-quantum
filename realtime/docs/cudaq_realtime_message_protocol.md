...
This document defines the RPC (Remote Procedure Call) payload encoding used by the realtime dispatch kernel for processing data and returning results. It complements
`cudaq_realtime_host_api.bs`, which focuses on wiring and API usage.
...
[cudaq_realtime_host_api.md](cudaq_realtime_host_api.md) provides implementation details for both GPU and 
...
See [cudaq_realtime_host_api.md](cudaq_realtime_host_api.md) for function ID hashing, handler naming, and function
...
Schema type descriptor (see [cudaq_realtime_host_api.md](cudaq_realtime_host_api.md) for full definition):
