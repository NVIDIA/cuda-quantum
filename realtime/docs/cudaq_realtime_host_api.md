# CUDA-Q Realtime Host API # {#host-api}
...
## Schema Data Structures # {#schema-structures}
...
## RPC Messaging Protocol # {#rpc-protocol}
...
## Host API Overview # {#api-overview}

Header: `realtime/include/cudaq/nvqlink/daemon/dispatcher/cudaq_realtime.h`

## Manager and Dispatcher Topology # {#manager-dispatcher}
...
## Host API Functions # {#api-functions}

Function usage:

<strong><code>cudaq_dispatch_manager_create</code></strong> creates the top-level manager that owns
dispatchers.
...
<strong><code>cudaq_dispatch_manager_destroy</code></strong> releases the manager and any internal
resources.
...
<strong><code>cudaq_dispatcher_create</code></strong> allocates a dispatcher instance and validates the
configuration.
...
<strong><code>cudaq_dispatcher_destroy</code></strong> releases a dispatcher after it has been stopped.
...
<strong><code>cudaq_dispatcher_set_ringbuffer</code></strong> provides the RX/TX flag and data
...
<strong><code>cudaq_dispatcher_set_function_table</code></strong> supplies the function table
...
<strong><code>cudaq_dispatcher_set_control</code></strong> supplies the shutdown flag and stats buffer
...
<strong><code>cudaq_dispatcher_set_launch_fn</code></strong> provides the host-side launch wrapper that
...
<strong><code>cudaq_dispatcher_start</code></strong> launches the persistent dispatch kernel and begins
processing slots.
...
<strong><code>cudaq_dispatcher_stop</code></strong> signals the dispatch kernel to exit and waits for it
to shut down.
...
<strong><code>cudaq_dispatcher_get_processed</code></strong> reads the processed‑packet counter from the
...
### Occupancy Query and Eager Module Loading # {#occupancy-query}
...
<strong><code>cudaq_create_dispatch_graph_regular</code></strong> creates a graph-based dispatch context that enables device-side graph launching.
...
<strong><code>cudaq_launch_dispatch_graph</code></strong> launches the dispatch graph to begin processing RPC messages.
...
<strong><code>cudaq_destroy_dispatch_graph</code></strong> destroys the graph context and releases all associated resources.
...
<strong><code>cudaq_launch_dispatch_kernel_regular</code></strong> launches the dispatch kernel in regular (non-cooperative) mode.
...
<strong><code>cudaq_launch_dispatch_kernel_cooperative</code></strong> launches the dispatch kernel in cooperative mode.
...
## Memory Layout and Ring Buffer Wiring # {#memory-layout}
...
## Step-by-Step: Wiring the Host API (Minimal) # {#wiring}
...
## Device Handler and Function ID # {#device-handler}
...
## CUDA Graph Dispatch Mode # {#graph-dispatch}
...
## Reading the Response # {#read-response}
...
## Schema-Driven Argument Parsing # {#schema-parsing}
...
## Hololink 3-Kernel Workflow (Primary) # {#hololink-workflow}
...
## NIC-Free Testing (No Hololink / No ConnectX-7) # {#nic-free}
