...
static cudaq_status_t validate_dispatcher(cudaq_dispatcher_t* dispatcher) {
  ...
  if (!dispatcher->launch_fn || !dispatcher->shutdown_flag || !dispatcher->stats)
    return CUDAQ_ERR_INVALID_ARG;
...
cudaq_status_t cudaq_dispatcher_start(cudaq_dispatcher_t* dispatcher) {
  ...
  dispatcher->launch_fn(
      dispatcher->ringbuffer.rx_flags,
      dispatcher->ringbuffer.tx_flags,
      dispatcher->table.device_function_ptrs,
      dispatcher->table.function_ids,
      dispatcher->table.count,
      dispatcher->shutdown_flag,
      dispatcher->stats,
      dispatcher->config.num_slots,
      dispatcher->config.num_blocks,
...
cudaq_status_t cudaq_dispatcher_get_processed(
  ...
  if (cudaMemcpy(out_packets, dispatcher->stats,
                 sizeof(uint64_t), cudaMemcpyDeviceToHost) != cudaSuccess)
    return CUDAQ_ERR_CUDA;
