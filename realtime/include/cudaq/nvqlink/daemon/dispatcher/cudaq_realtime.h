...
typedef struct {
  void** device_function_ptrs; // device pointer to array of device function ptrs
...
typedef void (*cudaq_dispatch_launch_fn_t)(
    volatile uint64_t* rx_flags,
    volatile uint64_t* tx_flags,
    void** function_table,
...
// Manager lifecycle
cudaq_status_t cudaq_dispatch_manager_create(cudaq_dispatch_manager_t** out_mgr);
cudaq_status_t cudaq_dispatch_manager_destroy(cudaq_dispatch_manager_t* mgr);

// Dispatcher lifecycle
cudaq_status_t cudaq_dispatcher_create(
    cudaq_dispatch_manager_t* mgr,
...
// Wiring inputs
cudaq_status_t cudaq_dispatcher_set_ringbuffer(
    cudaq_dispatcher_t* dispatcher,
    const cudaq_ringbuffer_t* ringbuffer);
cudaq_status_t cudaq_dispatcher_set_function_table(
    cudaq_dispatcher_t* dispatcher,
    const cudaq_function_table_t* table);
cudaq_status_t cudaq_dispatcher_set_control(
    cudaq_dispatcher_t* dispatcher,
    volatile int* shutdown_flag,
    uint64_t* stats);
...
// Stats
cudaq_status_t cudaq_dispatcher_get_processed(
    cudaq_dispatcher_t* dispatcher,
