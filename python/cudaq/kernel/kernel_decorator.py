...
class PyKernelDecorator(object):
    ...
    def __init__(
        ...
        if is_deserializing:
            ...
            self.location = ((
                inspect.getfile(self.kernelFunction),
        ...
        if self.kernelFunction is None and not is_deserializing:
            if self.module is not None:
                ...
                if nvqppPrefix + self.name in symbols:
                    ...
                    self.returnType = (self.signature["return"]
                                       if "return" in self.signature else None)
                return
            else:
                emitFatalError(
    ...
    def compile(self):
        ...
        self.dependentCaptures = (extraMetadata["dependent_captures"]
                                  if "dependent_captures" in extraMetadata else
                                  None)
    ...
    def __call__(self, *args):
        ...
        for i, arg in enumerate(args):
            ...
            if (not cc.CallableType.isinstance(mlirType) and
                    mlirType != self.argTypes[i]):
                emitFatalError(
