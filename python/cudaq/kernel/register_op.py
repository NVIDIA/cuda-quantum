...
def register_operation(operation_name:str, num_targets:int, num_params:int, unitary):
    ...
    if isinstance(unitary, np.ndarray):
        if(len(unitary.shape) != unitary.ndim):
            raise RuntimeError("provide a 1D array for the matrix representation in row-major format.")
        matrix = unitary
