...
def add_to_bloch_sphere( psi: cudaq_runtime.State, existing_sphere=None, **kwargs) -> Bloch:
    """ 
        Creates a Bloch sphere representation of the given single-qubit state. If an (optional) existing Bloch sphere object is supplied, 
        then adds the state to the existing Bloch sphere and returns it. The Bloch sphere is created with qutip, and any other keyword args 
        provided are passed directly to the qutip.Bloch() function.  
    ...
    if (not isinstance(existing_sphere,Bloch)) and (not existing_sphere == None):
        existing_sphere = None
        print("Existing sphere object is not a valid Bloch sphere. A new sphere will be created.")

    b = Bloch(**kwargs) if existing_sphere == None else existing_sphere 
    st_rep = np.array(psi)
    if (st_rep.shape == (2,) and isclose( st_rep.dot(st_rep).real , 1.0 )) or (st_rep.shape == (2,2) and isclose(st_rep.trace().real, 1.0)):
        b.add_states(Qobj(st_rep))
    else:
        raise Exception("The provided argument is not a valid single-qubit state or density matrix.")
    return b
...
def show_bloch_sphere(sphere_data=None, ncols = 2, nrows = 1) -> None:
    """
        ...
        Returns:
        --------
            Nothing. Displays the figure in the window. This functionality is typically aimed at jupyter/IPython notebook environments.
    """
    if sphere_data==None:
        ...
        if isinstance(sphere_data, Bloch):
            sphere_data.show()
        elif isinstance(sphere_data,list) and isinstance(sphere_data[0],Bloch):
            if (not nrows*ncols >= len(sphere_data)) and nrows > 1:
                raise Exception(f"Incompatible number of rows and columns for sphere_data. Please make sure that nrows*ncols={len(sphere_data)} if nrows >=1.")
            fig, axList = plt.subplots(nrows = max(1,nrows),ncols= min(len(sphere_data), ncols),subplot_kw={'axes_class':Axes3D})
            ...
            raise TypeError(f" Expected Bloch sphere (or a list of bloch spheres), got {type(sphere_data)}. run the add_to_bloch_sphere() to add a statevector to the bloch sphere before showing it. ")
    return
