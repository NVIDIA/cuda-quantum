...
class PerfTrace:
    ...
    def dump():
        # Sort by total time
        sorted_data = sorted(PerfTrace.__data.items(), key=lambda x: sum(x[1]), reverse=True)
        ...
        for func_name, elapsed_times in sorted_data:
            print(f"{func_name}\t{sum(elapsed_times)}\t{len(elapsed_times)}\t{sum(elapsed_times) / len(elapsed_times):.6f}")
            print()
