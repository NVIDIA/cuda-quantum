...
# Under the single-`measure_handle` API, `mz` returns a `!cc.measure_handle`,
# `quake.discriminate` is inserted at every `bool`-coercion site (here the `==`
# in `Flag == True`), and the `if`/`else` branches each carry their `mz` out as
# an SSA value through `cc.if` (the unused side defaults to `cc.undef`).
