...
# CHECK:           %[[VAL_6:.*]] = cc.alloca !cc.array<i8 x 5>
# CHECK:           %[[VAL_7:.*]] = cc.loop while ((%[[VAL_8:.*]] = %{{.*}}) -> (i64)) {
# CHECK:           } do {
# CHECK:             %[[VAL_13:.*]] = cc.compute_ptr %[[VAL_6]][%{{.*}}] : (!cc.ptr<!cc.array<i8 x 5>>, i64) -> !cc.ptr<i8>
# CHECK:             cc.store %{{.*}}, %[[VAL_13]] : !cc.ptr<i8>
# CHECK:           } step {
# CHECK:           } {invariant}
# CHECK:           %[[VAL_18:.*]]:2 = cc.loop while ((%[[VAL_19:.*]] =
# CHECK-SAME: ) -> (i64, i1)) {
# CHECK:           } do {
# CHECK:             %[[VAL_23:.*]] = cc.compute_ptr %[[VAL_6]][%{{.*}}] : (!cc.ptr<!cc.array<i8 x 5>>, i64) -> !cc.ptr<i8>
# CHECK:             %[[VAL_24:.*]] = cc.load %[[VAL_23]] : !cc.ptr<i8>
# CHECK:             %[[VAL_26:.*]] = cc.if(%{{.*}}) -> i1 {
# CHECK:               cc.continue %{{.*}} : i1
# CHECK:             } else {
# CHECK:               cc.continue %{{.*}} : i1
# CHECK:             }
# CHECK:             cc.continue %{{.*}}, %[[VAL_26]] : i64, i1
# CHECK:           } step {
# CHECK:           return %{{.*}} : i1
...
# CHECK:           %[[VAL_5:.*]] = cc.alloca !cc.array<f64 x 5>
# CHECK:           %[[VAL_6:.*]] = cc.loop while ((%[[VAL_7:.*]] =
# CHECK-SAME: ) -> (i64)) {
# CHECK:           } do {
# CHECK:             %[[VAL_12:.*]] = cc.compute_ptr %[[VAL_5]][%{{.*}}] : (!cc.ptr<!cc.array<f64 x 5>>, i64) -> !cc.ptr<f64>
# CHECK:           } step {
# CHECK:           } {invariant}
# CHECK:           %[[VAL_25:.*]]:2 = cc.loop while ((%[[VAL_26:.*]] =
# CHECK-SAME: ) -> (i64, f64)) {
# CHECK:           } do {
# CHECK:             %[[VAL_31:.*]] = cc.compute_ptr %[[VAL_5]][%{{.*}}]
...
# CHECK-SAME: () -> f64 attributes {"cudaq-entrypoint", "cudaq-kernel"} {
# CHECK:           %[[VAL_5:.*]] = cc.alloca !cc.array<complex<f64> x 5>
# CHECK:           %[[VAL_6:.*]] = cc.loop while ((%[[VAL_7:.*]] =
# CHECK-SAME: ) -> (i64)) {
# CHECK:           } do {
# CHECK:           } step {
...
def test_list_comprehension_variable():
    ...
    out = cudaq.run(kernela1, shots_count=1)
    assert (len(out) == 1 and out[0] == True)
    # keep after assert, such that we have no output if assert fails
    print(kernela1)
    ...
    out = cudaq.run(kernela2, shots_count=1)
    ...
    out = cudaq.run(kernela3, shots_count=1)
    assert (len(out) == 1 and out[0] == 5.0)
    # keep after assert, such that we have no output if assert fails
    print(kernela3)
...
def test_list_comprehension_capture():
    ...
    out = cudaq.run(kernelb1, shots_count=1)
    assert (len(out) == 1 and out[0] == True)
    # keep after assert, such that we have no output if assert fails
    print(kernelb1)
    ...
    out = cudaq.run(kernelb2, shots_count=1)
    assert (len(out) == 1 and out[0] == 5.)
    # keep after assert, such that we have no output if assert fails
    print(kernelb2)
    ...
    out = cudaq.run(kernelb3, shots_count=1)
    assert (len(out) == 1 and out[0] == 5.)
    # keep after assert, such that we have no output if assert fails
    print(kernelb3)


# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelb1..
# CHECK-SAME: (%[[VAL_0:.*]]: i1 {quake.pylifted}) -> i1 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return

# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelb2..
# CHECK-SAME: (%[[VAL_0:.*]]: f64 {quake.pylifted}) -> f64 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return

# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelb3..
...
def test_list_comprehension_list_of_constant():
    ...
    out = cudaq.run(kernelc1, shots_count=1)
    assert (len(out) == 1 and out[0] == True)
    # keep after assert, such that we have no output if assert fails
    print(kernelc1)
    ...
    out = cudaq.run(kernelc2, shots_count=1)
    ...
    out = cudaq.run(kernelc3, shots_count=1)
    assert (len(out) == 1 and out[0] == 1.)
    # keep after assert, such that we have no output if assert fails
    print(kernelc3)


# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelc1
# CHECK-SAME: () -> i1 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return

# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelc2
# CHECK-SAME: () -> f64 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return

# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelc3..
# CHECK-SAME: () -> f64 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
...
def test_list_comprehension_list_of_variable():
    ...
    out = cudaq.run(kerneld1, shots_count=1)
    assert (len(out) == 1 and out[0] == True)
    print(kerneld1
         )  # keep after assert, such that we have no output if assert fails
    ...
    out = cudaq.run(kerneld2, shots_count=1)
    assert (len(out) == 1 and out[0] == 1.)
    print(kerneld2
         )  # keep after assert, such that we have no output if assert fails
    ...
    out = cudaq.run(kerneld3, shots_count=1)
    assert (len(out) == 1 and out[0] == 1.)
    print(kerneld3
         )  # keep after assert, such that we have no output if assert fails


# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kerneld1
# CHECK-SAME: () -> i1 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kerneld2
# CHECK-SAME: () -> f64 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kerneld3
# CHECK-SAME: () -> f64 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
...
def test_list_comprehension_list_of_capture():
    ...
    out = cudaq.run(kernele1, shots_count=1)
    assert (len(out) == 1 and out[0] == True)
    # keep after assert, such that we have no output if assert fails
    print(kernele1)
    ...
    out = cudaq.run(kernele2, shots_count=1)
    assert (len(out) == 1 and out[0] == 1.)
    # keep after assert, such that we have no output if assert fails
    print(kernele2)
    ...
    out = cudaq.run(kernele3, shots_count=1)
    assert (len(out) == 1 and out[0] == 1.)
    # keep after assert, such that we have no output if assert fails
    print(kernele3)


# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernele1..
# CHECK-SAME: (%[[VAL_0:.*]]: i1 {quake.pylifted}) -> i1 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernele2
# CHECK-SAME: (%[[VAL_0:.*]]: f64 {quake.pylifted}) -> f64 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernele3
...
def test_list_comprehension_variable_list():
    ...
    out = cudaq.run(kernelf1, shots_count=1)
    assert (len(out) == 1 and out[0] == True)
    print(kernelf1
         )  # keep after assert, such that we have no output if assert fails
    ...
    out = cudaq.run(kernelf2, shots_count=1)
    assert (len(out) == 1 and out[0] == 1.)
    print(kernelf2
         )  # keep after assert, such that we have no output if assert fails
    ...
    out = cudaq.run(kernelf3, shots_count=1)
    assert (len(out) == 1 and out[0] == 1.)
    print(kernelf3
         )  # keep after assert, such that we have no output if assert fails


# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelf1
# CHECK-SAME: () -> i1 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelf2
# CHECK-SAME: () -> f64 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelf3
# CHECK-SAME: () -> f64 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
...
def test_list_comprehension_capture_list():
    ...
    out = cudaq.run(kernelg1, shots_count=1)
    assert (len(out) == 1 and out[0] == True)
    # keep after assert, such that we have no output if assert fails
    print(kernelg1)
    ...
    out = cudaq.run(kernelg2, shots_count=1)
    assert (len(out) == 1 and out[0] == 1.)
    # keep after assert, such that we have no output if assert fails
    print(kernelg2)
    ...
    out = cudaq.run(kernelg3, shots_count=1)
    assert (len(out) == 1 and out[0] == 1.)
    # keep after assert, such that we have no output if assert fails
    print(kernelg3)
...
def test_list_comprehension_tuple():
    ...
    out = cudaq.run(lenrek1, shots_count=1)
    assert (len(out) == 1 and out[0] == True)
    # keep after assert, such that we have no output if assert fails
    print(lenrek1)
    ...
    out = cudaq.run(lenrek4, shots_count=1)
    assert (len(out) == 1 and out[0] == True)
    # keep after assert, such that we have no output if assert fails
    print(lenrek4)
...
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__lenrek1
# CHECK-SAME: () -> i1 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL: func.func @__nvqpp__mlirgen__lenrek1..
# CHECK-SAME: .run() attributes {"cudaq-entrypoint", "cudaq-kernel", no_this, quake.cudaq_run = [i1]} {
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__lenrek2
# CHECK-SAME: () attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__lenrek3
# CHECK-SAME: ) -> !cc.struct<"MyTuple" {f64, f64}> attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__lenrek4
# CHECK-SAME: () -> i1 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
...
def test_list_comprehension_call():
    ...
    out = cudaq.run(kernelj1, shots_count=1)
    assert (len(out) == 1 and out[0] == 5.)
    # keep after assert, such that we have no output if assert fails
    print(kernelj1)
    ...
    out = cudaq.run(kernelj2, shots_count=1)
    assert (len(out) == 1 and out[0] == 5)
    # keep after assert, such that we have no output if assert fails
    print(kernelj2)

    @cudaq.kernel
    def kernelj3() -> MyTuple:
    ...
    out = cudaq.run(kernelj3, shots_count=1)
    assert (len(out) == 1 and out[0] == MyTuple(0., 5.))
    # keep after assert, such that we have no output if assert fails
    print(kernelj3)
    ...
    out = cudaq.run(kernelj4, shots_count=1)
    assert (len(out) == 1 and out[0] == 5.)
    # keep after assert, such that we have no output if assert fails
    print(kernelj4)
    ...
    out = cudaq.run(kernelj5, shots_count=1)
    assert (len(out) == 1 and out[0] == True)
    # keep after assert, such that we have no output if assert fails
    print(kernelj5)
    ...
    out = cudaq.run(kernelj6, shots_count=1)
...
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelj1
# CHECK-SAME: () -> f64 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelj2
# CHECK-SAME: () -> i64 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelj3
# CHECK-SAME: () -> !cc.struct<"MyTuple" {f64, f64}> attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelj4
# CHECK-SAME: ) -> f64 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelj5
# CHECK-SAME: () -> i1 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelj6
# CHECK-SAME: () -> i64 attributes {"cudaq-entrypoint", "cudaq-kernel"
# CHECK: return
...
def test_list_comprehension_expressions():
    ...
    out = cudaq.run(kernelm4, shots_count=1)
    ...
    out = cudaq.run(kernelm5, shots_count=1)
    assert (len(out) == 1 and out[0] == True)
    # keep after assert, such that we have no output if assert fails
    print(kernelm5)
    ...
    out = cudaq.run(kernelm6, shots_count=1)
...
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelm1
# CHECK-SAME: () attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelm2
# CHECK-SAME: () attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelm3
# CHECK-SAME: () attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelm4
# CHECK-SAME: () -> i64 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelm5
# CHECK-SAME: () -> i1 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
# CHECK-LABEL:   func.func @__nvqpp__mlirgen__kernelm6
# CHECK-SAME: () -> f64 attributes {"cudaq-entrypoint", "cudaq-kernel"}
# CHECK: return
...
def test_list_comprehension_qubit_refs():
    ...
    out = cudaq.sample(kerneln5)
    assert len(out) == 1 and '1111' in out
    # keep after assert, such that we have no output if assert fails
    print(kerneln5)

    @cudaq.kernel
    def kerneln6(qs: cudaq.qvector, indices: list[int]):
        target = cudaq.qubit()
        x.ctrl([qs[i] for i in indices], target)
...
# CHECK-LABEL: func.func @__nvqpp__mlirgen__kerneln1..
# CHECK-NOT: quake.concat
# CHECK: return
# CHECK-LABEL: func.func @__nvqpp__mlirgen__kerneln2..
# CHECK: quake.extract_ref
# CHECK: cc.loop
# CHECK: quake.concat
# CHECK: return
# CHECK-LABEL: func.func @__nvqpp__mlirgen__kerneln3..
# CHECK: quake.extract_ref
# CHECK: cc.loop
# CHECK: quake.concat
# CHECK: return
# CHECK-LABEL: func.func @__nvqpp__mlirgen__kerneln4..
# CHECK: quake.extract_ref
# CHECK: cc.loop
# CHECK: quake.concat
# CHECK: return
# CHECK-LABEL: func.func @__nvqpp__mlirgen__kerneln5..
# CHECK: quake.extract_ref
# CHECK: cc.loop
# CHECK: quake.concat
# CHECK: return
