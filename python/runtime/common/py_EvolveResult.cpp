...
void bindEvolveResult(py::module &mod) {
  py::class_<evolve_result>(
      ...
      /*
        ...
        Instantiates an EvolveResult representing the output generated when
        evolving a single initial state under a set of operators. See
        `cudaq.evolve` for more detail.

        Arguments:
            state: A single state or a sequence of states of a quantum system.
        If a single state is given, it represents the final state of the system
        after time evolution. If a sequence of states are given, they represent
        the state of the system after each steps in the schedule specified in
        the call to `cudaq.evolve`. expectation: An one-dimensional array of
        results from the call to `cudaq.observe` or a sequence of arrays
        thereof. If a single array is provided, it contains the results from the
        calls to `cudaq.observe` at the end of the evolution for each observable
        defined in the call to `cudaq.evolve`. If a sequence of arrays is
        provided, they represent the results computed at each step in the
