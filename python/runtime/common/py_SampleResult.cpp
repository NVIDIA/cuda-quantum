...
void bindMeasureCounts(py::module &mod) {
  ...
  py::class_<sample_result>(
      ...
      .def(
          ...
          "Returns the number of elements in `self`. Equivalent to the number "
          "of uniquely measured bitstrings.")
      .def(
          "__iter__",
          [](sample_result &self) {
      ...
      .def("expectation_z", &sample_result::exp_val_z,
           ...
           "that was sampled.")
      .def("probability", &sample_result::probability, py::arg("bitstring"),
           py::arg("register_name") = GlobalRegisterName,
      ...
      .def("get_marginal_counts",
           ...
           Args:
           marginal_indices (list[int]): A list of the qubit indices to 
           extract the measurement data from.
           
           register_name (Optional[str]): The optional measurement register 
           name to extract the counts data from. Defaults to the '__global__' 
           register.
