...
jitAndCreateArgs(const std::string &name, MlirModule module,
  ...
  if (allowCache && jitCache->hasJITEngine(hashKey)) {
    ...
    auto allowBreak = true;
    auto signalOnFailure = false;
    cudaq::opt::LoopUnrollOptions luo{threshold, signalOnFailure, allowBreak};
    ...
    // func.func(lower-to-cfg,canonicalize,multicontrol-decomposition),ionq-gate-set-mapping
    pm.addNestedPass<func::FuncOp>(createCanonicalizerPass());
    pm.addNestedPass<func::FuncOp>(cudaq::opt::createLiftArrayAlloc());
    ...
    pm.addNestedPass<func::FuncOp>(cudaq::opt::createLoopNormalize(lno));
    pm.addNestedPass<func::FuncOp>(createCanonicalizerPass());
    cudaq::opt::LoopUnrollOptions luo2{threshold, false, allowBreak};
    ...
    // pm.addPass(cudaq::opt::createBasisConversionPass(options));
