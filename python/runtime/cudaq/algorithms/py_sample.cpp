...
Returns:
  ...
  mod.def(
      "sample",
      [&](py::object kernel, py::args arguments, std::size_t shots_count,
          std::optional<noise_model> noise, std::size_t qpuId)
          -> std::variant<sample_result, std::vector<sample_result>> {
        ...
        if (!disallowBroadCast &&
            isBroadcastRequest(firstArgIsList, arguments)) {
          auto argSet = createArgumentSet(arguments);
          auto N = argSet.size();

          std::vector<sample_result> results;
          for (std::size_t currentIter = 0; auto &a : argSet) {
            // Launch the observation task
            auto ret = details::runSampling([&]() mutable { kernel(*a); },
                                            platform, kernelName, shots_count,
                                            0, nullptr, currentIter, N)
                           .value();
            currentIter++;
            results.push_back(ret);
          }
        ...
        // FIXME Future implementation can check for conditionals on
        // measurements
        ...
        [[maybe_unused]] auto hasConditionalOnMeasure =
            kernel.attr("metadata")
                ...
                .cast<bool>();
        auto res =
            details::runSampling([&]() mutable { kernel(*arguments); },
                                 platform, kernelName, shots_count, qpuId,
                                 nullptr, 0, 0, hasConditionalOnMeasure)
                .value();
        if (noise)
          unset_noise();
        return res;
...
Args:
  kernel (:class:`Kernel`): The CUDA Quantum kernel function to execute `shots_count`
    times on the QPU.
  *arguments (Optional[Any]): The concrete values to evaluate the kernel
    function at. Leave empty if the kernel doesn't accept any arguments. For 
    example, if the kernel takes two `float` values as input, the `sample` call 
