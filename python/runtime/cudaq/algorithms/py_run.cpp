...
static std::vector<nb::object> readRunResults(mlir::ModuleOp module,
                                              mlir::Type ty,
  ...
  for (std::size_t i = 0; i < results.lengthInBytes; i += byteSize) {
    nb::object obj = convertResult(module, ty, results.data + i);
    ret.push_back(obj);
  }
  return ret;
}
...
static std::vector<nb::object> pyReadResults(details::RunResultSpan results,
                                             mlir::ModuleOp mod,
...
void cudaq::bindPyRun(nb::module_ &mod) {
...
void cudaq::bindPyRunAsync(nb::module_ &mod) {
  nb::class_<async_run_result>(mod, "AsyncRunResultImpl", "")
      .def(
          "get",
