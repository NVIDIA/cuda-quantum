...
/// computation (PBC) form. Every Clifford (h/s/sdg/x/y/z/sx/cx/cz) is commuted
/// to the end via one symplectic stabilizer frame, so the program becomes a
/// sequence of pi/4 Pauli-product rotations (one per T/T-dagger, emitted as
/// `qlx.apply #qlx.action<pauli_rotation>` with angle pi/4) followed by the
/// terminal measurements conjugated into Pauli products.
///
