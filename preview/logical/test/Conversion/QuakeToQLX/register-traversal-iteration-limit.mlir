...
// RUN: not qlx-opt '--expand-quake-register-traversals=maximum-iterations=2' %s 2>&1 | FileCheck %s

module {
  func.func @over_iteration_limit() {
...
// CHECK: error: 'cc.loop' op wire-blocking register traversal has 3 iterations, exceeding the 2 preparation limit
