...
CUDA-Q Logical builds against CUDA-Q. The CUDA-Q installation supplies:
...
must resolve from the single `libcudaqMLIR` image provided by CUDA-Q.
...
`-DPython3_EXECUTABLE=/path/to/python` to pin a specific environment): CMake
adds the interpreter's `site-packages` to the front of `CMAKE_PREFIX_PATH`, so
CUDA-Q, LLVM, and MLIR all come from that one prefix.
...
Set `CUDAQ_INSTALL_PREFIX` to the install prefix of a CUDA-Q build. Use this
route to validate a CUDA-Q Logical change against an unreleased CUDA-Q work
tree.

CUDA-Q must be built with the LLVM/MLIR revision it expects. We therefore
recommend installing CUDA-Q with `CUDAQ_BUNDLE_MLIR_INSTALL=ON` so the matching
