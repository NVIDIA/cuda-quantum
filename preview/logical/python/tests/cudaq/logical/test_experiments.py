...
def test_definitions_attach_to_one_user_owned_mlir_module():
    owner = cql.compiler.CompilationContext()
    module = owner.module
    ...
    @cql.program
    def second() -> bool:
        return cql.measure_x(cql.ops.prepare_plus())

    first_build = cql.compile(first, module=module)
    cql.compile(cql.codes.Steane, module=module)
    second_build = cql.compile(second, module=module)
    cql.targets.mlir.materialize(module=module)
...
def test_direct_and_explicit_experiment_requests_normalize_identically():
    direct = cql.compile(experiment_memory, parameters={"bias": 10.0})
    explicit = cql.compile(
        cql.compiler.Experiment(root=experiment_memory,
    ...
    assert direct.experiment.stage == cql.stages.P0
...
def test_experiment_survives_a_clean_build_replay():
    build = cql.compile(
        cql.compiler.Experiment(
            root=experiment_memory,
            policy={"evidence": "require"},
            parameters={"rounds": 3},
        ))
    replayed = cql.compiler.Build.replay(build.serialize())
...
def test_placement_callback_is_eliminated_to_an_exact_witness():
    ...
    @cql.machine
    class OneSlot:
        memory = cql.architecture.Space(capacity=1)

    placed = cql.compile(
        experiment_memory,
        pipeline=cql.compiler.pipelines.placed(),
        device=OneSlot,
...
def test_compile_many_is_an_immutable_self_describing_sweep_bundle():
    points = tuple(
        cql.compiler.Experiment(root=experiment_memory, parameters={"p": p})
        for p in (1e-4, 1e-3, 1e-2))
    bundle = cql.compiler.compile_many(
        points, pipeline=cql.compiler.pipelines.logical())

    assert isinstance(bundle, cql.compiler.ExperimentBundle)
    assert len(bundle) == 3
...
def test_unsupported_profile_routes_fail_as_invalid_pipelines_not_placeholders(
    ...
    p0 = cql.compile(experiment_memory)
    with pytest.raises(ValueError,
                       match="no QLX compilation route from 'p0' to 'p2s'"):
        cql.compile(p0, pipeline=cql.compiler.pipelines.qec_definitions())
