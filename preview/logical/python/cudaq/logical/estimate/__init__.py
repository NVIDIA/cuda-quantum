...
from .types import FabricCounts, Tier
...
class _CallableEstimate(_py_types.ModuleType):
    ...
    def __call__(self, build, *, tier=Tier.STATIC, **kwargs):
        ...
        if tier is Tier.LOGICAL:
            if kwargs:
                raise TypeError("Tier.LOGICAL does not accept options")
            return logical_counts(_as_build(build))
        if tier is Tier.STATIC:
            if kwargs:
                raise TypeError("Tier.STATIC does not accept options")
...
def _as_build(value):
    ...
    preparation, so requiring an otherwise redundant ``cudaq.logical.compile`` at every
