...
.notranslate} decorator. Let's begin with a simple GHZ-state example,
producing a state of maximal entanglement amongst an allocated set of
qubits.
...
.notranslate} annotation. For the following example, we will define a
kernel for a simple GHZ-state as a standard free function.
...
the `qubit_count`{.code .docutils .literal .notranslate} argument allows
us to dynamically control the number of qubits allocated to the kernel.
