...
Previous](../../using/examples/multi_gpu_workflows.html "Multi-GPU Workflows"){.btn
.btn-neutral .float-left} [Next []{.fa .fa-arrow-circle-right
aria-hidden="true"}](noisy_simulations.html "Noisy Simulation"){.btn
...
::: {#Optimizers-and-Gradients .section}
...
`observe`{.docutils .literal .notranslate} expectation value). The
gradient is calculated using the `compute`{.docutils .literal
.notranslate} method.
...
CUDA-Q's `mqpu`{.docutils .literal .notranslate} backend allows for
parallel computation of parameter shift gradients using multiple
simulated QPUs. Gradients computed this way can be used in any of the
previously discussed optimization procedures. Below is an example
demonstrating how parallel gradient evaluation can be used for a VQE
procedure.

The parameter shift procedure computes two expectations values for each
parameter shifted forwards and backwards. These are used to estimate the
gradient contribution for that parameter.

The following code defines a function that takes a kernel, a Hamiltonian
(spin operator), and the circuit parameters and produces a parameter
shift gradient with shift `epsilon`{.docutils .literal .notranslate}.
The first step of the function builds `xplus`{.docutils .literal
.notranslate} and `xminus`{.docutils .literal .notranslate} , arrays
consisting of the shifted parameters.

Next, a for loop iterates over all of the parameters and uses the
`cudaq.observe_async`{.docutils .literal .notranslate} to compute the
expectation value. This command also takes `qpu_id`{.docutils .literal
.notranslate} as an in out which specifies the GPU that will be used to
simulate the ith QPU. In the example below, four GPUs (simulated QPUs)
are available so the gradient is batched over four devices.

The results are saved in the `g_plus`{.docutils .literal .notranslate}
and `g_minus`{.docutils .literal .notranslate} lists, the elements of
which are accessed with commands like
`g_plus[1].expectation()`{.docutils .literal .notranslate} to compute
the finite differences and construct the final gradient.
...
.literal .notranslate} is used to evaluate the gradient at each step.
This objective function will return the cost and gradient at each step
and can be used with any SciPy optimizer that uses a gradient.
