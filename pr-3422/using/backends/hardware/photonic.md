...
[![](../../../_images/orca_tbi.png){.align-center}](../../../_images/orca_tbi.png){.reference
...
.notranslate} that can be set so that the ORCA target can look for it
...
`ORCA_AUTH_TOKEN`{.docutils .literal .notranslate}. For example, if the
token is `token123`{.code .docutils .literal .notranslate}, you can set
...
To set which ORCA URL to be used, set the `url`{.code .docutils .literal
.notranslate} parameter.
...
flag `--target orca`{.docutils .literal .notranslate} to the
`nvq++`{.docutils .literal .notranslate} compiler. You should then pass
the `--orca-url`{.docutils .literal .notranslate} flag set with the
previously set environment variable `$ORCA_ACCESS_URL`{.docutils
.literal .notranslate} or an `url`{.code .docutils .literal
.notranslate}.
