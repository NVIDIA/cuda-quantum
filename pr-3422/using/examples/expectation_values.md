...
`ansatz`{.code .docutils .literal .notranslate} that takes as input a
single angle `theta`{.code .docutils .literal .notranslate}. This angle
becomes the argument of a single `ry`{.code .docutils .literal
.notranslate} rotation.

In host code, we define a Hamiltonian operator via the CUDA-Q
`spin_op`{.code .docutils .literal .notranslate} type. CUDA-Q provides a
generic function `cudaq::observe`{.code .docutils .literal
.notranslate}. This function takes as input three terms. The first two
terms are a parameterized kernel and the `spin_op`{.code .docutils
.literal .notranslate} whose expectation value we wish to compute. The
last term contains the runtime parameters at which we evaluate the
parameterized kernel.

The return type of this function is an `cudaq::observe_result`{.code
.docutils .literal .notranslate} which contains all the data from the
execution, but is trivially convertible to a double, resulting in the
...
.notranslate} platform by executing the following commands:
...
`nvidia-mqpu`{.code .docutils .literal .notranslate} platform is
