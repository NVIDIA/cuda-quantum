...
-   [Quick Start](#){.current .reference .internal}
    -   [Install CUDA-Q](#install-cuda-q){.reference .internal}
    -   [Validate your
        Installation](#validate-your-installation){.reference .internal}
    -   [CUDA-Q Academic](#cuda-q-academic){.reference .internal}
...
Previous](../index.html "CUDA-Q"){.btn .btn-neutral .float-left} [Next
[]{.fa .fa-arrow-circle-right
aria-hidden="true"}](basics/basics.html "CUDA-Q Basics"){.btn
...
Python and in C++. Learn more about the [key benefits of
CUDA-Q](https://developer.nvidia.com/cuda-q){.reference .external}.

This Quick Start page guides you through installing CUDA-Q and running
your first program. If you have already installed and configured CUDA-Q,
or if you are using our [Docker
image](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/quantum/containers/cuda-quantum){.reference
.external}, you can move directly to our [[Basics
Section]{.doc}](basics/basics.html){.reference .internal}. More
information about working with containers and Docker alternatives can be
found in our complete [[Installation
Guide]{.doc}](install/install.html){.reference .internal}.
...
Install CUDA-Q[¶](#install-cuda-q "Permalink to this heading"){.headerlink}
---------------------------------------------------------------------------
...
To develop CUDA-Q applications using Python, please follow the
instructions for [installing
CUDA-Q](https://pypi.org/project/cudaq/){.reference .external} from
PyPI. If you have an NVIDIA GPU, make sure to also follow the
instructions for enabling GPU-acceleration.

CUDA-Q does not require a GPU to use, but some components are
GPU-accelerated.

Getting started with CUDA-Q simply requires
`pip install cudaq`{.docutils .literal .notranslate}. Please make sure
your `pip`{.docutils .literal .notranslate} version is \>= 24.0. If you
have an NVIDIA GPU on your host system, you will be able to use it
...
> the `cuda-quantum`{.docutils .literal .notranslate},
> `cudaq-quantum-cu11`{.docutils .literal .notranslate}, or
> `cuda-quantum-cu12`{.docutils .literal .notranslate} package, and
> uninstall it prior to installation. Different CUDA-Q binary
...
.notranslate} from the `conda-forge`{.docutils .literal .notranslate}
channel on recent versions of Python can lead to segmentation faults. If
you see such errors, please switch to one of the Python versions
3.10.15, 3.11.10, or 3.12.7 as per the issue
...
*If you do not set these variables you may encounter a segmentation
fault.*

**Important**: It is *not* sufficient to set these variable within the
Conda environment, like the commands above do for
`LD_LIBRARY_PATH`{.docutils .literal .notranslate}. To avoid having to
set them every time you launch a new shell, we recommend adding them to
`~/.profile`{.docutils .literal .notranslate} (create the file if it
does not exist), and to `~/.bash_profile`{.docutils .literal
.notranslate} or `~/.bash_login`{.docutils .literal .notranslate} if
such a file exists.
...
.docutils .literal .notranslate} version 11 or newer. Download the
`install_cuda_quantum`{.code .docutils .literal .notranslate} file for
your processor architecture and CUDA version (`_cu11`{.code .docutils
.literal .notranslate} suffix for CUDA 11 and `_cu12`{.code .docutils
.literal .notranslate} suffix for CUDA 12) from the assets of the
respective [GitHub
release](https://github.com/NVIDIA/cuda-quantum/releases){.reference
.external}; that is, the file with the `aarch64`{.code .docutils
.literal .notranslate} extension for ARM processors, and the one with
`x86_64`{.code .docutils .literal .notranslate} for, e.g., Intel and AMD
processors.
...
Please see the complete [[installation guide]{.std
.std-ref}](install/local_installation.html#install-prebuilt-binaries){.reference
.internal} for more details, including

-   a list of [[supported operating systems]{.std
...
-   instructions on how to [[enable MPI parallelization]{.std
    ...
    .internal} within CUDA-Q, and

-   information about [[updating CUDA-Q]{.std
    .std-ref}](install/local_installation.html#updating-cuda-quantum){.reference
    .internal}.
...
::: {#validate-your-installation .section}
[]{#validate-installation}

Validate your Installation[¶](#validate-your-installation "Permalink to this heading"){.headerlink}
---------------------------------------------------------------------------------------------------

Let's run a simple program to validate your installation. The quantum
kernel in the following program creates and measures the state
[\\((\|00\\rangle + \|11\\rangle) / \\sqrt{2}\\)]{.math .notranslate
.nohighlight}. That means each kernel execution should either yield
`00`{.code .docutils .literal .notranslate} or `11`{.code .docutils
.literal .notranslate}. The program samples, meaning it executes, the
kernel 1000 times and prints how many times each output was measured. On
average, the values `00`{.code .docutils .literal .notranslate} and
`11`{.code .docutils .literal .notranslate} should be observed around
500 times each.
...
::: {.tab-content .docutils}
Create a file titled `program.py`{.code .docutils .literal
.notranslate}, containing the following code:
...
::: {.tab-content .docutils}
Create a file titled `program.cpp`{.code .docutils .literal
.notranslate}, containing the following code:
...
Compile the program using the `nvq++`{.code .docutils .literal
.notranslate} compiler and run the built application with the following
command:
...
.docutils .literal .notranslate} target ([[GPU-accelerated statevector
simulator]{.std
.std-ref}](backends/sims/svsims.html#cuquantum-single-gpu){.reference
.internal}) to the time when setting the target to `qpp-cpu`{.code
.docutils .literal .notranslate} ([[OpenMP parallelized CPU-only
statevector simulator]{.std
.std-ref}](backends/sims/svsims.html#openmp-cpu-only){.reference
.internal}):
...
When you change the target to `qpp-cpu`{.code .docutils .literal
.notranslate}, the program simply seems to hang; that is because it
takes a long time for the CPU-only backend to simulate 28+ qubits!
Cancel the execution with `Ctrl+C`{.code .docutils .literal
.notranslate}.

For more information about enabling GPU-acceleration, please see our
complete [[Installation Guide]{.std
.std-ref}](install/local_installation.html#additional-cuda-tools){.reference
.internal}. For further information on available targets, see
...
::: {#cuda-q-academic .section}
CUDA-Q Academic[¶](#cuda-q-academic "Permalink to this heading"){.headerlink}
-----------------------------------------------------------------------------

[CUDA-Q Academic](https://github.com/NVIDIA/cuda-q-academic){.reference
.external} is a collection of courses designed for academic use based on
the CUDA-Q platform. Courses range in complexity from the basics of
quantum computing up to implementations of algorithms from the research
literature. Users will learn quantum computing, CUDA-Q, and how to run
hybrid algorithms that make the most of QPUs and AI supercomputers
through coding exercises and interactive lessons.

![[Example of the interactive material within CUDA-Q
Academic.]{.caption-text}[¶](#id2 "Permalink to this image"){.headerlink}](../_images/widget-recording.gif)

The courses can be run using a local installation of CUDA-Q or via the
cloud using `qBraid`{.code .docutils .literal .notranslate},
`CoCalc`{.code .docutils .literal .notranslate}, or `Google Colab`{.code
.docutils .literal .notranslate}.

Users just learning quantum computing are encouraged to begin with the
[Quick Start to
Quantum](https://github.com/NVIDIA/cuda-q-academic/blob/main/quick-start-to-quantum/README.md){.reference
.external} course which provides an introduction to the basics of
quantum computing and helps you implement your first algorithm.

Advanced users can choose from a number of learning paths spanning
topics ranging from QEC to hybrid algorithms
[here](https://github.com/NVIDIA/cuda-q-academic){.reference .external}
.
:::
:::
:::
