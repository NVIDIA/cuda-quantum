...
-   [Examples](../../using/examples/examples.html){.reference .internal}
    ...
    -   [Measuring
        ...
        -   [Mid-circuit Measurement and Conditional
            ...
            .internal}
    -   [Visualizing Kernels](#){.current .reference .internal}
        -   [Qubit Visualization](#Qubit-Visualization){.reference
            .internal}
        -   [Kernel Visualization](#Kernel-Visualization){.reference
            .internal}
    -   [Executing
...
Previous](../../using/examples/measuring_kernels.html "Measuring Kernels"){.btn
.btn-neutral .float-left accesskey="p"} [Next []{.fa
.fa-arrow-circle-right
aria-hidden="true"}](../../using/examples/executing_kernels.html "Executing Kernels"){.btn
...
# Visualization[¶](#Visualization "Permalink to this heading"){.headerlink}

::: {#Qubit-Visualization .section}
## Qubit Visualization[¶](#Qubit-Visualization "Permalink to this heading"){.headerlink}

What are the possible states a qubit can be in and how can we build up a
visual cue to help us make sense of quantum states and their evolution?

We know our qubit can have two distinct states: [\\(\\ket{0}\\)]{.math
.notranslate .nohighlight} and [\\(\\ket{1}\\)]{.math .notranslate
.nohighlight}. If these were the only two states, we could represent
them as two vectors on a one-dimensional line (i.e., the z-axis in the
image below). We also know that qubits can be in an equal superposition
of states: [\\(\\ket{+}\\)]{.math .notranslate .nohighlight} and
[\\(\\ket{-}\\)]{.math .notranslate .nohighlight}. In order to capture
all of the states in equal superposition, we will need a 2D plane (i.e.,
the [\\(xy\\)]{.math .notranslate .nohighlight}-plane in the image
below). If you dive deeper you will learn about the existence of other
states that will call for a 3D extension.

In general, a quantum state can be written in the form [\\(\\ket{\\psi}
=
\\cos(\\frac{\\theta}{2})\\ket{0}+e\^{i\\varphi}\\sin(\\frac{\\theta}{2})\\ket{1}\\)]{.math
.notranslate .nohighlight} where [\\(\\theta\\)]{.math .notranslate
.nohighlight} is a real number between [\\(0\\)]{.math .notranslate
.nohighlight} and [\\(\\pi\\)]{.math .notranslate .nohighlight} and
[\\(\\varphi\\)]{.math .notranslate .nohighlight} is a real value
between [\\(0\\)]{.math .notranslate .nohighlight} and
[\\(2\\pi\\)]{.math .notranslate .nohighlight}. For example, the minus
state, [\\(\\ket{-} = \\frac{1}{\\sqrt{2}}\\ket{0}-
\\frac{1}{\\sqrt{2}}\\ket{1}\\)]{.math .notranslate .nohighlight}, can
be rewritten as

::: {.math .notranslate .nohighlight}
\\\[\\ket{-} =
\\cos(\\frac{\\theta}{2})\\ket{0}+e\^{i\\varphi}\\sin(\\frac{\\theta}{2})\\ket{1}\\text{
with }\\theta = \\frac{\\pi}{2}\\text{ and }\\varphi = \\pi.\\\]
:::

This can be visualized in the image below as a unit vector pointing in
the direction of the negative [\\(x\\)]{.math .notranslate
.nohighlight}-axis.

Using spherical coordinates, it is possible to depict all the possible
states of a single qubit on a sphere. This is called a Bloch sphere.

![Bloch Sphere](../../_images/Bloch_sphere.png){.no-scaled-link
style="width: 300px; height: 300px;"}

Let us try to showcase the functionality to render such a 3D
representation with CUDA-Q. First, let us define a single-qubit kernel
that returns a different state each time. This kernel uses random
rotations.

Note: CUDA-Q uses the [QuTiP](https://qutip.org){.reference .external}
library to render Bloch spheres. The following code will throw an error
...
[`cudaq.add_to_bloch_sphere()`{.docutils .literal .notranslate}]{.pre}
on the output state obtained from the random kernel.
...
We can display the spheres with [`cudaq.show()`{.docutils .literal
.notranslate}]{.pre}. Show the first sphere:
...
![](../../_images/examples_python_visualization_7_0.png)
:::
:::

We can also show multiple Bloch spheres side by side - simply set the
[`nrows`{.docutils .literal .notranslate}]{.pre} and [`ncols`{.docutils
.literal .notranslate}]{.pre} in the call to [`cudaq.show()`{.docutils
.literal .notranslate}]{.pre} accordingly. Make sure to have more spaces
than spheres in your list, else it will throw an error! Let us show two
spheres in a row:
...
::: {.output_area .docutils .container}
![](../../_images/examples_python_visualization_9_0.png)
:::
:::

We can show them in a column too, if we want! Simply set the
[`nrows`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal
.notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils
.literal .notranslate}[`2`{.docutils .literal .notranslate}]{.pre} and
[`ncols`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal
.notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils
.literal .notranslate}[`1`{.docutils .literal .notranslate}]{.pre}.
...
::: {.output_area .docutils .container}
![](../../_images/examples_python_visualization_11_0.png)
:::
:::
...
::: {.output_area .docutils .container}
![](../../_images/examples_python_visualization_13_0.png)
:::
:::

What if we had to add multiple vectors to a single Bloch sphere? CUDA-Q
uses the [QuTiP](https://www.qutip.org){.reference .external} toolbox to
construct Bloch spheres. We can then add multiple states to the same
Bloch sphere by passing the sphere object as an argument to
[`cudaq.add_to_bloch_sphere()`{.docutils .literal .notranslate}]{.pre}.
...
This created a single Bloch sphere with 10 random vectors. Let us see
how it looks.
...
::: {.output_area .docutils .container}
![](../../_images/examples_python_visualization_17_0.png)
:::
:::

Unfortunately, there is no such handy visualization for multi-qubit
states. In particular, a multi-qubit state cannot be visualized as
multiple Bloch spheres due to the nature of entanglement that makes
quantum computing so powerful.
:::

::: {#Kernel-Visualization .section}
## Kernel Visualization[¶](#Kernel-Visualization "Permalink to this heading"){.headerlink}

A CUDA-Q kernel can be visualized using the [`cudaq.draw`{.docutils
.literal .notranslate}]{.pre} API which returns a string representing
the drawing of the execution path, in the specified format. ASCII
(default) and LaTeX formats are supported.
...
![Circuit in PDF format](../../_images/circuit_pdf.png){.no-scaled-link
