...
-   [Examples](examples.html){.reference .internal}
    ...
    -   [Detector Error Models](dem_from_kernel.html){.reference
        ...
        -   [Limitations](dem_from_kernel.html#limitations){.reference
            .internal}
    -   [Rotation Synthesis (Clifford+T)](#){.current .reference
        .internal}
        -   [Synthesizing a
            rotation](#synthesizing-a-rotation){.reference .internal}
        -   [Estimating the T count of a
            kernel](#estimating-the-t-count-of-a-kernel){.reference
            .internal}
        -   [Choosing epsilon](#choosing-epsilon){.reference .internal}
        -   [Dependencies](#dependencies){.reference .internal}
...
::: {#rotation-synthesis-clifford-t .section}
# Rotation Synthesis (Clifford+T)[¶](#rotation-synthesis-clifford-t "Permalink to this heading"){.headerlink}

It is typical in fault-tolerant quantum computing to consider only the
discrete Cliffords augmented with the T gate for universality, like
[`H`{.docutils .literal .notranslate}]{.pre}, [`S`{.docutils .literal
.notranslate}]{.pre}, [`T`{.docutils .literal .notranslate}]{.pre},
[`X`{.docutils .literal .notranslate}]{.pre} and [`CNOT`{.docutils
.literal .notranslate}]{.pre}, the Clifford+T set. A continuous rotation
such as [`rz(0.6)`{.docutils .literal .notranslate}]{.pre} is not a
member of this set, so before a kernel can run on such hardware every
rotation must be synthesized into a sequence of digital operations which
approximates it.

CUDA-Q does this with Gridsynth (Ross and Selinger,
[arXiv:1403.2975](https://arxiv.org/abs/1403.2975){.reference
.external}). Given an angle and a tolerance [`epsilon`{.docutils
.literal .notranslate}]{.pre}, it finds a short Clifford+T sequence
[\\(U\\)]{.math .notranslate .nohighlight} satisfying
[\\(\\\|R_z(\\theta) - U\\\| \\le \\epsilon\\)]{.math .notranslate
.nohighlight} in the operator norm.

There are two ways to use it. [`cudaq.synth`{.docutils .literal
.notranslate}]{.pre} is a Python API for approximating individual
rotations, and is shown first. Synthesizing a whole kernel is done by a
compiler pass, which is opt-in. It is not part of any default target
pipeline, and the targets that run it are benchmarking targets for
estimating how many T gates an algorithm would cost.

::: {#synthesizing-a-rotation .section}
## Synthesizing a rotation[¶](#synthesizing-a-rotation "Permalink to this heading"){.headerlink}

To approximate a rotation, use [`cudaq.synth.gridsynth`{.docutils
.literal .notranslate}]{.pre}. Note that the [`synth`{.docutils .literal
.notranslate}]{.pre} submodule is imported explicitly:
...
[`gridsynth`{.docutils .literal .notranslate}]{.pre} returns a
[`CliffordTSequence`{.docutils .literal .notranslate}]{.pre}. Its
[`t_count`{.docutils .literal .notranslate}]{.pre} is the number of T
gates, and [`str()`{.docutils .literal .notranslate}]{.pre} gives the
gate string over [`{H,`{.docutils .literal
.notranslate}]{.pre}` `{.docutils .literal .notranslate}[`S,`{.docutils
.literal .notranslate}]{.pre}` `{.docutils .literal
.notranslate}[`T,`{.docutils .literal .notranslate}]{.pre}` `{.docutils
.literal .notranslate}[`X,`{.docutils .literal
.notranslate}]{.pre}` `{.docutils .literal .notranslate}[`W}`{.docutils
.literal .notranslate}]{.pre}, where [`W`{.docutils .literal
.notranslate}]{.pre} is the global phase [\\(\\omega =
e\^{i\\pi/4}\\)]{.math .notranslate .nohighlight}. The gates are listed
in matrix-multiplication order, so as a circuit they apply right to
left.

[`cudaq.synth.rz_error`{.docutils .literal .notranslate}]{.pre} reports
the error the sequence actually achieves. It is computed exactly in
arbitrary precision, and it never exceeds the [`epsilon`{.docutils
.literal .notranslate}]{.pre} you asked for.

A [`CliffordTSequence`{.docutils .literal .notranslate}]{.pre} can also
be built directly from a gate string, which is useful for inspecting
Clifford+T circuits that came from somewhere else.
[`normalized`{.docutils .literal .notranslate}]{.pre} rewrites a
sequence into Matsumoto-Amano normal form. An exactly equal sequence
with the smallest possible T count.
...
[`gridsynth`{.docutils .literal .notranslate}]{.pre} already returns
sequences in normal form, so normalizing its output changes nothing.

To use a synthesized sequence in a circuit, [`to_kernel`{.docutils
.literal .notranslate}]{.pre} builds a kernel that takes a single qubit,
ready for [`apply_call`{.docutils .literal .notranslate}]{.pre}:
...
The synthesized sequence reproduces the exact rotation. Note that
[`to_kernel`{.docutils .literal .notranslate}]{.pre} drops the
[`W`{.docutils .literal .notranslate}]{.pre} phase factors, so the
kernel it returns equals [\\(R_z(\\theta)\\)]{.math .notranslate
.nohighlight} only up to a global phase. That phase has no effect when
the kernel is used on its own, but it becomes an observable relative
phase if the kernel is made the target of a controlled operation. The
[`compiler-bench-ftqc-clifford-t`{.docutils .literal
.notranslate}]{.pre} target does not have this limitation. It emits the
phase explicitly, so the circuit it produces is exactly equal to the
original.
:::

::: {#estimating-the-t-count-of-a-kernel .section}
## Estimating the T count of a kernel[¶](#estimating-the-t-count-of-a-kernel "Permalink to this heading"){.headerlink}

The T gate is the expensive resource on fault-tolerant hardware, so the
T count of a circuit is the number usually worth measuring. The
[`compiler-bench-ftqc-clifford-t`{.docutils .literal
.notranslate}]{.pre} target runs synthesis over a whole kernel so that
count can be read off directly. It reduces [`rx`{.docutils .literal
.notranslate}]{.pre}, [`ry`{.docutils .literal .notranslate}]{.pre} and
[`r1`{.docutils .literal .notranslate}]{.pre} rotations to
[`rz`{.docutils .literal .notranslate}]{.pre}, synthesizes each one, and
optimizes the result. Its [`epsilon`{.docutils .literal
.notranslate}]{.pre} argument sets the per-rotation tolerance and
defaults to [`1e-10`{.docutils .literal .notranslate}]{.pre}.
...
This is a benchmarking target for resource counting, not a path for
compiling kernels to run on hardware. It is also Python only. An
[`nvq++`{.docutils .literal .notranslate}]{.pre} build accepts the
target but does not run the synthesis.

Rotation angles must be compile-time constants when synthesis runs. A
kernel whose angle comes from a runtime argument has to be specialized
first.
:::

::: {#choosing-epsilon .section}
## Choosing epsilon[¶](#choosing-epsilon "Permalink to this heading"){.headerlink}

[`epsilon`{.docutils .literal .notranslate}]{.pre} trades accuracy
against cost. A tighter tolerance means more T gates and more compile
time; the T count grows roughly with [\\(\\log(1/\\epsilon)\\)]{.math
.notranslate .nohighlight}, while compile time grows much faster and
becomes significant below about [`1e-30`{.docutils .literal
.notranslate}]{.pre}.

Synthesis results are cached per distinct angle, so a kernel that
applies the same rotation a hundred times pays the cost once.

Synthesis is randomized. Repeated calls with the same angle and
tolerance return sequences with the same T count, but not necessarily
the same gates. Pass [`seed`{.docutils .literal .notranslate}]{.pre} to
make a run reproducible, as the example above does. The remaining
arguments to [`gridsynth`{.docutils .literal .notranslate}]{.pre} are
work budgets that trade compile time against T count; see the [[Python
API reference]{.doc}](../../api/languages/python_api.html){.reference
.internal} for what each one bounds.
:::

::: {#dependencies .section}
## Dependencies[¶](#dependencies "Permalink to this heading"){.headerlink}

Rotation synthesis uses exact arbitrary-precision arithmetic, provided
by the GMP and MPFR libraries. These ship with every CUDA-Q binary
distribution, so no action is needed to use the feature. See [[Dynamic
linking to GMP and MPFR]{.std
.std-ref}](../install/local_installation.html#dynamic-linking-gmp-mpfr){.reference
.internal} for details on how they are packaged and how to substitute
your own builds.
:::
:::
:::
