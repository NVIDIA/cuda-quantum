    ...
    -   [Messaging Protocol](protocol.html){.reference .internal}
        ...
        -   [QEC-Specific Usage
            ...
            -   [Decoding
                ...
                .internal}
    -   [CPU RoCE Transport](#){.current .reference .internal}
        -   [C ABI](#c-abi){.reference .internal}
        -   [Two-phase bring-up ([`setup`{.docutils .literal
            .notranslate}]{.pre} / [`connect`{.docutils .literal
            .notranslate}]{.pre})](#two-phase-bring-up-setup-connect){.reference
            .internal}
        -   [TX modes](#tx-modes){.reference .internal}
        -   [Testing ([`hsb_bridge_cpu`{.docutils .literal
            .notranslate}]{.pre})](#testing-hsb-bridge-cpu){.reference
            .internal}
    -   [Device Call Channels](device_call.html){.reference .internal}
        -   [The [`device_call`{.docutils .literal .notranslate}]{.pre}
            model](device_call.html#the-device-call-model){.reference
            .internal}
        -   [Selecting a
            channel](device_call.html#selecting-a-channel){.reference
            .internal}
        -   [Extending an in-process
            service](device_call.html#extending-an-in-process-service){.reference
            .internal}
        -   [The [`cpu_roce`{.docutils .literal .notranslate}]{.pre}
            channel](device_call.html#the-cpu-roce-channel){.reference
            .internal}
            -   [Wire pattern
                (FPGA-compatible)](device_call.html#wire-pattern-fpga-compatible){.reference
                .internal}
            -   [Connection
                setup](device_call.html#connection-setup){.reference
                .internal}
            -   [Running it](device_call.html#running-it){.reference
                .internal}
            -   [Test harness](device_call.html#test-harness){.reference
                .internal}
...
Previous](protocol.html "CUDA-Q Realtime Messaging Protocol"){.btn
.btn-neutral .float-left accesskey="p"} [Next []{.fa
.fa-arrow-circle-right
aria-hidden="true"}](device_call.html "CUDA-Q device_call Channels"){.btn
...
::: {#cpu-roce-transport .section}
# CPU RoCE Transport[¶](#cpu-roce-transport "Permalink to this heading"){.headerlink}

[`CpuRoceTransceiver`{.docutils .literal .notranslate}]{.pre} is a
**pure-CPU [`RoCEv2`{.docutils .literal .notranslate}]{.pre} RDMA
transport** -- no GPU, no DOCA, no GpuRoceTransceiver/HSB. It moves RPC
messages between a quantum control system (FPGA) and a CPU-based decoder
entirely in host memory at microsecond latency. It is a reference
implementation of the [`libibverbs`{.docutils .literal
.notranslate}]{.pre} transport option described in the [[CUDA-Q Realtime
Host API]{.std .std-doc}](host.html){.reference .internal}: it
implements the same ring-buffer slot + flag protocol, so it plugs into
the realtime dispatcher the same way a GPU transport does.
...
the [`GpuRoceTransceiver`{.docutils .literal .notranslate}]{.pre}
ring-buffer API so the same dispatcher wiring works with either, and it
pairs naturally with the [`CUDAQ_DISPATCH_HOST_CALL`{.docutils .literal
.notranslate}]{.pre} dispatch mode for a fully GPU-free decode loop.

It serves two kinds of consumers:

-   **Direct realtime dispatch**: wired to a
    [`libcudaq-realtime`{.docutils .literal .notranslate}]{.pre}
    dispatcher (e.g. [`hsb_bridge_cpu`{.docutils .literal
    .notranslate}]{.pre} with [`CUDAQ_DISPATCH_HOST_CALL`{.docutils
    .literal .notranslate}]{.pre}), as a drop-in CPU replacement for the
    GPU HSB transport.

-   **Compiler-lowered [`device_call`{.docutils .literal
    .notranslate}]{.pre}**: it is the transport behind the
    [[[`cpu_roce`{.docutils .literal .notranslate}]{.pre} device-call
    channel]{.std .std-doc}](device_call.html){.reference .internal}, so
    a quantum kernel's [`cudaq::device_call`{.docutils .literal
    .notranslate}]{.pre} -- lowered by [`nvq++`{.docutils .literal
    .notranslate}]{.pre} to the realtime RPC ABI -- can be dispatched to
    a **CPU-based** service over RDMA, with no GPU on either the
    transport or the handler side.

**HSB interoperability.** Although this end runs no HSB software, the
on-wire RDMA framing is **HSB-compatible**: the peer can be a real FPGA
containing the **HSB IP**. The HSB IP receives RDMA Sends and transmits
RDMA Writes, so [`CpuRoceTransceiver`{.docutils .literal
.notranslate}]{.pre} interoperates with it directly by selecting the
matching [[TX mode]{.std .std-ref}](#tx-modes){.reference .internal} --
the same FPGA/bridge wire the GPU [`GpuRoceTransceiver`{.docutils
.literal .notranslate}]{.pre} uses. The only HSB-specific piece is the
out-of-band control plane used to exchange QP/rkey at connection time
(see below); the data plane is identical.

::: {#c-abi .section}
## C ABI[¶](#c-abi "Permalink to this heading"){.headerlink}

A C ABI shim ([`roce_wrapper.h`{.docutils .literal .notranslate}]{.pre})
exposes the transceiver so callers need not include the C++ header:
...
The RX/TX rings live in host memory; the worker threads poll
CPU-accessible flags and move data with [`libibverbs`{.docutils .literal
.notranslate}]{.pre} UC (Unreliable Connected) queue pairs.
:::

::: {#two-phase-bring-up-setup-connect .section}
## Two-phase bring-up ([`setup`{.docutils .literal .notranslate}]{.pre} / [`connect`{.docutils .literal .notranslate}]{.pre})[¶](#two-phase-bring-up-setup-connect "Permalink to this heading"){.headerlink}

Connected (UC) QPs require each end to know the other's QP number before
any traffic flows, so bring-up is split:

1.  [`cpu_roce_setup()`{.docutils .literal .notranslate}]{.pre} opens
    the device, registers the host rings, creates the QP, and
    transitions it to INIT. After this,
    [`cpu_roce_get_qp_number()`{.docutils .literal .notranslate}]{.pre}
    and [`cpu_roce_get_rkey()`{.docutils .literal .notranslate}]{.pre}
    are valid to hand to the peer.

2.  The two ends exchange [`{qp_number,`{.docutils .literal
    .notranslate}]{.pre}` `{.docutils .literal
    .notranslate}[`rkey,`{.docutils .literal
    .notranslate}]{.pre}` `{.docutils .literal
    .notranslate}[`roce_ipv4}`{.docutils .literal .notranslate}]{.pre}
    out of band. Against a real FPGA this is the HSB control plane
    ([`authenticate`{.docutils .literal .notranslate}]{.pre} /
    [`configure_roce`{.docutils .literal .notranslate}]{.pre}); tests
    use a minimal socket rendezvous so nothing pulls in an HSB
    dependency.

3.  [`cpu_roce_connect()`{.docutils .literal .notranslate}]{.pre} adopts
    the peer parameters and transitions the QP to RTR/RTS.

Only the rendezvous step changes between deployments; the data-plane
wire does not.
:::

::: {#tx-modes .section}
## TX modes[¶](#tx-modes "Permalink to this heading"){.headerlink}

[`cpu_roce_tx_mode_t`{.docutils .literal .notranslate}]{.pre} names the
RDMA verb this transceiver's TX path issues. The name describes the verb
the transmitter uses, not a peer role -- whoever sets the mode is the
source:

-   [`CPU_ROCE_TX_MODE_RDMA_SEND`{.docutils .literal
    .notranslate}]{.pre}: TX issues [`IBV_WR_SEND`{.docutils .literal
    .notranslate}]{.pre} into the peer's pre-posted recv WQEs. Use when
    the peer consumes Sends -- e.g. an FPGA's HSB IP (which can only
    receive Sends), or a transceiver acting as the responder.

-   [`CPU_ROCE_TX_MODE_RDMA_WRITE_WITH_IMM`{.docutils .literal
    .notranslate}]{.pre}: TX issues
    [`IBV_WR_RDMA_WRITE_WITH_IMM`{.docutils .literal
    .notranslate}]{.pre} into the peer's rx ring using the peer's rkey,
    with the slot index in the immediate. Use when the peer's memory is
    the write target -- e.g. an FPGA pushing syndromes, or a transceiver
    acting as the requester.

These compose into an asymmetric, FPGA-compatible pattern: a requester
Writes into the responder's ring and the responder Sends results back,
mirroring how a real FPGA RDMA-Writes syndromes and receives Sends. The
[[[`cpu_roce`{.docutils .literal .notranslate}]{.pre} device-call
channel]{.std .std-doc}](device_call.html){.reference .internal} is
built on exactly this pattern.
:::

::: {#testing-hsb-bridge-cpu .section}
## Testing ([`hsb_bridge_cpu`{.docutils .literal .notranslate}]{.pre})[¶](#testing-hsb-bridge-cpu "Permalink to this heading"){.headerlink}

[`hsb_bridge_cpu`{.docutils .literal .notranslate}]{.pre}
([`realtime/unittests/cpu_transport/`{.docutils .literal
.notranslate}]{.pre}) is a GPU-less bridge that wires
[`CpuRoceTransceiver`{.docutils .literal .notranslate}]{.pre} to a
[`CUDAQ_DISPATCH_HOST_CALL`{.docutils .literal .notranslate}]{.pre}
dispatcher -- the CPU analogue of the GPU HSB bridge.
[`hsb_test_cpu.sh`{.docutils .literal .notranslate}]{.pre} orchestrates
it against either the HSB FPGA emulator ([`--emulate`{.docutils .literal
.notranslate}]{.pre}) or a **real FPGA containing the HSB IP** (driven
over the HSB control plane, exactly as the GPU path is):
