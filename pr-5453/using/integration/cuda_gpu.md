...
-   [Integration](integration.html){.reference .internal}
    -   [Downstream CMake Integration](cmake_app.html){.reference
        .internal}
    -   [Combining CUDA with CUDA-Q](#){.current .reference .internal}
    -   [Integrating with Third-Party
        Libraries](libraries.html){.reference .internal}
        -   [Calling a CUDA-Q library from
            C++](libraries.html#calling-a-cuda-q-library-from-c){.reference
            .internal}
        -   [Calling an C++ library from
            CUDA-Q](libraries.html#calling-an-c-library-from-cuda-q){.reference
            .internal}
        -   [Interfacing between binaries compiled with a different
            toolchains](libraries.html#interfacing-between-binaries-compiled-with-a-different-toolchains){.reference
            .internal}
...
Previous](cmake_app.html "CUDA-Q and CMake"){.btn .btn-neutral
.float-left accesskey="p"} [Next []{.fa .fa-arrow-circle-right
aria-hidden="true"}](libraries.html "Integrating with Third-Party Libraries"){.btn
...
::: {#using-cuda-and-cuda-q-in-a-project .section}
# Using CUDA and CUDA-Q in a Project[¶](#using-cuda-and-cuda-q-in-a-project "Permalink to this heading"){.headerlink}

It may be the case that a project that uses CUDA-Q kernels may also want
to use CUDA code to do computation on a GPU. This is possible by using
both the CUDA Toolkit and CUDA-Q tools. More about programming GPUs in
CUDA can be found in the [Quick Start
Guide](https://docs.nvidia.com/cuda/cuda-quick-start-guide/index.html){.reference
.external}.

Once the [`nvcc`{.code .docutils .literal .notranslate}]{.pre} compiler
is installed, it is possible to write CUDA kernels and have them execute
on the system GPU. See NVIDIA's [An Easy Introduction to CUDA C and
C++](https://developer.nvidia.com/blog/easy-introduction-cuda-c-and-c/){.reference
.external} for more information on getting started with CUDA.

CUDA code uses a unique syntax and is, typically, saved in a file with
the extension [`.cu`{.code .docutils .literal .notranslate}]{.pre}. For
our example, assume we have written our CUDA code in the file
[`my_proj.cu`{.code .docutils .literal .notranslate}]{.pre}.

CUDA-Q code is a library-based extension of C++ and uses standard
conforming C++ syntax. Typically, a quantum kernel would be saved in a
file with the [`.cpp`{.code .docutils .literal .notranslate}]{.pre}
extension. Again for our example, let's assume that we've written
quantum kernels and saved them in the file [`my_proj_quantum.cpp`{.code
.docutils .literal .notranslate}]{.pre}.

By default, CUDA-Q uses C++ 20 and builds source code against the LLVM
C++ standard library ([`libc++`{.code .docutils .literal
.notranslate}]{.pre}). To create a CUDA library that can link against
CUDA-Q code, make sure to define an API that does not rely on C++ data
structures that rely on a specific C++ toolchain for all functions
intended to be called from CUDA-Q (see also [[Interfacing between
binaries compiled with a different toolchains]{.std
.std-ref}](libraries.html#clike-shim){.reference .internal}). For
example, if you define a CUDA kernel in [`my_proj.cu`{.code .docutils
...
Create the corresponding header file [`my_proj.h`{.code .docutils
.literal .notranslate}]{.pre} declaring the [`setFirstNElements`{.code
.docutils .literal .notranslate}]{.pre} template. You can then call
[`setFirstNElements`{.code .docutils .literal .notranslate}]{.pre} from
within [`my_proj_quantum.cpp`{.code .docutils .literal
.notranslate}]{.pre} by including [`my_proj.h`{.code .docutils .literal
.notranslate}]{.pre}, for example
...
Above, [`nvq++`{.code .docutils .literal .notranslate}]{.pre} is used
for the link step and will make sure the CUDA-Q runtime libraries are
linked correctly to the executable program. The CUDA runtime is
explicitly added to this command.
:::
:::
:::
