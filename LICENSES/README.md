...
`cudaq/lib/Target/cmake/Dependencies.cmake`), and are statically linked into
the CUDAQ target catalog library as private dependencies. reflect-cpp bundles
the header-only CTRE and enchantum libraries under `include/rfl/thirdparty`;
`LICENSE.ctre` reproduces the Apache License 2.0 text with LLVM Exceptions
embedded in the bundled `ctre.hpp` header. reflect-cpp also bundles the yyjson header, but that code
is not compiled into CUDA-Q binaries because reflect-cpp's JSON support is
disabled; it is therefore not listed above.
