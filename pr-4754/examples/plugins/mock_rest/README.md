...
-   [Examples](../../../using/examples/examples.html){.reference
    ...
    -   [Measuring
        ...
        -   [Mid-circuit Measurement and Conditional
            ...
            .internal}
    -   [Visualizing
        Kernels](../../python/visualization.html){.reference .internal}
        -   [Qubit
            Visualization](../../python/visualization.html#Qubit-Visualization){.reference
            .internal}
        -   [Kernel
            Visualization](../../python/visualization.html#Kernel-Visualization){.reference
            .internal}
    -   [Executing
    ...
    -   [Multi-GPU
        ...
        -   [Parallel execution over multiple QPUs ([`mqpu`{.code
            ...
            -   [Circuit
                ...
                .internal}
    -   [Optimizers &
        Gradients](../../python/optimizers_gradients.html){.reference
        .internal}
        -   [CUDA-Q Optimizer
            Overview](../../python/optimizers_gradients.html#CUDA-Q-Optimizer-Overview){.reference
            .internal}
            -   [Gradient-Free Optimizers (no gradients
                required):](../../python/optimizers_gradients.html#Gradient-Free-Optimizers-(no-gradients-required):){.reference
                .internal}
            -   [Gradient-Based Optimizers (require
                gradients):](../../python/optimizers_gradients.html#Gradient-Based-Optimizers-(require-gradients):){.reference
                .internal}
        -   [1. Built-in CUDA-Q Optimizers and
            Gradients](../../python/optimizers_gradients.html#1.-Built-in-CUDA-Q-Optimizers-and-Gradients){.reference
            .internal}
            -   [1.1 Adam Optimizer with Parameter
                Configuration](../../python/optimizers_gradients.html#1.1-Adam-Optimizer-with-Parameter-Configuration){.reference
                .internal}
            -   [1.2 SGD (Stochastic Gradient Descent)
                Optimizer](../../python/optimizers_gradients.html#1.2-SGD-(Stochastic-Gradient-Descent)-Optimizer){.reference
                .internal}
            -   [1.3 SPSA (Simultaneous Perturbation Stochastic
                Approximation)](../../python/optimizers_gradients.html#1.3-SPSA-(Simultaneous-Perturbation-Stochastic-Approximation)){.reference
                .internal}
        -   [2. Third-Party
            Optimizers](../../python/optimizers_gradients.html#2.-Third-Party-Optimizers){.reference
            .internal}
        -   [3. Parallel Parameter Shift
            Gradients](../../python/optimizers_gradients.html#3.-Parallel-Parameter-Shift-Gradients){.reference
            .internal}
    -   [Noisy
        Simulations](../../python/noisy_simulations.html){.reference
        .internal}
    ...
    -   [Constructing
        ...
        -   [Pauli Words and Exponentiating Pauli
            ...
            .internal}
    -   [Performance
        Optimizations](../../python/performance_optimizations.html){.reference
        .internal}
        -   [Gate
            Fusion](../../python/performance_optimizations.html#Gate-Fusion){.reference
            .internal}
    ...
    -   [Dynamics
        ...
        -   [Python Examples (Jupyter
            ...
            -   [Introduction to CUDA-Q Dynamics (Jaynes-Cummings
                Model)](../../python/dynamics/dynamics_intro_1.html){.reference
                .internal}
            -   [Introduction to CUDA-Q Dynamics (Time Dependent
                Hamiltonians)](../../python/dynamics/dynamics_intro_2.html){.reference
                .internal}
            -   [Superconducting
                Qubits](../../python/dynamics/superconducting.html){.reference
                .internal}
            -   [Spin
                Qubits](../../python/dynamics/spinqubits.html){.reference
                .internal}
            -   [Trapped Ion
                Qubits](../../python/dynamics/iontrap.html){.reference
                .internal}
            -   [Control](../../python/dynamics/control.html){.reference
                .internal}
        -   [C++
...
::: {#mock-rest-plugin .section}
# mock_rest Plugin[¶](#mock-rest-plugin "Permalink to this heading"){.headerlink}

[`mock_rest`{.docutils .literal .notranslate}]{.pre} is a reference
REST-style external QPU plugin. It demonstrates how to add a new target
by providing a [`ServerHelper`{.docutils .literal .notranslate}]{.pre}
while reusing CUDA-Q's built-in [`remote_rest`{.docutils .literal
.notranslate}]{.pre} QPU implementation. It also demonstrates how a
plugin author can build the target as a Python package that can be
distributed to and installed by end users.

This example is intentionally small. It does not simulate the submitted
quantum program. Instead, it sends a configured bitstring to a simple
mock REST service and expects that same bitstring as the sample result.

::: {#what-it-demonstrates .section}
## What It Demonstrates[¶](#what-it-demonstrates "Permalink to this heading"){.headerlink}

-   A standalone external plugin directory with [`targets/`{.docutils
    .literal .notranslate}]{.pre}, [`lib/`{.docutils .literal
    .notranslate}]{.pre}, and Python package metadata generated into
    [`build/external/mock-rest`{.docutils .literal .notranslate}]{.pre}.

-   A custom [`cudaq::ServerHelper`{.docutils .literal
    .notranslate}]{.pre} registered under the target name
    [`mock_rest`{.docutils .literal .notranslate}]{.pre}.

-   A target YAML file that selects [`platform-qpu:`{.docutils .literal
    .notranslate}]{.pre}` `{.docutils .literal
    .notranslate}[`remote_rest`{.docutils .literal .notranslate}]{.pre}.

-   Required [`cudaq-version`{.docutils .literal .notranslate}]{.pre}
    metadata that prevents selection by an older CUDA-Q installation and
    warns across newer major or minor release lines.

-   Python auto-discovery through the [`cudaq.backends`{.docutils
    .literal .notranslate}]{.pre} entry point.

-   Distribution as a Python package that end users can install with
    [`pip`{.docutils .literal .notranslate}]{.pre}.

-   Optional registration of the installed package in the user plugin
    scope for [`nvq++`{.docutils .literal .notranslate}]{.pre}.
:::
...
-   [`targets/mock_rest.yml.in`{.docutils .literal .notranslate}]{.pre}:
    declares the [`mock_rest`{.docutils .literal .notranslate}]{.pre}
    target, selects [`remote_rest`{.docutils .literal
    .notranslate}]{.pre}, records the CUDA-Q version used to build the
    plugin, links [`-lcudaq-rest-qpu`{.docutils .literal
    .notranslate}]{.pre}, and exposes [`url`{.docutils .literal
    .notranslate}]{.pre} and [`value`{.docutils .literal
    .notranslate}]{.pre} target arguments.

-   [`MockRestServerHelper.cpp`{.docutils .literal .notranslate}]{.pre}:
    implements the REST contract for this backend. It builds a job
    payload containing the compiled kernel, shot count, and the
    configured [`value`{.docutils .literal .notranslate}]{.pre}
    bitstring. When the server returns that value, the helper maps it to
    CUDA-Q sample counts.

-   [`python/__init__.py.in`{.docutils .literal .notranslate}]{.pre}:
    registers the built plugin root with CUDA-Q when the package is
    discovered through the [`cudaq.backends`{.docutils .literal
    .notranslate}]{.pre} entry point.

-   [`python/__main__.py.in`{.docutils .literal .notranslate}]{.pre}:
    exposes [`python3`{.docutils .literal
    .notranslate}]{.pre}` `{.docutils .literal
    .notranslate}[`-m`{.docutils .literal
    .notranslate}]{.pre}` `{.docutils .literal
    .notranslate}[`cudaq_example_mock_rest`{.docutils .literal
    .notranslate}]{.pre}` `{.docutils .literal
    .notranslate}[`--install-nvqpp`{.docutils .literal
    .notranslate}]{.pre}, which installs the built plugin into the user
    plugin scope.

For local testing, this plugin is meant to talk to the generic
[`mock_qpu`{.docutils .literal .notranslate}]{.pre} echo server in
CUDA-Q's Python test utilities. It lives in
[`python/tests/utils/mock_qpu/echo/`{.docutils .literal
.notranslate}]{.pre} and is launched through the shared test server
driver:
...
The echo server accepts any POST payload, stores the payload's
[`value`{.docutils .literal .notranslate}]{.pre}, and returns that same
value from [`GET`{.docutils .literal .notranslate}]{.pre}` `{.docutils
.literal .notranslate}[`/jobs/<id>`{.docutils .literal
.notranslate}]{.pre}.
:::

::: {#build .section}
## Build[¶](#build "Permalink to this heading"){.headerlink}
...
-   [`targets/mock_rest.yml`{.docutils .literal .notranslate}]{.pre}

-   [`lib/libcudaq-serverhelper-mock_rest.so`{.docutils .literal
    .notranslate}]{.pre} on Linux, or the platform equivalent

-   [`pyproject.toml`{.docutils .literal .notranslate}]{.pre},
    [`__init__.py`{.docutils .literal .notranslate}]{.pre}, and
    [`__main__.py`{.docutils .literal .notranslate}]{.pre} for Python
    packaging
:::

::: {#test .section}
## Test[¶](#test "Permalink to this heading"){.headerlink}
...
The test checks that the plugin build output exists, the YAML target is
valid, the plugin can be installed into a user plugin scope,
[`nvq++`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal
.notranslate}[`--list-targets`{.docutils .literal .notranslate}]{.pre}
finds it, and Python can load the target through direct registration and
...
[`cudaq.sample(...)`{.docutils .literal .notranslate}]{.pre} submits the
REST job and waits for the result. Use
[`cudaq.sample_async(...).get()`{.docutils .literal .notranslate}]{.pre}
for the same flow when you want the future-based API.
:::

::: {#install-as-an-end-user .section}
## Install as an End User[¶](#install-as-an-end-user "Permalink to this heading"){.headerlink}

The build produces a standard Python package containing the target YAML
and shared library. A plugin author can build a wheel from this package,
publish it to a package index, or otherwise distribute it to end users.
The commands below show how an end user installs and enables that
distributed target. They are distinct from the plugin-author build steps
above.
...
package for each operating system and architecture that they intend to
support, and ensure that each wheel has the corresponding platform tag.

For this local example, install the package directly from its build
directory (an end user would normally install the published package name
or wheel):
...
its [`cudaq.backends`{.docutils .literal .notranslate}]{.pre} entry
point, so explicit [`cudaq.register_backend_path(...)`{.docutils
.literal .notranslate}]{.pre} is no longer needed.

To make the installed Python package visible to [`nvq++`{.docutils
