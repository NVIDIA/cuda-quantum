...
Quantum noise can be characterized into coherent and incoherent sources
of errors that arise during a computation. Coherent noise is commonly
due to systematic errors originating from device miscalibrations, for
example, gates implementing a rotation [\\(\\theta + \\epsilon\\)]{.math
.notranslate .nohighlight} instead of [\\(\\theta\\)]{.math .notranslate
.nohighlight}.

Incoherent noise has its origins in quantum states being entangled with
the environment due to decoherence. This leads to mixed states which are
probability distributions over pure states and are described by
employing the density matrix formalism.

We can model incoherent noise via quantum channels which are linear,
completely positive, and trace preserving maps. These maps are called
Kraus operators, [\\(\\{ K\_i \\}\\)]{.math .notranslate .nohighlight},
which satisfy the condition [\\(\\sum\_{i} K\_i\^\\dagger K\_i =
\\mathbb{I}\\)]{.math .notranslate .nohighlight}.

The bit-flip channel flips the qubit with probability [\\(p\\)]{.math
.notranslate .nohighlight} and leaves it unchanged with probability
[\\(1-p\\)]{.math .notranslate .nohighlight}. This can be represented by
employing Kraus operators:

::: {.math .notranslate .nohighlight}
\\\[\\begin{split}K\_0 = \\sqrt{1-p} \\begin{pmatrix} 1 & 0 \\\\ 0 & 1
\\end{pmatrix}\\end{split}\\\]
:::

::: {.math .notranslate .nohighlight}
\\\[\\begin{split}K\_1 = \\sqrt{p} \\begin{pmatrix} 0 & 1 \\\\ 1 & 0
\\end{pmatrix}\\end{split}\\\]
:::
...
In addition to gate-based noise injection, CUDA-Q also supports
fine-grained, explicit noise injection via the
`cudaq.apply_noise`{.docutils .literal .notranslate} function. You can
place this method in your kernels, and inject specific noise on specific
qubits wherever you need.
