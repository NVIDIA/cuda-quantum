...
CUDA-Q's `Kernel`{.code .docutils .literal .notranslate} API.

When you create a `Kernel`{.code .docutils .literal .notranslate} and
invoke its methods, a quantum program is constructed that can then be
executed by calling, for example, `cudaq::sample`{.code .docutils
.literal .notranslate}. Let's take a closer look!
...
CUDA-Q kernels are any typed callable in the language that is annotated
with the `__qpu__`{.code .docutils .literal .notranslate} attribute.
Let's take a look at a very simple "Hello World" example, specifically a
CUDA-Q kernel that prepares a GHZ state on a programmer-specified number
of qubits.
...
.literal .notranslate} that is templated on a `size_t`{.code .docutils
.literal .notranslate} parameter. Our kernel expression is free to use
this template parameter in the allocation of a compile-time-known
register of qubits. Within the kernel, we are free to apply various
quantum operations, like a Hadamard on qubit 0 `h(q[0])`{.code .docutils
.literal .notranslate}. Controlled operations are **modifications** of
single-qubit operations, like the `x<cudaq::ctrl>(q[0],q[1])`{.code
.docutils .literal .notranslate} operation which implements a
controlled-X gate. We can measure single qubits or entire registers.
...
`cudaq::sample`{.code .docutils .literal .notranslate} function, which
returns a data type encoding the qubit measurement strings and the
corresponding number of times that string was observed (here the default
number of shots is used, `1000`{.code .docutils .literal .notranslate}).
