...
-   [Applications](../../using/applications.html){.reference .internal}
    ...
    -   [Bernstein-Vazirani
        ...
        -   [Implementing in
            ...
            .internal}
    -   [Cost Minimization](#){.current .reference .internal}
...
Previous](bernstein_vazirani.html "Bernstein-Vazirani Algorithm"){.btn
.btn-neutral .float-left accesskey="p"} [Next []{.fa
.fa-arrow-circle-right
aria-hidden="true"}](deutsch_algorithm.html "Deutsch’s Algorithm"){.btn
...
::: {#Cost-Minimization .section}
# Cost Minimization[¶](#Cost-Minimization "Permalink to this heading"){.headerlink}

Below we start with a basic example of a hybrid variational algorithm
which involves flipping the Bloch vector of a qubit from the
[\\(\\ket{0}\\)]{.math .notranslate .nohighlight} to the
[\\(\\ket{1}\\)]{.math .notranslate .nohighlight} state. First we import
the relevant packages and set our backend to simulate our workflow on
NVIDIA GPUs.
...
We build our cost function such that its minimal value corresponds to
the qubit being in the [\\(\\ket{1}\\)]{.math .notranslate .nohighlight}
state. The observe call below allows us to simulate our statevector
[\\(\\ket{\\psi}\\)]{.math .notranslate .nohighlight}, and calculate
[\\(\\bra{\\psi}Z\\ket{\\psi}\\)]{.math .notranslate .nohighlight}.
...
Below we use our built-in optimization suite to minimize the cost
function. We will be using the gradient-free COBYLA alogrithm.
...
::: {.output_area .docutils .container}
![](../../_images/applications_python_cost_minimization_8_1.png)
:::
:::

We see that the final value or our cost function,
[\\(\\bra{\\psi}Z\\ket{\\psi} = -1\\)]{.math .notranslate .nohighlight}
demonstrating that the qubit is in the [\\(\\ket{1}\\)]{.math
.notranslate .nohighlight} state.
:::
:::
:::
