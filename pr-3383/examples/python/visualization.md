    ...
    -   [Dynamics
        ...
        -   [Introduction to CUDA-Q Dynamics (Jaynes-Cummings
            ...
            -   [Section 2 - Simulating open quantum systems with the
                ...
                .notranslate}](dynamics/dynamics_intro_1.html#Section-2---Simulating-open-quantum-systems-with-the-collapse_operators){.reference
                .internal}
...
`cudaq.add_to_bloch_sphere()`{.docutils .literal .notranslate} on the
output state obtained from the random kernel.
...
.notranslate}. Show the first sphere:
...
`nrows`{.docutils .literal .notranslate} and `ncols`{.docutils .literal
.notranslate} in the call to `cudaq.show()`{.docutils .literal
.notranslate} accordingly. Make sure to have more spaces than spheres in
your list, else it will throw an error! Let us show two spheres in a
row:
...
`nrows = 2`{.docutils .literal .notranslate} and `ncols = 1`{.docutils
.literal .notranslate}.
...
`cudaq.add_to_bloch_sphere()`{.docutils .literal .notranslate}.
...
.literal .notranslate} API which returns a string representing the
drawing of the execution path, in the specified format. ASCII (default)
and LaTeX formats are supported.
